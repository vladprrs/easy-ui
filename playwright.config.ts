...
export default defineConfig({
  ...
  webServer: [
    ...
    {
      command: `rm -rf .e2e-data/dev && ${adminEnv} ${surfacesEnv} DATA_DIR=.e2e-data/dev PORT=8787 PUBLIC_ORIGIN=http://${viteHost}:5173 ${bun} server/main.ts`,
    ...
    },
    {
      ...
      command: `rm -rf .e2e-data/auth-preview && until ${bun} -e "const r=await fetch('http://127.0.0.1:4173/api/health').catch(()=>null);process.exit(r?.ok?0:1)"; do sleep 1; done; ${adminEnv} ${surfacesEnv} DATA_DIR=.e2e-data/auth-preview SERVE_DIST=dist PORT=4174 LEGACY_BASIC_AUTH=edge:secret PUBLIC_ORIGIN=http://127.0.0.1:4174 ${bun} server/main.ts`,
