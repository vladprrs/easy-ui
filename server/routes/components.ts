...
export async function routeComponents(request:Request,db:Database,segments:string[],principal:Principal,dataDir:string):Promise<Response>{
  ...
  if(segments.length===1){if(request.method==="GET")return json(repo.list(),200,noStore);if(request.method==="POST"){const actor=requireUser(principal);const b=body(await readJson(request));const id=text(b.id,"id")!,name=text(b.name,"name")!,source=text(b.source,"source")!,designSystem=text(b.designSystem,"designSystem",false)??DEFAULT_DESIGN_SYSTEM_ID;requireResourceOwner(db,"design_systems",designSystem,principal);if(!slug.test(id))bad("id must be a slug","id");if(!componentName.test(name))bad("name must match ^[A-Z][A-Za-z0-9]*$","name");reserveHostPrimitiveName(name);requireRegisteredDesignSystem(db,designSystem,["designSystem"]);if(Object.values(designSystems).some(system=>Object.hasOwn(system.definitions,name)))throw new ApiError(409,"already_exists","Component name conflicts with a builtin component");const path=await materializeSource(dataDir,id,1,source);await checkSource(source,path);const figma=parseFigmaInput(db,b.figma,"figma");const result=repo.create(id,name,source,designSystem,text(b.message,"message",false),figma,actor.userId);db.query("UPDATE component_revisions SET author=? WHERE component_id=? AND rev=1").run(actor.userId,id);writeAuditEvent(db,{actorId:actor.userId,action:"component.revision.saved",subjectType:"component",subjectId:id,detail:{rev:1}});return json(result,201,{...noStore,location:`/api/components/${id}`});}throw new ApiError(405,"method_not_allowed","Method not allowed");}
  const id=segments[1]!,tail=segments.slice(2);
  if(!tail.length){if(request.method==="GET")return json(repo.meta(id),200,noStore);if(request.method==="PUT"){const actor=requireResourceOwner(db,"components",id,principal);reserveHostPrimitiveName(repo.meta(id).name);const b=body(await readJson(request)),source=text(b.source,"source",false),designSystem=text(b.designSystem,"designSystem",false),baseRev=base(b);const figmaProvided=Object.hasOwn(b,"figma");const figma=figmaProvided?parseFigmaInput(db,b.figma,"figma"):null;if(source===undefined&&designSystem===undefined&&!figmaProvided)throw new ApiError(400,"invalid_request","source, designSystem or figma is required");if(designSystem!==undefined){requireRegisteredDesignSystem(db,designSystem,["designSystem"]);requireResourceOwner(db,"design_systems",designSystem,principal);}const current=repo.cas(id,baseRev),head=repo.source(id,current.head_rev),nextSource=source??head.source,nextSystem=designSystem??current.design_system;if(nextSource===head.source&&nextSystem===current.design_system&&!figmaProvided)throw new ApiError(400,"invalid_request","Component source and design system are unchanged");const next=current.head_rev+1,path=await materializeSource(dataDir,id,next,nextSource);await checkSource(nextSource,path);const result=repo.save(id,source,designSystem,baseRev,text(b.message,"message",false),figma);db.query("UPDATE component_revisions SET author=? WHERE component_id=? AND rev=?").run(actor.userId,id,result.rev);writeAuditEvent(db,{actorId:actor.userId,action:"component.revision.saved",subjectType:"component",subjectId:id,detail:{rev:result.rev}});return json(result,200,noStore);}if(request.method==="DELETE"){requireResourceOwner(db,"components",id,principal);const b=body(await readJson(request));repo.delete(id,base(b));return new Response(null,{status:204,headers:noStore});}throw new ApiError(405,"method_not_allowed","Method not allowed");}
  ...
  if(tail[0]==="publish"&&tail.length===1){if(request.method!=="POST")throw new ApiError(405,"method_not_allowed","Method not allowed");const actor=requireResourceOwner(db,"components",id,principal);requireResourceOwner(db,"design_systems",repo.row(id).design_system,principal);const b=body(await readJson(request));const result=await publishComponent(db,repo,id,base(b),dataDir,text(b.message,"message",false));writeAuditEvent(db,{actorId:actor.userId,action:"component.version.published",subjectType:"component",subjectId:id,detail:{version:result.version}});return json(result,201,{...noStore,location:`/api/components/${id}/versions/${result.version}`});}
  if(tail[0]==="versions"){
