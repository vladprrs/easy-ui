...
export function catalogManifest(db:Database){return (db.query(`SELECT c.id,c.name,c.design_system,p.version,p.bundle_hash,p.definition_meta,p.host_abi_version FROM components c JOIN component_publishes p ON p.component_id=c.id AND p.status='active' WHERE c.deleted_at IS NULL AND p.version=(SELECT MAX(x.version) FROM component_publishes x WHERE x.component_id=c.id AND x.status='active') ORDER BY c.id`).all() as {id:string;name:string;design_system:string;version:number;bundle_hash:string;definition_meta:string;host_abi_version:number}[]).map(r=>({id:r.id,name:r.name,designSystem:r.design_system,version:r.version,bundleUrl:`/api/components/${encodeURIComponent(r.id)}/versions/${r.version}/bundle.js`,bundleHash:r.bundle_hash,...JSON.parse(r.definition_meta),hostAbiVersion:r.host_abi_version}));}
...
export async function routeComponents(request:Request,db:Database,segments:string[],dataDir:string):Promise<Response>{
  ...
  if(segments.length===1){if(request.method==="GET")return json(repo.list(),200,noStore);if(request.method==="POST"){const b=body(await readJson(request));const id=text(b.id,"id")!,name=text(b.name,"name")!,source=text(b.source,"source")!,designSystem=text(b.designSystem,"designSystem",false)??DEFAULT_DESIGN_SYSTEM_ID;if(!slug.test(id))bad("id must be a slug","id");if(!componentName.test(name))bad("name must match ^[A-Z][A-Za-z0-9]*$","name");if(!Object.hasOwn(designSystems,designSystem))bad(`unknown design system: ${designSystem}`,"designSystem");if(Object.values(designSystems).some(system=>Object.hasOwn(system.definitions,name)))throw new ApiError(409,"already_exists","Component name conflicts with a builtin component");const path=await materializeSource(dataDir,id,1,source);await checkSource(source,path);const result=repo.create(id,name,source,designSystem,text(b.message,"message",false));return json(result,201,{...noStore,location:`/api/components/${id}`});}throw new ApiError(405,"method_not_allowed","Method not allowed");}
