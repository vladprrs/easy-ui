 ...
 *
 * **Санкционированный сдвиг W1b** (план 2026-08-07 §W1b, точки 1–2): кадр теперь несёт безусловные
 * факты `rootBounds`/`rootClip`/`referenceExportDims`, поэтому `geometry.json` каждого **нового**
 * рана длиннее на эти ключи, и его `bytes` входит в нормализованный манифест. Сдвиг именно такой,
 * какой волна обязана оплатить: замер безусловен by design (иначе первая декларация ожидания
 * стоила бы пересъёмки, AC §3.4), вердикты доволновых случаев при этом не меняются (golden
 * байт-идентичности легаси-ветки — `geometryPolicy.test.ts`), а уже записанные манифесты принятых
