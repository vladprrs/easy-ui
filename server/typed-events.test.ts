...
async function setup() { const dir = await mkdtemp(resolve(process.cwd(), ".typed-events-test-")); dirs.push(dir); const db = openDatabase(":memory:"); const handler = createTestHandler(db, { dataDir: dir }); return { dir, db, handler }; }
...
describe("typed event payloads + ABI v2", () => {
  test("publishes typed events with a serialized eventPayloads schema, ABI 2 via capabilities", async () => {
    ...
    expect((await handler(req("/components", "POST", {designSystem:"yandex-pay", id: "typed-stars", name: "TypedStars", source: await fixture("typed-events-stars.tsx"), intent:"Collects typed rating events from product cards" }))).status).toBe(201);
  ...
  test("rejects a non-serializable event payload schema with event_schema_not_serializable", async () => {
    const { db, handler } = await setup();
    const response = await handler(req("/components", "POST", {designSystem:"yandex-pay", id: "bad-event", name: "BadEvent", source: await fixture("nonserializable-event.tsx"), intent:"Tests rejection of invalid event payload schemas" }));
  ...
  test("computes host ABI 2 from an easy-ui/runtime import and maps shims to v2", async () => {
    const { db, handler } = await setup();
    expect((await handler(req("/components", "POST", {designSystem:"yandex-pay", id: "token-user", name: "TokenUser", source: await fixture("token-import.tsx"), intent:"Renders tokenized content using the runtime API" }))).status).toBe(201);
  ...
  test("legacy string[] events publish as ABI 1 with no eventPayloads", async () => {
    const { db, handler } = await setup();
    expect((await handler(req("/components", "POST", {designSystem:"yandex-pay", id: "rating-stars", name: "RatingStars", source: await fixture("rating-stars.tsx"), intent:"Collects star ratings from product card users" }))).status).toBe(201);
