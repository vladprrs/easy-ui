...
function v14():Database {
  ...
  // v34 завела кадры экранов галереи (impacted snap) — тот же приём.
  db.run("DROP TABLE IF EXISTS prototype_screen_frames");
  // v35 завела саги миграционного коммита — тот же приём.
  db.run("DROP TABLE IF EXISTS migration_commits");
