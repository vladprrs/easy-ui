...
import { componentDefinitions, normalizeDefinitions, type ComponentDefinition } from "../src/catalog/definitions";
...
export async function snapshotDefinitions(db:Database,doc:PrototypeDoc,dataDir:string):Promise<{definitions:Record<string,ComponentDefinition>;pins:ComponentPin[]}> {
  const types=new Set(doc.screens.flatMap(s=>Object.values(s.spec.elements).map(e=>e.type)).filter(t=>!Object.hasOwn(componentDefinitions,t)));
  ...
  for(const name of [...types].sort()) {
    const row=db.query(`SELECT c.id,c.name,cp.version,cp.rev,cp.bundle_hash bundleHash,cr.source
      ...
      WHERE c.name=? AND c.deleted_at IS NULL ORDER BY cp.version DESC LIMIT 1`).get(name) as {id:string;name:string;version:number;rev:number;bundleHash:string;source:string}|null;
    if(!row) throw new ApiError(422,"validation_failed","Prototype document is invalid",{issues:[{path:["screens"],message:`Unknown or unpublished component type: ${name}`}]});
  ...
  return {definitions:{...componentDefinitions,...normalizeDefinitions(custom)},pins};
}
