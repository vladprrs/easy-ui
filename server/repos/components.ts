...
export class ComponentRepo {
  ...
  create(id:string,name:string,source:string,designSystem="shadcn",message?:string,figmaJson:string|null=null) { return this.db.transaction(()=>{if(this.db.query("SELECT 1 FROM components WHERE id=? OR name=?").get(id,name)) throw new ApiError(409,"already_exists","Component id or name already exists"); const at=now(); this.db.query("INSERT INTO components (id,name,head_rev,design_system,deleted_at,created_at,updated_at) VALUES (?,?,1,?,NULL,?,?)").run(id,name,designSystem,at,at); this.db.query("INSERT INTO component_revisions (component_id,rev,source,design_system,figma_json,message,created_at) VALUES (?,1,?,?,?,?,?)").run(id,source,designSystem,figmaJson,message??null,at); return {id,rev:1 as const};})(); }
