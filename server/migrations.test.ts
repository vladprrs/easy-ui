...
function rollbackPostV22(db:Database):void {
  ...
  // v34 (план 2026-08-07 §W5) создала кадры экранов галереи — то же правило отката.
...
test("migrations upgrade a fresh v0 database to latest and a v16 database is idempotent",()=>{
  ...
  expect((db.query("PRAGMA user_version").get() as {user_version:number}).user_version).toBe(37);
 ...
 */
/**
 * Откат колонок v37 (BR-06) до состояния «populated vN»: тесты ниже строят базу нужной версии,
 * мигрируя её целиком и снимая всё, что появилось позже. Вынесено функцией, потому что снимать
 * приходится в семи тестах подряд, и разъехаться они не должны.
 */
function dropV37Columns(db:Database):void {
  db.run("DROP INDEX acceptance_runs_resumed_from");
  db.run("ALTER TABLE acceptance_cases DROP COLUMN error_json");
  db.run("ALTER TABLE acceptance_runs DROP COLUMN resumed_from_run_id");
  db.run("ALTER TABLE acceptance_runs DROP COLUMN attempt");
  db.run("ALTER TABLE acceptance_runs DROP COLUMN resume_json");
}
...
test("v30 adds multi-run provenance columns to a populated v29 database without touching existing rows",()=>{
  ...
  // Искусственный откат до v29 снимает и колонки v31/v32/v33 — иначе повторный прогон ловит
  ...
  db.run("ALTER TABLE acceptance_runs DROP COLUMN overlay_manifest_json");
...
/**
 * v33 (план 2026-08-07 §W3): candidate dependency overlay рана. Проверяется на **populated** базе:
 * доволновой ран обязан пережить миграцию с NULL в обеих колонках (backfill'а нет — overlay-ранов
 * до неё не существовало), а состав INSERT'а v32 обязан продолжать работать: колонки nullable и
 * без DEFAULT, иначе откат образа ронял бы постановку ранов.
 */
test("v33 adds overlay columns to a populated v32 database without touching existing runs",()=>{
  const db=new Database(":memory:"); migrate(db);
  dropV37Columns(db);
  db.run("ALTER TABLE acceptance_runs DROP COLUMN overlay_manifest_json");
  db.run("ALTER TABLE acceptance_runs DROP COLUMN overlay_hash");
  ...
  db.run("INSERT INTO component_revisions (component_id,rev,source,design_system,created_at) VALUES ('yp-ovl',1,'src','yandex-pay','now')");
  db.query(`INSERT INTO component_candidates
    (candidate_id,component_id,design_system,rev,source_hash,bundle_hash,host_abi_version,theme_version,build_fingerprint,
     observed_catalog_revision,policy_profile_hash,status,created_by,created_at,expires_at)
    VALUES ('cand_ovl','yp-ovl','yandex-pay',1,'src-hash','bundle-hash',4,NULL,'bf','cat','ph','validated','u',?,?)`).run(at,at);
  db.query(`INSERT INTO acceptance_runs
    (run_id,candidate_id,component_id,status,policy_profile_hash,policy_profile_id,progress_json,gates_json,created_by,created_at)
    VALUES ('acc_ovl','cand_ovl','yp-ovl','pass','ph','default-v1','{}','{}','u',?)`).run(at);
  ...
  expect(()=>db.query(`INSERT INTO acceptance_runs
    (run_id,candidate_id,component_id,status,policy_profile_hash,policy_profile_id,progress_json,gates_json,created_by,created_at)
    VALUES ('acc_ovl2','cand_ovl','yp-ovl','queued','ph','default-v1','{}','{}','u',?)`).run(at)).not.toThrow();
  expect(db.query("SELECT overlay_manifest_json manifest FROM acceptance_runs WHERE run_id='acc_ovl2'").get()).toEqual({manifest:null});
  expect(db.query("PRAGMA foreign_key_check").all()).toEqual([]);
  db.close();
});

/**
 * v34 (план 2026-08-07 §W5): кадры экранов галереи. Проверяется на **populated** v33-базе: таблица
 * создаётся поверх живых прототипов и ревизий, каскадится по ревизии, держит несколько условий
 * съёмки одной ревизии экрана (light/dark) и не мешает ничему из доволновой схемы.
 */
test("v34 adds prototype screen frames to a populated v33 database and cascades with the revision",()=>{
  ...
  const at="2026-08-07T00:00:00.000Z";
  db.run("INSERT INTO prototypes (id,name,device,screen_count,head_rev,design_system,instance_id,created_at,updated_at) VALUES ('gal','Gallery','mobile',1,1,'yandex-pay','inst','now','now')");
  db.run(`INSERT INTO prototype_revisions (prototype_id,rev,doc,builtin_catalog_hash,created_at) VALUES ('gal',1,'{"version":1,"id":"gal","designSystem":"yandex-pay"}','h','now')`);
  ...
  // условиях, и ключ без него затирал бы светлый кадр тёмным.
  expect(columns.filter(column=>column.pk>0).map(column=>column.name)).toEqual(["prototype_id","rev","screen_id","screen_frame_fingerprint"]);
  expect(columns.find(column=>column.name==="receipt_json")?.notnull).toBe(0);
  expect((db.query("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='prototype_screen_frames'").all() as {name:string}[]).map(row=>row.name))
    .toContain("prototype_screen_frames_fingerprint");
  const insert=(fingerprint:string,theme:string)=>db.query("INSERT INTO prototype_screen_frames (prototype_id,rev,screen_id,screen_frame_fingerprint,png_sha256,receipt_json,created_at) VALUES ('gal',1,'welcome',?,?,?,?)")
    .run(fingerprint,`sha-${theme}`,null,at);
  insert("fp-light","light"); insert("fp-dark","dark");
  expect(db.query("SELECT COUNT(*) count FROM prototype_screen_frames").get()).toEqual({count:2});
  // Кадр без своей ревизии не существует.
  expect(()=>db.query("INSERT INTO prototype_screen_frames (prototype_id,rev,screen_id,screen_frame_fingerprint,png_sha256,receipt_json,created_at) VALUES ('gal',9,'welcome','fp','sha',NULL,?)").run(at)).toThrow();
  db.run("DELETE FROM prototype_revisions WHERE prototype_id='gal' AND rev=1");
  expect(db.query("SELECT COUNT(*) count FROM prototype_screen_frames").get()).toEqual({count:0});
  expect(db.query("PRAGMA foreign_key_check").all()).toEqual([]);
  db.close();
});

/**
 * v35 (план 2026-08-07 §W4): саги миграционного коммита. Проверяется на **populated** v34-базе:
 * таблица создаётся поверх живых компонентов и прототипов, `idempotency_key` обязателен, ключ
 * идемпотентности уникален в пределах компонента, а partial unique index держит **позитивный
 * список активных фаз** — `needs-*` намеренно не блокирует новую сагу того же компонента.
 */
test("v35 adds migration commits to a populated v34 database with a positive-list in-flight index",()=>{
  ...
  const at="2026-08-07T00:00:00.000Z";
  db.run("INSERT INTO components (id,name,head_rev,design_system,created_at,updated_at) VALUES ('yp-mig','YpMig',1,'yandex-pay','now','now')");
  db.run("INSERT INTO component_revisions (component_id,rev,source,design_system,created_at) VALUES ('yp-mig',1,'src','yandex-pay','now')");
  db.run("INSERT INTO prototypes (id,name,device,screen_count,head_rev,design_system,instance_id,created_at,updated_at) VALUES ('gal-mig','Gallery','mobile',1,1,'yandex-pay','inst','now','now')");
  db.run(`INSERT INTO prototype_revisions (prototype_id,rev,doc,builtin_catalog_hash,created_at) VALUES ('gal-mig',1,'{"version":1,"id":"gal-mig","designSystem":"yandex-pay"}','h','now')`);
  ...
  expect(columns.find(column=>column.name==="candidate_id")?.notnull).toBe(0);
  const indexes=(db.query("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='migration_commits'").all() as {name:string}[]).map(row=>row.name);
  expect(indexes).toContain("migration_commits_one_in_flight");

  const insert=(id:string,key:string,phase:string)=>db.query(`INSERT INTO migration_commits
    ...
    VALUES (?,'yp-mig',NULL,'yandex-pay','gal-mig',?,'[]','{}',NULL,?,'u',?,?,?)`).run(id,phase,key,at,at,at);
  insert("mig_a","key-a","promote");
  // Второй коммит того же компонента в активной фазе — отсекается partial-индексом.
  expect(()=>insert("mig_b","key-b","gallery-save")).toThrow();
  // `needs-*` вне позитивного списка: сага ждёт человека, но компонент не держит.
  expect(()=>insert("mig_c","key-c","needs-gallery-save")).not.toThrow();
  expect(()=>insert("mig_d","key-d","needs-audit")).not.toThrow();
  expect(()=>insert("mig_e","key-e","complete")).not.toThrow();
  expect(()=>insert("mig_f","key-f","cancelled")).not.toThrow();
  // Ключ идемпотентности уникален в пределах компонента.
  expect(()=>insert("mig_g","key-c","cancelled")).toThrow();
