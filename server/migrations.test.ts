...
test("v30 adds multi-run provenance columns to a populated v29 database without touching existing rows",()=>{
  ...
  // Искусственный откат до v29 снимает и колонку v31 — иначе повторный прогон ловит
  // duplicate column (шаг v31 применяется в том же вызове `migrate` следом за v30).
