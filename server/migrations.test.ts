...
test("migrations are idempotent and install the complete v4 schema",()=>{
  ...
  expect(names).toEqual(expect.arrayContaining(["prototypes","prototype_revisions","prototype_revision_components","prototype_publishes","components","component_revisions","component_publishes","seed_log","design_systems","validation_records"]));
  db.close();
});

test("upgrades a populated v2 database and backfills revision design systems",()=>{
  const db=new Database(":memory:"); migrate(db);
  db.run("PRAGMA user_version = 2"); db.run("DROP TABLE design_systems"); db.run("DROP TABLE validation_records");
...
test("adds validation_records to a populated v3 database without touching existing rows",()=>{
  ...
  db.run("DROP TABLE validation_records"); db.run("PRAGMA user_version = 3");
