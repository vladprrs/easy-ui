...
## Дуо-прототип: YP-приложение и вторая поверхность

Если история идёт через два устройства сразу (терминал/касса ↔ приложение), документ несёт `surfaces` — две панели в плеере с общим стейтом. Канон целиком (структура, обязательный `surface` на каждом экране, `canvas` у desktop-поверхности, `companions`, ссылки `?on.<surface>=<screen>`, съёмка) — скилл `author` §«Дуо-док: флоу через две поверхности». YP-специфика короткая:

- **`yandex-pay` — primary-поверхность** (`surfaces[0]`, `device: "mobile"`), и `doc.designSystem` = `yandex-pay`. Причина не в старшинстве, а в рантайме: `token()` и `Icon` читают глобальный снапшот именно primary-системы, а YP-компоненты плотно завязаны на иконки и токены — на второй поверхности они получали бы чужие значения. Тогда `doc.device: "mobile"` и `doc.startScreen` = стартовый экран YP-поверхности;
- вторая поверхность (терминал, касса) — своя ДС, и её компоненты должны красить себя `color()`/`space()` (чистые `var(--eui-*)`, скоупятся на панель), а не `token()`/`Icon`;
- desktop-поверхность обязана давать всем своим экранам `canvas` — а значит, **без** `@eui/FlowRoot`-регионов: sticky-футер терминала рисуется прямо в макете. YP-поверхность остаётся mobile и регионы сохраняет как обычно;
- смена статуса на второй панели делается общим стейтом (`setState` + биндинг), а не вторым `navigate`: терминальный экшен в событии допустим только один.
...
- **Login rate-limit (429).** Сессия кэшируется на диске между вызовами драйвера (`scripts/easyui-auth.mjs`, кэш в `$XDG_STATE_HOME/easyui`, выключатель `EASYUI_SESSION_CACHE=0`) — в норме логин один на серию. 429 возможен при выключенном кэше или форс-`login()`-скриптах (`interact.mjs`): подождать минуту и повторить.
