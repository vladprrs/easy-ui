...
const inputClass = "mt-1 w-full rounded-md border bg-background px-2 py-1.5 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring";
const overridesSchema = z.record(z.string(), jsonValueSchema);

function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="border-b p-4 last:border-b-0"><h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">{title}</h2>{children}</section>;
}
...
function BlurText({ label, value, multiline, onCommit }: { label: string; value: string; multiline?: boolean; onCommit: (value: string) => void }) {
  ...
  return <label className="block text-sm font-medium">{label}{multiline ? <textarea className={`${inputClass} min-h-20`} value={draft} onChange={(event) => setDraft(event.target.value)} onBlur={() => onCommit(draft)} /> : <input className={inputClass} value={draft} onChange={(event) => setDraft(event.target.value)} onBlur={() => onCommit(draft)} onKeyDown={enter} />}</label>;
}
...
function JsonEditor({ label, value, onCommit, objectOnly = false, validate }: { label: string; value: unknown; onCommit: (value: Record<string, JsonValue>) => void; objectOnly?: boolean; validate?: (value: JsonValue) => string | null }) {
  ...
  return <label className="block text-sm font-medium">{label}<textarea className={`${inputClass} min-h-28 font-mono`} value={text} onChange={(event) => setText(event.target.value)} onBlur={commit} />{error ? <span role="alert" className="mt-1 block text-xs text-destructive">{error}</span> : null}</label>;
}
...
function CanvasEditor({ canvas, onCommit }: { canvas?: { width: number; height: number }; onCommit: (canvas: { width: number; height: number } | undefined) => void }) {
  ...
  return <fieldset><legend className="text-sm font-medium">Холст</legend><div className="mt-1 grid grid-cols-2 gap-2"><label className="text-xs text-muted-foreground">Ширина<input aria-label="Ширина холста" type="number" className={inputClass} value={width} onChange={(event) => setWidth(event.target.value)} onBlur={commit} /></label><label className="text-xs text-muted-foreground">Высота<input aria-label="Высота холста" type="number" className={inputClass} value={height} onChange={(event) => setHeight(event.target.value)} onBlur={commit} /></label></div>{error ? <p role="alert" className="mt-1 text-xs text-destructive">{error}</p> : null}</fieldset>;
}
...
export function InspectorPanel({ state, definitions, dispatch }: { state: EditorState; definitions: Record<string, ComponentDefinition>; dispatch: Dispatch<EditorAction> }) {
  ...
  if (!screen) return <aside className="w-90 shrink-0 border-l bg-background p-4"><p className="text-sm text-muted-foreground">Экран не найден.</p></aside>;
  ...
  return <aside className="w-90 shrink-0 overflow-y-auto border-l bg-background" aria-label="Инспектор">
    <Section title="Элемент">
      ...
      {element ? <div className="mt-4 border-t pt-4"><p className="mb-3 text-sm"><span className="text-muted-foreground">Тип:</span> <strong>{element.type}</strong></p>
      ...
      </div> : <p className="mt-3 text-sm text-muted-foreground">Выберите элемент на холсте или в дереве.</p>}
    </Section>
    ...
    <Section title="Прототип"><div className="space-y-3">
      ...
      <label className="block text-sm font-medium">Стартовый экран<select className={inputClass} value={state.doc.startScreen} onChange={(event) => dispatch({ type: "set-doc-meta", patch: { startScreen: event.target.value } })}>{state.doc.screens.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <label className="block text-sm font-medium">Устройство<select className={inputClass} value={state.doc.device} onChange={(event) => dispatch({ type: "set-doc-meta", patch: { device: event.target.value as EditorState["doc"]["device"] } })}><option value="mobile">Телефон</option><option value="tablet">Планшет</option><option value="desktop">Компьютер</option></select></label>
    </div></Section>
  </aside>;
}
