...
export function ElementTree({ spec, selectedKey, onSelect }: { spec: Spec; selectedKey: string | null; onSelect: (key: string) => void }) {
  ...
  return <div><ul className="space-y-0.5">{tree.map(row)}</ul>{orphans.length ? <details className="mt-2"><summary className="cursor-pointer font-eui-ui text-sm font-medium text-eui-slate-500">{editor.orphans(orphans.length)}</summary><ul className="mt-1 space-y-0.5">{orphans.map(row)}</ul></details> : null}</div>;
}
