...
export function EditorView({ loaded, custom, runtimeKey, onReload }: { loaded: PrototypeDraft; custom?: CustomPlayerRuntime; runtimeKey: string; onReload: () => void }) {
  ...
  return <main className="flex h-full min-h-0 w-full min-w-0 max-w-[100vw] flex-col overflow-hidden bg-white">
    <header className="flex flex-wrap items-center justify-between gap-4 border-b border-eui-ink/10 bg-white px-6 py-3">
      <div className="flex items-center gap-4"><Link className={`${pillGhost} font-eui-ui`} to={`/p/${state.doc.id}/cjm`}>{editor.backToCjm}</Link><h1 className="font-eui-display text-xl font-medium text-eui-ink">{state.doc.name}{state.dirty ? <span className="ml-2 text-eui-magenta" aria-label={editor.dirtyAria}>●</span> : null}</h1></div>
      <div className="flex items-center gap-3 font-eui-ui"><span aria-live="polite" className="rounded-full bg-eui-lilac-100 px-3 py-1 text-xs text-eui-slate-500">{saving ? editor.saving : state.dirty ? editor.notSaved : editor.saved}</span><button type="button" disabled={saving} onClick={save} className={`${pillPrimary} disabled:opacity-50`}>{editor.save}</button></div>
      <div className="basis-full font-eui-ui"><Issues issues={issues} />{conflictRev !== null ? <div role="alert" className="flex flex-wrap items-center gap-3 rounded-2xl bg-eui-lilac-100 p-3 text-eui-magenta"><strong>{editor.conflictTitle(conflictRev)}</strong><button type="button" className={pillGhost} onClick={copy}>{editor.copyLocalJson}</button><button type="button" className={pillGhost} onClick={onReload}>{editor.reloadDraft}</button></div> : null}</div>
    </header>
