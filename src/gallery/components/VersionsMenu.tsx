...
import { pillGhost, popover, popoverItem } from "../../app/chrome";
...
export function VersionsMenu({ prototype, isOwner }: VersionsMenuProps): ReactElement {
  ...
  return <details ref={ref} className="relative" onToggle={onToggle}>
    <summary className={`${pillGhost} cursor-pointer list-none px-3 py-1.5 text-[13px]`}>{gallery.versionsMenu}</summary>
    <div aria-label={gallery.versionsMenuAria(prototype.name)} className={`${popover} absolute right-0 z-20 mt-2 w-64`}>
      {isOwner ? <div className="mb-1 border-b border-pay-lavender pb-1">
        <button type="button" disabled={downloading !== null} className={`${popoverItem} disabled:opacity-50`} onClick={() => void runExport("draft", exportUrl(null), exportName(null))}>{downloading === "draft" ? gallery.exporting : gallery.exportDraft}</button>
      </div> : null}
      ...
      {versions.status === "error" ? <><p role="alert" className="px-2 py-1 text-xs text-pay-red">{gallery.versionsLoadFailed}</p><button type="button" className={`${pillGhost} mt-1`} onClick={load}>{common.retry}</button></> : null}
      ...
      {versions.status === "ready" ? <ul className="space-y-1">{versions.data.map((version) => <li key={version.version} className="flex items-center gap-1">
        <Link className={`${popoverItem} flex-1`} to={`/p/${prototype.id}/v/${version.version}`}>{versionLink(version.version)}</Link>
        {isOwner ? <button type="button" disabled={downloading !== null} className="rounded-item px-2 py-2 text-xs font-medium text-pay-red transition-colors duration-100 hover:bg-pay-lavender disabled:opacity-50" onClick={() => void runExport(`v${version.version}`, exportUrl(version.version), exportName(version.version))}>{downloading === `v${version.version}` ? gallery.exporting : gallery.exportVersionAction(version.version)}</button> : null}
      </li>)}</ul> : null}
      {exportError ? <p role="alert" className="mt-1 px-2 py-1 text-xs text-pay-red">{exportError}</p> : null}
    </div>
  </details>;
}
