...
export function GalleryPage() {
  ...
  return <main className="mx-auto h-full w-full max-w-6xl p-6 font-eui-ui sm:p-8">
    ...
    <p className="mt-2 text-eui-slate-500">Выберите флоу, чтобы открыть его первый экран.</p>
    ...
    {!loading && !failed && visiblePrototypes.length ? <ul className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {visiblePrototypes.map((prototype) => <li className="relative flex flex-col rounded-3xl bg-eui-lav p-6 transition-shadow focus-within:shadow-lg hover:shadow-lg" key={prototype.id}>
        ...
        <p className="mt-2 min-h-10 text-sm text-eui-slate-500">{prototype.description ?? "Без описания"}</p>
        <dl className="mt-5 grid grid-cols-3 gap-3 text-sm">
          <div className="flex flex-col items-start gap-1.5"><dt className="text-eui-slate-500">Устройство</dt><dd className="inline-flex rounded-full bg-white px-2.5 py-1 text-xs font-medium">{deviceNames[prototype.device]}</dd></div>
          <div className="flex flex-col items-start gap-1.5"><dt className="text-eui-slate-500">Экраны</dt><dd className="inline-flex rounded-full bg-white px-2.5 py-1 text-xs font-medium">{prototype.screenCount}</dd></div>
          <div className="flex flex-col items-start gap-1.5"><dt className="text-eui-slate-500">Система</dt><dd className="inline-flex rounded-full bg-eui-lilac-200 px-2.5 py-1 text-xs font-medium">{systemNames.get(prototype.designSystem ?? "shadcn") ?? prototype.designSystem ?? "shadcn"}</dd></div>
        </dl>
        <div className="relative z-10 mt-5 flex flex-wrap gap-2">
          ...
          {prototype.latestVersion !== null ? <>
            <Link className={`${pillGhost} bg-white`} to={`/p/${prototype.id}/v/${prototype.latestVersion}`}>Версия v{prototype.latestVersion}</Link>
            <Link className={pillGhost} to={`/p/${prototype.id}/v/${prototype.latestVersion}/cjm`}>CJM v{prototype.latestVersion}</Link>
          </> : null}
        </div>
      </li>)}
    </ul> : null}
