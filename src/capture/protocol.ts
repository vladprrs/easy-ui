...
/**
 * **Ожидания барьера ресурсов** (BR-03, план 2026-08-08 §3) — то, что о ресурсах кадра знает
 * сервер на постановке и не может узнать страница без гонки.
 *
 * `themeIcons` закрывает вопрос «как понять, что темы нет»: страница грузит тему best-effort
 * (`CaptureComponent.loadComponent`), и «реестр ещё пуст» неотличимо от «реестра не будет» ровно
 * до момента, когда кадр уже снят. Ответ даёт **сервер**: он держит содержимое темы джобы в руках
 * (тем же чтением, которым строит `fonts`), поэтому объявляет число иконок реестра. `0` — фаза
 * `registry` завершается мгновенно (дедлока нет by construction), `>0` — барьер ждёт применения
 * реестра до `resourceBarrier.registryDeadlineMs`, а его исчерпание называется
 * `resource_barrier_timeout` с `ref:"registry:…"`, а не молчанием.
 *
 * `expectedAssets` — id ассетов кандидата и его overlay-детей (`draft.assetIds`/`overlayAssetIds`,
 * они же питают allowlist джобы): **ожидаемый** манифест кадра. Ассет, объявленный исходником, но
 * не наблюдённый в DOM, остаётся report-only записью (случай мог не рендерить эту ветку) — а
 * наблюдённый доказывается наравне с остальными.
 *
 * Поле кладётся в bootstrap **только** при активной политике v4: джоба доволнового барьера обязана
 * присылать байт-в-байт прежний bootstrap.
 */
export interface CaptureResourceExpectations {
  themeIcons: number;
...
export interface CaptureSlotTreeEntry {
  ...
  /**
   * Индексы **собственных** детей узла в этом же плоском массиве (план 2026-08-06 §W6, вложенные
   * слоты). Отсутствует у листа — и у всего дерева глубины 1, поэтому депth-1-джобы остаются
   * байт-в-байт прежними. Корни уровня случая — записи, на которые не ссылается ничей `children`.
   */
  children?: number[];
}
...
/**
 * **Поле краски paint-режима** — две формы (план 2026-08-08 §2, EUI-BR-02).
 *
 * - `{marginPx}` — скалярное поле: доволновая форма, единственная до этой волны. Джоба без
 *   объявленного per-side поля обязана присылать **именно её**, иначе bootstrap каждого уже снятого
 *   кадра перестал бы быть прежним;
 * - `{paddingPx: {top,right,bottom,left}}` — поле по сторонам: компонент с декором, уезжающим в одну
 *   сторону, снимается без четырёхкратного симметричного поля и без обрезанной краски.
 *
 * Union, а не `marginPx` + опциональные стороны: две формы описывают два **разных** решения автора,
 * и «сторона переопределяет скаляр» превратилось бы в третью, никем не объявленную семантику.
 */
export type CapturePaintField =
  | { marginPx: number }
...
export interface CaptureBootstrap {
  ...
  /**
   * BR-03: ожидания барьера ресурсов (реестр иконок темы, ассеты кандидата/overlay). Присутствует
   * ровно у джобы с политикой v4 — bootstrap доволновой джобы обязан остаться прежним.
   */
  ...
  /**
   * Аварийный kill-switch runtime-дефолтов (план 2026-08-07 §1.6, §W9): сервер прочитал
   * `EASYUI_RUNTIME_DEFAULTS_DISABLED=1` на постановке и просит поверхность рендерить так, как
   * рендерил бы хост до волны. Поле **отсутствует** в штатной джобе — bootstrap без флага обязан
   * остаться байт-в-байт прежним, и никогда не едет как `false`.
   *
   * В отпечатки не входит (render-affecting, триаж O-m16): кадр, снятый при поднятом флаге, от
   * доволнового неотличим, и именно поэтому приёмка флагнутых семей при нём недействительна.
   */
  runtimeDefaultsDisabled?: true;
