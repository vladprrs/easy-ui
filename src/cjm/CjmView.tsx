...
export function CjmView({ doc, custom, runtimeKey, routeBase, editable }: { doc: PrototypeDoc; custom?: CustomPlayerRuntime; runtimeKey: string; routeBase: string; editable: boolean }) {
  ...
  return <main className="h-full min-h-0 bg-eui-lav p-6 sm:p-8">
    <header className="mx-auto flex max-w-[1600px] items-start justify-between gap-6">
      ...
      <div className="flex shrink-0 gap-2">{editable ? <Link className={`${pillPrimary} font-eui-ui`} to={`/p/${doc.id}/edit`}>Редактировать</Link> : null}<Link className="inline-flex items-center justify-center rounded-full bg-white px-4 py-2 font-eui-ui text-sm font-medium text-eui-ink transition-colors hover:bg-eui-lilac-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-eui-brand" to={routeBase}>Открыть плеер</Link></div>
    </header>
