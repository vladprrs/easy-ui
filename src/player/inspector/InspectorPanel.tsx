  ...
  switch (entry.kind) {
    case "event":
      return <>
        ...
        {entry.payloadValid ? null : <div className="text-eui-orange">payload invalid</div>}
      </>;
    case "action":
      return <>
        ...
        {entry.result.type === "skipped" ? <div className="text-white/50">skipped ($if is false)</div> : null}
...
export function InspectorPanel({ log }: { log: InspectorLog }) {
  ...
  if (!open) {
    ...
    >Inspector ({entries.length})</button>;
  }
  ...
  >
    <header className="flex items-center gap-2 border-b border-white/15 px-3 py-2">
      ...
      <button type="button" onClick={log.clear} className="rounded border border-white/20 px-2 py-0.5 hover:bg-white/10">Clear</button>
      <button type="button" aria-label="Collapse inspector" onClick={() => setOpen(false)} className="rounded border border-white/20 px-2 py-0.5 hover:bg-white/10">—</button>
    </header>
    <ol aria-label="Inspector entries" className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
      {visible.length === 0 ? <li className="py-2 text-white/50">No entries yet — interact with the prototype.</li> : null}
    ...
    <section aria-label="Font statuses" className="border-t border-white/15 px-3 py-2">
      <h2 className="mb-1 text-[10px] uppercase tracking-wide text-white/40">Fonts (document.fonts)</h2>
      {fonts.length === 0
        ? <p className="text-white/50">No fonts registered.</p>
