...
const scopeFilters: { id: string | null; label: string }[] = [
  { id: null, label: "All" },
  { id: "prototype-screen", label: "Prototype screens" },
...
export function VisualPage() {
  ...
  return <main className="mx-auto h-full w-full max-w-6xl p-6 font-eui-ui sm:p-8">
    <h1 className={headingPage}>Visual regression</h1>
    <p className="mt-2 text-eui-slate-500">Pinned baselines compared against captured candidates with an honest evidence report — exact-rgba and pixelmatch-v1, never a fabricated percentage.</p>

    <div className="mt-6 flex flex-wrap gap-2" aria-label="Scope">
    ...
    {references.status === "loading" ? <p className={`${plate} mt-8 text-eui-slate-500`} aria-live="polite">Loading references…</p> : null}
    {references.status === "error" ? <div className={`${plate} mt-8 text-eui-magenta`} role="alert"><p>References are unavailable.</p><button className={`${pillGhost} mt-3`} type="button" onClick={references.reload}>Retry</button></div> : null}

    <div className="mt-6 grid gap-6 lg:grid-cols-[20rem_1fr]">
      <aside className="space-y-2">
        <UploadReference onCreated={(id) => { references.reload(); setSelectedId(id); }} />
        {references.status === "ready" && !list.length ? <p className="rounded-xl bg-eui-lav p-3 text-sm text-eui-slate-500">No references yet.</p> : null}
        <ul className="space-y-2">
          {list.map((reference) => <li key={reference.id}>
            <button type="button" onClick={() => setSelectedId(reference.id)} aria-pressed={selected === reference.id}
              ...
              {reference.lastRun ? <span className={`mt-2 inline-block rounded-full px-2 py-0.5 text-xs font-medium ${statusTone(reference.lastRun.status)}`}>{statusLabel(reference.lastRun.status)}</span> : <span className="mt-2 inline-block text-xs text-eui-slate-500">No runs yet</span>}
            </button>
          </li>)}
        </ul>
      </aside>
      <section>{selected ? <ReferenceDetail id={selected} onChanged={references.reload} /> : <div className="flex h-full items-center justify-center rounded-3xl bg-eui-lav p-6 text-eui-slate-500">Select a reference.</div>}</section>
    </div>
  </main>;
}
...
function ReferenceDetail({ id, onChanged }: { id: string; onChanged: () => void }) {
  ...
  if (detail.status === "loading") return <p className={`${plate} text-eui-slate-500`}>Loading reference…</p>;
  if (detail.status === "error") return <div className={`${plate} text-eui-magenta`} role="alert">Reference unavailable.</div>;
  ...
  return <article className="space-y-5">
    <header className="rounded-3xl bg-eui-lav p-5">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <label className="text-sm text-eui-slate-500">Threshold %<input className={`${inputBase} ml-2 w-20`} value={threshold} inputMode="decimal" onChange={(e) => setThreshold(e.target.value)} /></label>
        <button type="button" className={pillPrimary} disabled={busy} onClick={runCheck}>{busy ? "Проверяем…" : "Проверить"}</button>
      </div>
      {error ? <p className="mt-3 text-sm text-eui-magenta" role="alert">{error}</p> : null}
    </header>

    {lastRun ? <RunDetail report={lastRun} /> : <p className="rounded-3xl bg-eui-lav p-5 text-sm text-eui-slate-500">No runs yet — run a check to capture a candidate.</p>}

    <section>
      <h3 className={kicker}>Run history</h3>
      <ul className="mt-2 space-y-1">
        ...
        </li>) : <li className="text-sm text-eui-slate-500">No runs recorded.</li>}
      </ul>
    </section>
  </article>;
}
...
function RunDetail({ report }: { report: RunReport }) {
  ...
  return <section className="rounded-3xl bg-eui-lav p-5">
    <div className="flex flex-wrap items-center gap-3">
      ...
      <span className="text-sm text-eui-slate-500">{report.metric ?? "no metric"}</span>
      <span className="text-sm font-medium">{formatPercent(report.diffPercent)}</span>
    </div>
    <div className="mt-4 grid gap-3 sm:grid-cols-3">
      <Frame title="Reference" url={report.reference?.url} sha={report.reference?.sha256} dims={report.reference} />
      <Frame title="Candidate" url={report.candidate?.url} sha={report.candidate?.sha256} dims={report.candidate} />
      <Frame title="Diff" url={report.diff?.url} sha={undefined} dims={undefined} />
    </div>
    <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
      ...
      <Evidence label="Diff pixels / denominator" value={report.diffPixels !== null && report.diffPixels !== undefined && denominator !== null ? `${report.diffPixels} / ${denominator}` : "—"} />
      <Evidence label="Metric options" value={report.metricOptions ? JSON.stringify(report.metricOptions) : "—"} />
      <Evidence label="Candidate meta" value={report.candidateMeta ? JSON.stringify(report.candidateMeta) : "—"} />
    </dl>
  </section>;
}
...
function Frame({ title, url, sha, dims }: { title: string; url?: string | null; sha?: string; dims?: { width: number | null; height: number | null } | null }) {
  return <figure className="rounded-2xl bg-white p-3">
    ...
    {url ? <img className="mt-2 max-h-64 w-full rounded-lg object-contain" src={url} alt={`${title} screenshot`} /> : <div className="mt-2 flex h-24 items-center justify-center rounded-lg bg-eui-lav text-xs text-eui-slate-500">Not available</div>}
...
function UploadReference({ onCreated }: { onCreated: (id: string) => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!file) throw new Error("Choose a PNG file first");
  ...
  if (!open) return <button type="button" className={`${pillGhost} w-full bg-eui-lav`} onClick={() => setOpen(true)}>+ Загрузить reference</button>;
  return <div className="rounded-2xl bg-eui-lav p-4">
    <div className="flex items-center justify-between"><h2 className={kicker}>New reference</h2><button type="button" className="text-sm text-eui-slate-500 underline" onClick={() => setOpen(false)}>Close</button></div>
    <div className="mt-3 space-y-2 text-sm">
      <select className={inputBase} value={scope} onChange={(e) => setScope(e.target.value as typeof scope)}><option value="prototype-screen">Prototype screen</option><option value="component">Component</option></select>
      ...
      <input className={inputBase} placeholder="note (optional)" value={fields.note} onChange={set("note")} />
      ...
      <button type="button" className={`${pillPrimary} w-full`} disabled={busy} onClick={submit}>{busy ? "Загрузка…" : "Сохранить reference"}</button>
    </div>
  </div>;
}
