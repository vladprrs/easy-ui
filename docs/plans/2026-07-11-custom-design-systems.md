...
**Версия плана: v3** (v1 и v2 отревьюированы адверсариально Codex gpt-5.6-sol + Fable 5; триаж обоих раундов — в конце документа). План продолжает реализованную мультисистемность из `2026-07-11-design-systems-atomic.md`: `designSystem` уже присутствует в prototype document, компонентах, DTO, runtime и UI. Эта итерация добавляет персистентный реестр пользовательских систем, `POST /api/design-systems`, регистрацию `yandex-pay`, безопасный перенос существующих компонентов и поддержку authoring driver.
...
| Компонент в двух системах | После move+publish компонент имеет active-версии в обеих системах. Это **осознанная семантика**: старые прототипы старой системы продолжают резолвить его по последней publish-версии своей системы (включая новые ревизии таких прототипов). Чтобы manifest/Library не расходились с этой семантикой, `catalog/manifest` возвращает **по entry на каждую пару `(component, designSystem)`** с latest active версией в этой системе; Library показывает компонент в обеих группах. Задокументировать в `docs/server-api.md` |
...
```
schema migration (+ seed design_systems rows + backfill)  ← обязательные изменения схемы/registry
→ read-only integrity audit (падение = сервер не стартует,
  последующие прикладные мутации не выполняются)
...
Точная гарантия: schema migration и backfill к моменту аудита уже применены (это неизбежно — аудит проверяет новые колонки); провал аудита предотвращает **recovery- и seeding-мутации** (`failStagingPublishes`, `seedPrototypes`), что и проверяется тестом.
...
Исторические `prototype_revisions.doc` на startup **не парсятся**. Это принятый residual risk: FK от JSON-документа к registry нет, и ручная/операционная порча БД может создать историческую ревизию с незарегистрированной системой — штатные write paths этого не допускают (save всегда валидировал систему, удаления систем нет). Компенсация: чтение повреждённой исторической ревизии (`revision`/`version`) обязано возвращать controlled-ошибку с ID прототипа и номером ревизии, а не необработанный parse failure. Тест на провал аудита обязан проверять, что staging publishes и seed не были изменены.
...
| `catalog/manifest` | `server/routes/components.ts:34` (сейчас `c.design_system`, один entry на компонент) | По entry на `(component, design_system)`: для каждой системы, где есть active publish, — latest active версия, система из `cr` (join `cp → cr`, group by `cr.design_system`) |
...
| `catalog/manifest` | по entry на `(component, design_system)`; система из `cr` latest active publish в этой системе |

После смены head system ранее опубликованные версии остаются доступны по immutable URL и старым pins. Компонент не появляется в новой системе до publish новой ревизии. Компонент с active-версиями в двух системах остаётся резолвимым в обеих и виден в manifest/Library в обеих группах — по одному entry на систему, каждый со своей latest active версией. Клиенты manifest (Library, authoring-инструменты) обязаны трактовать `(id, designSystem)` как ключ entry, а не `id`.
...
Read-back определён точно (meta не содержит source, `server/repos/components.ts:15`):

1. `GET /components/:id` → head `designSystem`, `headRev`, `versions[]` (в v3 каждая версия несёт систему своей ревизии);
2. `GET /components/:id/source` → head source; hash считается **локально**: SHA-256 от UTF-8 байтов строки `source` как есть, без канонизации — так же, как серверный `sha256(revision.source)` в publish pipeline;
3. сверка с manifest-ожиданиями.

Состояние каждого компонента определяется по state machine; любое не перечисленное состояние → **стоп, ручной разбор**:

| Состояние (по read-back) | Действие |
|---|---|
| `headRev == expectedHeadRev`, система исходная, source hash совпал | move: `PUT` c `designSystem: target`, `baseRev: expectedHeadRev` |
| `headRev == expectedHeadRev + 1`, система target, source hash совпал, head-ревизия не опубликована | publish head-ревизии (обрыв между move и publish) |
| `headRev == expectedHeadRev + 1`, система target, head-ревизия имеет active publish | complete, шаг пропускается |
| иное (чужие правки, несовпавший hash, неожиданный rev) | стоп, ручной разбор |

Недостаточно «какой-либо active-версии в target-системе» — она может принадлежать другой ревизии с другим source; проверяется именно publish **head-ревизии move**.

Обработка 409:

- `409 revision_conflict` → перечитать state machine заново (не повторять вслепую: retry `PUT` создаёт лишнюю ревизию);
- `409 already_published` при publish → `GET /components/:id/versions/:v` конфликтующей версии и сверить: `rev` равен ревизии move, `designSystem == target`, source hash ревизии совпадает (version DTO возвращает `source` — hash считается локально). Всё совпало → complete; иначе — стоп;
...
```bash
# upsert (create/update source) — существующая позиционная грамматика, source обязателен
...
# system-only move — отдельная команда, source-файла нет
node driver.mjs component-move yp-amount --design-system yandex-pay

# регистрация системы
node driver.mjs design-system yandex-pay "Yandex Pay Design System" "Production-like ..."
```

`component` остаётся upsert'ом с обязательным `<source.tsx>` (обратная совместимость со скиллом). System-only move выражается только командой `component-move <id> --design-system <target>` (флаг обязателен): она делает `getMeta`, `PUT` без `source` и publish новой ревизии.
...
| update с выбором ≠ текущая | `designSystem` в `PUT` вместе с `source` из файла |
| system-only move (`component-move`) | `PUT` c `designSystem` без `source`, затем publish |
...
- конкурирующий move/save/publish: **только DB-инварианты** — ровно один `PUT` выигрывает CAS, ревизия содержит согласованные source/system, publish join указывает на неё; filesystem-гарантии pre-CAS материализации этой итерацией не заявляются (residual risk, см. триаж C7); failed/staging publish после move не ломает манифест;
- компонент с active-версиями в двух системах: manifest содержит **два entry** `(id, system)`, каждый со своей latest active версией; Library показывает его в обеих группах; snapshot из старой системы резолвит её последнюю версию;
...
- driver: flag/env приоритет, create/update семантика по таблице, `component-move` без source-файла, malformed CLI падает до запроса, `design-system` команда обрабатывает `201`/`409`;
- operational retry: повторный прогон state machine из каждого промежуточного состояния (до move / после move до publish / после publish) пропускает выполненные шаги и не плодит ревизии; чтение повреждённой исторической ревизии возвращает controlled-ошибку.
...
## Триаж ревью v2 (раунд 2, тот же тред)

Раунд 2 подтвердил снятие блокеров C1/C2 и корректность понижений C3/C7 (с оговорками). Остаточные находки R# — все приняты в v3:

| # | Находка | Решение |
|---|---|---|
| R1 (major) | Семантика «компонент в двух системах» расходится с manifest (один глобальный entry) и Library | **Принято** (предпочтительный вариант Codex): manifest — по entry на `(component, designSystem)` с latest active версией в системе; Library показывает обе группы; тесты добавлены |
| R2 (major) | `expectedSourceHash` нечем проверить через meta (source там нет) | **Принято**: read-back определён точно — meta + `/source`, локальный SHA-256 от UTF-8 байтов source как есть (идентично серверному `sha256(revision.source)`) |
| R3 (major) | Возобновление между move и publish противоречит правилу стопа по `headRev` | **Принято**: явная state machine по `headRev`/системе/publish-статусу head-ревизии; неописанные состояния → ручной разбор |
| R4 (major) | `already_published` → «версия в целевой системе» — слишком слабо | **Принято**: сверка `rev` ревизии move + системы + source hash через version DTO (он возвращает `source`) |
| R5 (major) | Driver обещает system-only move, но грамматика требует source | **Принято**: отдельная команда `component-move <id> --design-system <target>`; `component` остаётся upsert'ом с обязательным source |
| R6 (minor) | Аргументация отказа от исторического аудита сформулирована слишком сильно | **Принято**: residual risk назван явно; чтение повреждённой исторической ревизии — controlled-ошибка вместо raw parse failure |
| R7 (minor) | «Данные не тронуты» при провале аудита — неточно (migration/backfill уже применены) | **Принято**: формулировка уточнена — предотвращаются recovery/seeding-мутации |
| R8 (minor) | Concurrency-тест обещает filesystem-гарантии, вынесенные из scope | **Принято**: тест ограничен DB-инвариантами, filesystem residual risk зафиксирован |
