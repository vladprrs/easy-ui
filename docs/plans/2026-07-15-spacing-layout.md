# Spacing/Layout Contract v1 + Agent-DX (2026-07-15) — v5 (после раундов 1–4 Codex-ревью)
...
- **m9‴ (`conformanceProps`)** → bounded JSON-safe object, server-only (не в manifest/hash); каждый кандидат прогоняется через `safeParse`, рендерится parsed-результат (D4).
...
- **API v3**: `token()` остаётся value-returning (семантика v2 — фактическое значение темы); CSS-ссылку возвращает только новый typed-хелпер **`space("md")` → `var(--eui-space-md, 12px)`** (canonical fallback конкретного токена запечён в ссылку — fail-soft для порталов и незасетапленных поверхностей). Девять переменных `--eui-space-*` ставятся на stage root каждой render surface компонентом `SurfaceSpacingScope` (вводится в W1, подключение поверхностей — W4b/W4c) из resolved-шкалы — решает первый рендер после async-темы и параллельные превью с разными темами. **Внешние порталы (custom `createPortal` в body) — нормативно вне v1**: там работает запечённый fallback; surface-local portal root — follow-up. Conformance-тесты: ABI3-ребёнок внутри Drawer/Dialog, две одновременные поверхности с разными темами. **ABI v2 не меняется** (включая `""` за отсутствующий токен). Обновлённые YpBox/YpBlock публикуются на ABI v3.
...
Скрипт: сквозной publish-цикл фикстурного компонента с полной декларацией против собранного сервера + snapshot-сверка сериализации W2a↔W2b; **плюс ABI v3 end-to-end**: фикстурный компонент импортирует `space()` из `easy-ui/runtime/v3`, публикуется с `host_abi_version=3`, браузер загружает v3-шим и computed spacing равен resolved-токену (Playwright). Артефакт сохраняется рядом с бейзлайнами.
...
**Файлы**: новый `src/catalog/hostPrimitives/` (definition + реализация + `HostStageSurface`), `src/catalog/runtime.ts` (merge host registry), `src/prototype/runtimeSpec.ts` (`splitHostPrimitives`; порядок сплитов hostPrimitives→canvas), `src/player/actionRuntime.ts` (полный spec для бюджета/стейта), новый `src/prototype/overlayRules.ts` (+строка в validate.ts — включая запрет на desktop-без-canvas), `server/routes/components.ts` + publish-path (резервация имени `Overlay`, 409 включая существующие записи — по W0-аудиту), unit-тесты.
**Done**: split/registry/validate-тесты (root/repeat/вложенность/Hotspot/desktop-без-canvas); repeat внутри Overlay учитывается в бюджете; 409-тесты; **Overlay ещё не рекламируется и не рендерится в поверхностях**.

### W4b — Primary stages
**Файлы**: `src/player/ScreenSurface.tsx`, `src/player/CanvasLayers.tsx` (третий слой), `src/player/DeviceFrame.tsx` + caller-файлы player/present (StageViewport/stageHostRef), `src/capture/CapturePrototype.tsx`, подключение `SurfaceSpacingScope` (W1) к этим поверхностям, DOM/scroll-тесты.
**Done**: player/present/capture рендерят Overlay (mobile flow/canvas, desktop canvas); viewport-sticky при скролле; DOM-тест containing block legacy absolute; `driver check` против W0 — zero drift без Overlay.

### W4c — Preview adapters
**Файлы**: `src/editor/EditorCanvas.tsx` + `src/editor/EditorScreenStrip.tsx`, `src/cjm/CjmScreenTile.tsx`, `src/gallery/GalleryPreview.tsx`, `src/catalog/stories/story-utils.tsx`, подключение `SurfaceSpacingScope`, тесты scale≠1 / zoom / pan.
**Done**: превью-поверхности рендерят Overlay в native transform chain (inset масштабируется вместе с контентом); Gallery-приёмка — Overlay с host/builtin-детьми.

### W4d — Exposure + gate
**Файлы**: `server/designSystems.ts` (host-дескриптор в discovery), `server/builtinHash.ts` + тест (hash bump, `renderContractVersion`), `reference/builtin-catalog.json` (regen), `/api/schemas/*` при затрагивании, stories, `docs/prototype-format.md` (Overlay + StageViewport truth table per-surface: DOM-узел, width/height, scroll owner, overflow, capture-поведение).
...
План v5 → повторное Codex-ревью (`--resume`) → триаж → при отсутствии blocking-возражений — исполнение волнами (`--fresh --write --effort medium`) с независимой верификацией done-критериев. W0, W2-gate, W4d-gate, W7 — оркестратор. Прод (W7) — после приёмки W1–W6.
