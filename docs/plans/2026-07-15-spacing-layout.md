# Spacing/Layout Contract v1 + Agent-DX (2026-07-15) — v4 (после раундов 1–3 Codex-ревью)
...
| shadcn | фиксируется в W1 по фактическим Tailwind-классам npm-пакета (subset none..xl; xs/2xl+ отсутствуют) |

- Подмножество канонического enum — валидно (shadcn Stack `none..xl`, Grid `sm..xl` as-is; npm-схемы не трогаем).
...
- **Доставка в custom-компоненты — ABI v3**: `token("space.md")` (и новый typed-хелпер `space("md")`) возвращает **`var(--eui-space-md)`**; девять переменных `--eui-space-*` ставятся на root конкретной render surface из resolved-шкалы. Это решает первый рендер после async-загрузки темы и несколько одновременных превью с разными темами (значения scoped на surface, не глобальны). **ABI v2 не меняется**: legacy-компоненты продолжают получать прежние значения (включая `""` за отсутствующий токен) — синтез не меняет поведение старых опубликованных компонентов. Обновлённые YpBox/YpBlock публикуются на ABI v3.
- **Формат токенов темы**: `space.*` — неотрицательные абсолютные px-строки (`"12px"`); все 9 ключей при объявлении хотя бы одного; `none="0px"`; монотонность; валидация на merged-теме; **только новые версии тем**. Legacy-темы: fail-soft — malformed/частичная группа `space.*` целиком игнорируется, ABI v3 получает canonical scale; PATCH старой темы, затрагивающий `space.*`, обязан привести группу к правилам v1.
...
- Инварианты (shape-валидация): `layout` без `spacing` и `spacer` — 422; дубликаты в `spacing` — 422; `flow` только вместе с `spacing ⊇ ["gap"]`; `direction.prop`/`wrap.prop` существуют в схеме; массивы `vertical`/`horizontal`/`none`/`enabled` непусты, JSON-safe scalars, попарно не пересекаются, каждое значение принимается схемой пропа; `flow.slot` — `"default"` или объявлен в `slots`.
...
- publish (SSR-conformance): **источник пропсов** — `props.safeParse({})` если проходит; иначе первый валидный `example`; иначе для `layoutNeutral` обязателен `example` или новое поле `conformanceProps` (без них — 422 с объяснением). Уникальный sentinel передаётся **одновременно** как `slots.default` и `children` (в точности adapter-контракт `easyUiRuntime.tsx:143`); отсутствие sentinel в SSR-выводе — **publish error**. Тест-кейс: required props без example.
...
- **Host-примитив**: реестр `hostPrimitives` — отдельная секция catalog summary/discovery (не в bundle-manifest). **Merge — в `src/catalog/runtime.ts`** (клиентский runtime registry), а не в provider definitions `src/designSystems/index.ts` — host-каталог не смешивается с builtin/DS summary. Имя `Overlay` резервируется на create/update/publish (409, включая существующие записи — по W0-аудиту); дескриптор входит в builtin compatibility hash (с W4).
- **Модель координат — два бокса**: `StageViewport` (фиксированная видимая область экрана: device frame viewport на mobile; на desktop — явно заданный viewport-бокс экрана) и `ContentBox/ContentScroller` (скроллящийся контент). **Overlay — viewport-sticky**: якорится к StageViewport и не двигается при скролле контента (соответствует кейсам badge/FAB/sticky-футер). Anchored-to-content поведение — вне v1.
- **Монтирование**: caller (player/present/capture/editor/CJM/gallery/story) предоставляет `stageHostRef`; overlay-portal монтируется в него — **никакой обёртки legacy flow** (не меняются margin collapsing и containing block существующих absolute-элементов; это проверяется отдельным DOM-тестом, не только пикселями). На canvas-экранах — третий явно упорядоченный слой в существующем `CanvasLayers`. Поведение при pan/zoom плеера и внешнем desktop-скролле фиксируется в truth table.
...
- **YpBox** (ABI v3): + `gap?/padding?/paddingX?/paddingY?: spaceToken`; CSS через `space("md")` → `var(--eui-space-md)`; `layout={version:1, spacing:[...], flow:{kind:"flex", direction:{prop:"mode", vertical:["col"], horizontal:["row"], none:["box"]}, wrap:{prop:"wrap", enabled:[true]}}}`; `layoutNeutral: true`.
...
Порядок: **W0 → W1 → (W2a ∥ W2b) → W2-gate → W3 → W4 → W4-gate → W5 → W6 → W7**. W4/W5 сериализованы. Правила: волна, трогающая `validate.ts`, выносит логику в свой модуль; `server/contracts.ts`, `prototype-format.md`, `server-api.md` — не в параллельных задачах одной волны.
...
**Done**: baseline-sets + tuple-манифест; `driver check` зелёный на нетронутом коде; оба аудита зафиксированы.

### W1 — Ядро контракта + ABI v3 + шкала
**Файлы**: `src/designSystems/types.ts`, `src/catalog/normalize.ts`, новый `src/catalog/zodIntrospect.ts` (+переключение `src/editor/propsForm/introspect.ts`), новый `src/designSystems/spacingScale.ts` (`resolveSpacingScale` — чистый; таблицы D2), `src/designSystems/theme.tsx` + surface-шеллы (player/present/capture/editor/CJM/gallery/story: установка `--eui-space-*` на stage root каждой поверхности), `server/shims/abi-v2.ts` (не меняется) + новый `server/shims/abi-v3.ts` (`token`→`var(...)`, `space()`), `server/components/pipeline.ts` (extraction-ABI: v3-шим), `server/designSystemsMeta.ts` (валидация `space.*` + fail-soft), `src/designSystems/shadcn/index.ts` (+`layout.ts`: spacing+flow; снапшот фактических px Tailwind-классов), `src/designSystems/wireframe/definitions.ts` + `components.tsx` (расширение enum по таблице D2, существующие px неизменны), `src/designSystems/fixtures.ts`, `src/catalog/definitions.test.ts`, `docs/prototype-format.md` («Spacing & layout contract v1»).
**Done**: тесты декларации (все инварианты D3, включая flow); «старые wireframe px неизменны»; `resolveSpacingScale` (тема/синтез/fail-soft/legacy); ABI v3 (первый рендер после async-темы, две параллельные темы, capture, SSR-extraction, unmount); ABI v2 без изменений (регресс-тест `""`); test+typecheck; `validate:prototypes`; `driver check` против W0 — zero drift.
...
Скрипт: сквозной publish-цикл фикстурного компонента с полной декларацией против собранного сервера + snapshot-сверка сериализации W2a↔W2b; артефакт сохраняется рядом с бейзлайнами.
...
**Файлы**: новый `src/catalog/hostPrimitives/` (definition + реализация + `HostStageSurface`), `src/catalog/runtime.ts` (merge host registry), `src/prototype/runtimeSpec.ts` (`splitHostPrimitives`; порядок сплитов; action runtime — полный spec: `src/player/actionRuntime.ts` при необходимости), `src/player/ScreenSurface.tsx` + `src/player/CanvasLayers.tsx` (третий слой) + `src/player/DeviceFrame.tsx` (StageViewport/stageHostRef), `src/editor/EditorCanvas.tsx` + `src/editor/EditorScreenStrip.tsx`, `src/cjm/CjmScreenTile.tsx`, `src/gallery/GalleryPreview.tsx`, `src/catalog/stories/story-utils.tsx`, новый `src/prototype/overlayRules.ts` (+строка в validate.ts), **host-дескриптор в discovery + builtin hash bump + regen reference** (`server/designSystems.ts`, `server/builtinHash.ts`+тест, `reference/builtin-catalog.json`), `/api/schemas/*` при затрагивании, stories, тесты, `docs/prototype-format.md` (Overlay + StageViewport truth table).
**Done**: Overlay рендерится в shadcn/wireframe/custom-only DS (Gallery-приёмка: Overlay с host/builtin-детьми; custom-children в Gallery — follow-up), flow/canvas, все поверхности (render-тест каждой); placement-семантика ширины; несколько Overlay; stacking; repeat внутри Overlay (бюджет/стейт); validate-errors; 409 включая существующие записи; DOM-тест containing block legacy absolute; `driver check` против W0 — zero drift без Overlay.

### W4-gate (оркестратор, исполняемый)
Скрипт: discovery → валидация → runtime registry → рендер Overlay-фикстуры на dev-сервере; артефакт сохраняется.
...
3. **Введение ABI v3** — новый контракт шимов; v2 заморожен (регресс-тест), v3 покрыт тестами первого рендера/параллельных тем.
4. **StageViewport/stageHostRef** — портальная модель без обёртки flow; DOM-тест containing block + W0-бейзлайны как ворота W4.
...
- **Сквозное**: `npm run verify`, `npm run e2e`, W2-gate и W4-gate (исполняемые, с артефактами), dev-прогон D9, `driver geometry`.

## Процесс

План v4 → повторное Codex-ревью (`--resume`) → триаж → при отсутствии blocking-возражений — исполнение волнами (`--fresh --write --effort medium`) с независимой верификацией done-критериев. W0, W2-gate, W4-gate, W7 — оркестратор. Прод (W7) — после приёмки W1–W6.
