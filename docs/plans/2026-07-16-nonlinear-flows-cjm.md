# Нелинейные прототипы: `doc.flows` + scenario lanes в CJM + guided browse в плеере (v5)
...
4. **Аллокатор блоков per `(lane, gap)` (по ревью r4 — max длины одного сегмента недостаточно: два сегмента одной lane с одним fork, `A→x→A→y→B`, коллидировали бы)**: сегменты внутри `(lane, gap)` упорядочиваются по первому `stepIndex` и размещаются последовательно (spacer 0); `laneFootprint(lane, gap) = Σ длин сегментов`; `gapWidth(gap) = max(laneFootprint)` по всем lanes. Колонка main-экрана `m` = `m + Σ gapWidth(g < m)` (включая `g = -1`); `columns = L + Σ gapWidth`. Выравнивание внутри промежутка: обычные сегменты прижаты влево к fork-якорю; ведущий сегмент прижат вправо к return-якорю.
...
- `CjmEdgesOverlay`: absolute-SVG, координаты по `[data-cjm-node]` через общий ResizeObserver. **Формальная маршрутизация (по ревью r3)**: рёбра — ортогональные полилинии; горизонтальные участки идут в **межрядных жёлобах** (зазор между рядами lane), вертикальные — в **межколоночных жёлобах**; порты — середины сторон тайлов (fork: низ/право якоря; return: верх/лево якоря). **Каналы (по ревью r4)**: назначение через interval coloring — канал переиспользуется рёбрами с неперекрывающимися горизонтальными интервалами (детерминизм: рёбра сортируются по edge key); **ёмкость жёлоба вычисляется из фактически занятых каналов** — модуль маршрутизации (чистая функция поверх измеренных прямоугольников тайлов) возвращает `channelCount` per жёлоб, рендер задаёт row/column gap как `channelCount × 8px + базовый отступ` (жёлобы растут под нагрузку, линии не выходят в тайлы). **Инвариант: ни один сегмент полилинии не пересекает bounds не-endpoint тайла.** Прямое (нежёлобное) ребро разрешено только после line-of-sight проверки против прямоугольников всех тайлов; иначе — жёлобный маршрут (по ревью r4: «короткий fork/return» между разделёнными рядами небезопасен без проверки).
