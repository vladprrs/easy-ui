# Нелинейные прототипы: `doc.flows` + scenario lanes в CJM + guided browse в плеере (v6)
...
3. **Промежутки main с sentinel'ами (по ревью r4)**: промежутки `g = -1` (перед main-колонкой 0), `g = 0..L-2` (между main-индексами `g` и `g+1`), `g = L-1` (после последнего main-node). Приписка: обычный/хвостовой/backward-сегмент → промежуток после main-индекса fork-якоря; ведущий сегмент → промежуток перед main-индексом return-якоря; flow без якорей → `g = -1`.
4. **Аллокатор блоков per `(lane, gap)` (по ревью r4 — max длины одного сегмента недостаточно: два сегмента одной lane с одним fork, `A→x→A→y→B`, коллидировали бы)**: `laneFootprint(lane, gap) = Σ длин сегментов`; `gapWidth(gap) = max(laneFootprint)` по всем lanes. Колонка main-экрана `m` = `m + Σ gapWidth(g < m)` (включая `g = -1`); `columns = L + Σ gapWidth` (формулы верифицированы r5). **Двухсторонний аллокатор внутри `(lane, gap)` (уточнение r5)**: обычные/хвостовые/backward-сегменты размещаются слева направо от fork-якоря в порядке первого `stepIndex`; ведущий сегмент — прижат вправо к return-якорю; flow без якорей — left-aligned в `g = -1`. Коллизии невозможны: `ordinaryFootprint + leadingLength ≤ gapWidth`.
5. Lane 0 = `flows[0]`: node на шаг в вычисленных колонках, рёбра `main` между соседями.
6. Lane k>0: якорный шаг → node не создаётся (референс на main-node); неякорный шаг → колонка из аллокатора (§4). Соседние пары шагов исчерпываются: якорь→якорь соседние в main (репрезентация — существующее main-ребро; many-to-one допустим), якорь→тайл (`fork`), тайл→тайл (`branch`), тайл→якорь (`return`, в т.ч. backward — длинное ребро в жёлобах). **Инвариант: каждая пара соседних шагов `(flowId, fromStepIndex)` отображается ровно в один `edgeKey`** (тест — проверка этого отображения, не сравнение количеств).
7. `(lane, column)` уникальна по построению аллокатора (последовательное размещение внутри `(lane, gap)`, разные gap не пересекаются по колонкам).
8. `unassigned` — просто список screenId (порядок `doc.screens`); в grid не участвует и колонок не расширяет.
9. `tileCount` = main nodes + неякорные nodes (без unassigned).

Unit-инварианты: уникальность `(lane,column)`/keys; отображение каждой пары `(flowId, fromStepIndex)` → ровно один `edgeKey`; существование endpoints; детерминизм; формулы gapWidth/columns; кейсы: ведущий/хвостовой сегмент, flow без якорей, повтор неякоря (MFA), backward-возврат (KYC), retry-петля к тому же якорю, **два сегмента одной lane с одним fork (`A,x,A,y,B`) и retry в тот же gap (`A,x,A,x,A,B`)**, ведущий + повторный fork в один gap, шаг = экрану другой ветки, note на якоре, main-adjacency якорей (checkout-declined), **max-density кейс** (все 12 flows с сегментами в одном промежутке — для ёмкости каналов, см. 5.2).
...
- `CjmEdgesOverlay`: absolute-SVG, координаты по `[data-cjm-node]` через общий ResizeObserver. **Формальная маршрутизация (по ревью r3)**: рёбра — ортогональные полилинии; горизонтальные участки идут в **межрядных жёлобах** (зазор между рядами lane), вертикальные — в **межколоночных жёлобах**; порты — середины сторон тайлов (fork: низ/право якоря; return: верх/лево якоря). **Каналы — двухфазная маршрутизация (по ревью r4/r5, без feedback loop между измерением и sizing)**:
  1. **Логическая фаза (до DOM-измерений)**: маршруты и interval coloring вычисляются по логическим координатам `(lane, column)` из `CjmLayout` — горизонтальные интервалы для row-жёлобов, вертикальные для column-жёлобов; детерминизм — сортировка рёбер по edge key; канал переиспользуется неперекрывающимися интервалами. Прямое (нежёлобное) ребро назначается здесь же по логическому line-of-sight (смежные `(lane,column)` без промежуточных занятых ячеек).
  2. Assignments фиксируются; **глобальные** `rowGap = base + 8px × maxRowChannels`, `columnGap = base + 8px × maxColumnChannels` (CSS Grid не умеет per-gutter размеры — глобальный gap по максимуму; spacer-tracks — v2-оптимизация при необходимости).
  3. После layout DOM измеряется (ResizeObserver), и зафиксированные логические маршруты **только материализуются в пиксельные полилинии** — измерение не меняет ни набор маршрутов, ни channelCount, ни gap в рамках render-цикла (осцилляция исключена по построению).

  **Инвариант: ни один сегмент полилинии не пересекает bounds не-endpoint тайла.** Max-density unit-кейс покрывает и row-, и column-каналы.
...
- Встраивание: `ScreenView.tsx` рядом с `FlowResetBanner` (~:256). `PrototypeChrome.tsx` (:47): Player↔CJM переносит `flow` и `step`; **CJM передаёт в Chrome явный Player-URL `buildPlayerPath(routeBase, flows[?].steps[step].screenId)` при валидной паре flow/step** (по ревью r4: голый `routeBase` редиректится на `startScreen`, и ScenarioBar удалил бы корректный `step`); e2e на Chrome round-trip Player → CJM → Player с сохранением экрана шага.
...
## Триаж ревью r4 (2026-07-17)

| # | Находка | Решение |
|---|---|---|
| 1 (blocker) | max-длина сегмента не даёт уникальность `(lane,column)` (`A,x,A,y,B`) | **Принято**: аллокатор блоков per `(lane,gap)` — последовательное размещение сегментов, `laneFootprint = Σ длин`, `gapWidth = max(laneFootprint)`; sentinel-промежутки `-1`/`L-1`; выравнивание leading вправо/обычных влево; кейсы в unit |
| 2 (major) | фиксированные 8px-каналы не помещаются в жёлоб; «короткие прямые» небезопасны | **Принято**: interval coloring, жёлобы растут из `channelCount`, прямое ребро только после line-of-sight; max-density кейс |
| 3 (major) | Chrome round-trip теряет экран шага (routeBase → startScreen → step удаляется) | **Принято**: CJM передаёт явный Player-URL через `buildPlayerPath`; e2e round-trip |
| — | инвариант репрезентации | Уточнён как отображение `(flowId, fromStepIndex) → edgeKey` (many-to-one для main-рёбер допустим) |

## Триаж ревью r5 (2026-07-17)

| # | Находка | Решение |
|---|---|---|
| 1 (major) | measure → route → resize gap — циклический layout; CSS Grid не умеет per-gutter размеры | **Принято**: двухфазная маршрутизация — логическая фаза по `(lane,column)` до измерений (routes+каналы+line-of-sight фиксируются), глобальные rowGap/columnGap из max channels, измерение только материализует полилинии в пиксели |
| 2 (уточнение) | «последовательно по stepIndex» vs «leading вправо» | **Принято**: двухсторонний аллокатор — обычные слева от fork по stepIndex, leading справа к return, безъякорный flow left-aligned в `g=-1` |
| — | формулы gapWidth/mainCol/columns | Верифицированы ревью, без изменений |
