...
- **Изоляция и выбор**: рендер-обёртка с `inert` + спек без `on` + прозрачный оверлей `absolute inset-0`. **Поправка 3.1 (обнаружено при e2e)**: Chromium исключает `inert`-поддерево из `document.elementsFromPoint` → геометрический hit-test делаем вручную: клик по оверлею → перебор всех `span[data-jr-key]` внутри `previewRootRef` → rect каждого через `Range.getClientRects()` → среди содержащих точку клика выбирается элемент с **наибольшей DOM-глубиной** (внутренний/верхний; при равенстве — меньшая площадь union-ректа), что сохраняет приоритет hotspot и вложенных элементов; ≤500 элементов/экран — перебор дёшев. Мимо элементов → снять выбор.
