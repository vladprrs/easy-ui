...
- Применение шрифтов — только через утилиты `font-eui-ui` / `font-eui-display`, и **только на узлах, не имеющих `<Renderer>`-потомков**: header, сайдбары, тулбары, инспектор, подписи тайлов, текстовые блоки. `main`-контейнеры плеера/CJM/редактора//debug содержат Renderer — на них (и любых их предках) шрифтовые утилиты ставить запрещено; шрифт навешивается на соседние chrome-ветки. Контейнеры рендера (`DeviceFrame` внутренность, `CjmFrame`, `EditorFrame`, SmokeSpec-рендер) — тем более.
...
После завершения T0.5 файлы `PrototypeLoader.tsx`, `CanvasLayers.tsx`, `CjmScreenTile.tsx` **замораживаются**: задачи T1–T5 и I к ним не прикасаются; найденные в них проблемы эскалируются оркестратору.
...
2. **Invariant-spec**: новый Playwright-тест открывает плеер, CJM, редактор и `/debug` с фикстурным прототипом и ассертит внутри отрендеренного shadcn-элемента: computed `font-family` начинается с системного стека (не Coil/YS), и `getComputedStyle(document.documentElement).getPropertyValue('--primary'|'--background'|'--radius')` равны базовым значениям (захардкоженным из текущего index.css). Для библиотеки — те же проверки внутри Storybook iframe. Дополнительно: (а) один портальный компонент (открытый Dialog/Popover в фикстуре плеера) проверяется теми же ассертами — порталы рендерятся в body вне prototype-root; (б) обратная проверка — computed `font-family` заголовка хрома галереи начинается с Coil, навигации — с YS Text (шрифты реально применены к хрому).
