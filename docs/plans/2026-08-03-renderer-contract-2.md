...
**Ресурсы прода.** Текущие лимиты (`mem_limit: 1g`, посылка «1 CPU») — консервативная закладка; фактические ресурсы хоста снимаются в R0 и лимиты поднимаются (N9). Все пороги ниже пересчитываются от фактического лимита после R0; до этого — консервативные оценки.
...
**Флаг.** `EASYUI_VISUAL_SIGNALS_V2=1` (opt-in).
...
**Done.** e2e: bundle одного рана содержит все артефакты, sha сходятся; **замер K5 «после»** (дельта к R0-baseline, ≥3 семьи, флаги ON в тест-стенде); `verify`.
