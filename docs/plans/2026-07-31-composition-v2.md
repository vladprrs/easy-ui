...
## 4a. Деплой и стадия A (2026-07-31)

- В прод выкачено двумя релизами: `037e0a4` (код проекта) и `d6568f7` (дефекты аудита,
  найденные стадией A). Оба — зелёный `build-image`, Dokploy `done`, runtime verify 5/5 PASS,
  `/api/capabilities` на проде отдаёт `compositionV2`, `catalogMigration` и `compositionDepth 5`.
- Runtime-приёмка стадии B на живом инстансе: `prepare` → `apply` → **рестарт процесса** →
  `rollback` из удержанного на диске образа возвращает состояние до cutover, снимает
  maintenance-lock и пишет `catalog.migration.rolled_back`.
- **Стадия A на проде выполнена**, результат и вердикт — `docs/audit/2026-07-31-catalog-dedup-stage-a.md`:
  445 артефактов, 295 кандидатов в композиции, ноль подтверждённых дубликатов; единственная
  группа матчера (`yp-app-home-loans` → `yp-app-home-savers`, score 0.7837) отклонена как
  ложноположительная и задокументирована.
- **Стадии B и C не запускаются**: после отклонения группы план пуст, отставлять нечего.
