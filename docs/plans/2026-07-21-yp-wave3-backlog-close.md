...
## Итог исполнения (2026-07-21)

Волна выполнена целиком за один день, все под-волны на проде, 0 незапланированных регрессий.

- **W-A/G1**: реестр v6 (186 записей, 0 без диспозиции), carriers (609 вхождений/91 компонент), инвентари H4/H7, dom-gate-v2 (self-test 22/22). Канон-поправки по фактам носителей: text.positive=#2c9e56 (не #56c776); 5 fill/hover-литералов не в hairline; **G1-A**: #777a85/#d6d7da — control-роли, literal-preserve (визуальная приёмка трека yp-switch).
- **W-B/G2**: валидация shadow/radial (965ec11), тема v6 → прод (17 v5-ключей побайтово, 58 append), прод-diff 0. Позже v7 (append fill-muted/control-*).
- **W-E**: yp-text v3 / yp-skeleton v4 (ABI v4, плоский union spacing → select в инспекторе); h4-migrate: 1 exact-правка на проде (8→"sm"); H4-инвентарь: 85 вхождений, все yp-text, off-token 20 сохранён.
- **W-F**: инвентарь показал прод 100% на $asset (0 предзаливок); 12 компонентов schema-hardening (regex asset-ref), негативный гейт 10/10×422. Инцидент stale-снапшота (pmc v15) исправлен v16, v15 superseded; правило «база = прод-source» закреплено.
- **W-G**: promo-banner v3 = канон (imageLayout/ctaSize/tone), banner-mid deprecated целиком и ушёл из manifest; миграция 4 узлов px-идентична (md5); helpers инлайн (импорт между бандлами невозможен); прод-diff 0/10.
- **W-C**: 6 батчей, 79 компонентов abi=4 на прод; прод-diff — только суб-перцептивные канон-дельты (приняты глазами); yp-confetti не мигрирован (canvas fillStyle ≠ var()) — осознанное исключение.
- **W-D**: D1 ассет подтверждён; D4 глифы «Я»/«✦» → Плюс-ассет (3 компонента, высота инвариантна, +6-8px ширины у знака, паритет с yp-plus-badge).
- **W-H**: доки design.md (канон-таблица v7, роли W-G, D4) + SKILL.md обновлены; триаж-протоколы в work/yp-wave3/triage-we-wf.md; бэкапы .backups/prod-wave3-20260721/.

H10 (runtime-адопция) закрыт зонтиком: почти весь каталог на ABI v4 color()/space().
