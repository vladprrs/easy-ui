...
Aggregate-записи (41 шт., `color.agg-*`) и var()-записи (36 шт.) — не мигрируются как строки: A4 разворачивает их в пер-вхождения конкретных литералов в носителях; правило переписывания `var(--x, <literal>)` → `color("<key>", "<canon|literal>")` (var заменяется целиком).

### A2 — Реестр v6
`work/yp-wave3/token-registry-v6.json` (перегенерация `gen-registry.mjs`): поля literal / key / disposition (canon-now|literal-preserve|skip) / canon-value / prev-value / family / frequency. Тени добавить вручную из design.md §1.1 (в colors-агрегате их нет). Done-критерий: **каждая из 173 записей имеет диспозицию**, счётчики сходятся.
...
## W-D — H8: тени, градиенты, знак Плюса (1 владелец; пересечения файлов с W-C — тот же владелец)
...
**Первичный гейт — программный, не глаза** (152 записи вручную — риск штамповки): расширить `h1-dom-gate.mjs` со шиммеров на **все** мигрируемые носители — для каждого example снять computed-стили до/после и assert: изменились **ровно целевые свойства** и ровно `prev-value → canon-value` из реестра v6 (для literal-preserve — байтово равны), layout-метрики не изменились. Поверх — per-example capture+diff (`dimsMatch:true`, дельта локализована).
