...
Дата: 2026-07-16 · Статус: **approved** (v5, 5 раундов Codex-ревью gpt-5.6-sol max; раунд 5: «остаточных blocker/major нет, plan approved»; триаж в §6)
...
3. **Loader = только shared transport/retry, без generation.** Rejected import: запись удаляется из `moduleCache` **только если всё ещё содержит тот же rejected promise**, а повторная попытка использует новый URL `bundle.js?retry=<n>` (канонический первый URL — без суффикса): браузер кеширует неуспешный ESM-load по URL, повторный `import()` того же URL не делает сетевого запроса (воспроизведено в ревью). **Граница retry-механизма:** суффикс bust'ит только корневой модуль — статически импортируемые `/api/shims/vN/*.js` при своём отказе остаются отравленными в module map независимо от суффикса (воспроизведено в ревью). Классифицировать причину по ошибке `import()` **нельзя**: Chromium возвращает `TypeError` с URL верхнего бандла и при падении транзитивного shim (воспроизведено в ревью). Политика — детерминированная эскалация без классификации: первый отказ → один SPA-retry с новым root-URL (`?retry=<n>`); повторный отказ → предложение **full-document reload** (новый module map), дальнейшие SPA-retry не выполняются. Generation/отмена — ответственность потребителя (W3): у loader'а общий кеш, abort одного потребителя не должен ронять другого ждущего тот же URL. Тесты: bundle-500 → retry → новый сетевой запрос и успех; shim-500 (bundle отдаётся, shim падает) → SPA-retry не помогает → предложение reload → e2e проверяет успешную загрузку в новом document, не URL в ошибке; два concurrent-потребителя одного URL, отмена одного не мешает второму.
...
- недоступный бандл: docs/код работают, retry после восстановления делает новый сетевой запрос и успешен; после повторного отказа — предложение перезагрузки страницы;
...
E2E: прямое открытие `/library/c/<id>?v=`; live-props; сохранение локального state при смене props; ошибка поля не ломает превью; заполнение required×2 из «заполните обязательные props» до живого превью; Docs/Код (экранирование HTML); `?v`-переключение и грамматика; rejected без запроса bundle.js; retry бандла = два сетевых запроса; shim-500 → эскалация до reload → успешная загрузка в новом document; слоты (все три фикстуры); keyboard-навигация вкладок. Переход Gallery → страница здесь не проверяется (маршрут пользователя появляется только с W4) — он в W5b.
...
### Раунд 4 (resume того же треда) — 0 blocker / 1 major

Стабильный theme-owner, защитный контур интроспекции, правила слотов и фикстуры, перенос Gallery-сценария в W5b, tracked compiled ABI-бандлы — подтверждены закрытыми.

| # | Находка | Вердикт | Реакция |
|---|---|---|---|
| 1 | Shim-отказ неотличим по URL ошибки `import()` — Chromium возвращает `TypeError` с URL верхнего бандла и при падении shim (воспроизведено) | **Принято** | W0.3: детерминированная эскалация без классификации — первый отказ → один SPA-retry с новым root-URL, повторный → предложение full-document reload; e2e shim-500 проверяет загрузку в новом document |
