...
5. **Бюджет смонтированных превью — 12, с вытеснением по расстоянию** (M‑1).
   ...
   нарушается, а гейт §8 достижим. Модуль остаётся в кэше `loadCustomComponents`,
   поэтому возврат в зону — без сети;
...
- приоритеты: явный выбор/атом → 0, recommended+high → 1, molecules → 2,
  near-viewport prefetch → 3.
