# План v5 (approved): screen regions — header / footer / OS status bar в прототипах
...
Решение — **host-owned нейтральный контейнер с типом `@eui/FlowRoot`** (`src/catalog/hostPrimitives/`, рядом с Image/Hotspot/Overlay, доступен каждой системе):
- **Имя collision-proof по построению** (ревью-3 blocker): `element.type` в схеме — произвольная строка, а грамматика имён custom-публикаций `/^[A-Z][A-Za-z0-9]*$/` не может породить `@eui/FlowRoot` → коллизия с legacy custom-компонентом невозможна, аудит БД/version-aware resolver не нужны, reservation-check не задействуется. Экспортируемая константа `FLOW_ROOT_TYPE` — единственный источник строки для eligibility/валидации/сравнения. W1 верифицирует имя end-to-end (defineCatalog → normalize → Renderer lookup → snapshot/classify — везде тип лишь ключ записи).
- Definition — **по образцу Overlay**: `slots: ["default"]` (в `@json-render/core` компонент принимает детей только при непустом `definition.slots`; это не namedSlots-capability), без props и событий. **НЕ добавлять в `extractionPrimitiveDefinitions`** (там остаётся только Overlay, иначе `splitHostPrimitives` извлечёт весь экран) — регистрируется как обычный структурный host-тип; тест: FlowRoot-root остаётся `content`.
- Контракт нейтральности (нормативная формулировка): обычный block-контейнер (`<div>`), который **не задаёт собственных inherited/layout-стилей** — без padding/gap/position/color/font, не создаёт stacking context. Тесты: отсутствие собственных стилей; идентичное наследование темы (color/font) у контента inline и в порталах.
- `region` допустим **только когда root экрана — `@eui/FlowRoot`** (+ на самом region-элементе нет `repeat`/`slot`); eligibility проверяется по одному документу, definitions не нужны;
...
- отсутствие custom root закрывает и blocker №2 раунда 2 (нет `slotIndices` у root).
...
- **`applyRegionPolicy` fail-closed через общий preflight** (ревью-3 №5, уточнено ревью-4 №1): часть данных (например `slot` на элементе) теряется уже в `toRuntimeSpec`, а после `splitHostPrimitives` Overlay-поддеревья вынесены из content-ветки — поэтому eligibility нельзя честно посчитать по post-split дереву. Вводится **pure preflight-анализатор `analyzeScreenRegions(screen)` по authored spec** (до `toRuntimeSpec`/split), покрывающий все правила 1–7 (включая `slot` на region-элементе, region внутри Overlay, nested region/FlowRoot, orphan/multi-parent); его используют и validator (`regionRules`), и runtime fail-closed — одна логика. Семантика fail-closed: отказ **только от region-transform** (весь экран inline, включая маркеры); обычное Overlay/Hotspot extraction работает как раньше. `regions` — по одному поддереву на kind, не массив. Тесты на каждый invalid-кейс.
- **Единый терминальный `ScreenRenderPlan`** (ревью-3 №3, уточнено ревью-4 №2): remap-helper внутри отдельных split-функций недостаточен — consumers берут `.spec` и передают в `EasyUiRuntimeProvider` исходную metadata, а последовательные split-стадии (`splitHostPrimitives → splitCanvas | applyRegionPolicy`) порождают вопрос композиции metadata. Вводится единственный composer **`buildScreenRenderPlan`**: формирует все конечные render-ветки (content, overlays, hotspots, regions) и только затем объединяет их remapped metadata (merge с проверкой коллизий element id — коллизия невозможна по построению, assert); `slotIndices` каждого элемента берутся из финального дерева его ветки. Обновить всех consumers (`ScreenSurface`, `EditorCanvas`, `EditorScreenStrip`, `CjmScreenTile`, `GalleryPreview`; оговорка «CJM не трогаем» на эту точку не распространяется); `runtime.setScreenSpec` остаётся на исходном unsplit-spec (зафиксировать). Дубликат `src/player/canvasSpec.ts` делегировать общей реализации или удалить. Тесты через реальный provider — два комбинированных: named-slot custom parent + Overlay + Hotspot; FlowRoot region + Overlay.
...
- **Safe-area**: `viewport-fit=cover` в v1 НЕ добавляем (консистентно с решением mobile-player плана; глобальная правка index.html затронула бы все маршруты). `env(safe-area-inset-bottom, 0px)` остаётся progressive enhancement (0 в браузере, работает в standalone/PWA). Padding живёт в портальном wrapper'е; схлопывание пустого wrapper'а — **явный CSS-контракт** `wrapper:empty { display: none }` (ревью-3 №7: `visible:false` даёт `null` от Renderer, но portal-таргет остаётся в DOM и padding сам не схлопнется). Тестовый seam (ревью-4 №5: `env()` не переопределить переменной напрямую): `padding-bottom: var(--eui-safe-area-bottom, env(safe-area-inset-bottom, 0px))` — тест задаёт `--eui-safe-area-bottom` и проверяет динамический `visible` при ненулевом inset.
...
- **Исполняемые hardcoded-списки host primitives** (ревью-4 №3): `scripts/validate-templates.ts:27` (знает только Image/Hotspot/Overlay — перейти на `hostPrimitiveNames`, иначе потребует custom definition для `@eui/FlowRoot`); `e2e/starter-ds.fixture.ts:78` (starterization превратит namespaced FlowRoot в `StarterStack` — сохранить `@eui/FlowRoot` буквально + тест).
- **Доки с перечислением тройки примитивов** (ревью-4 №4): `README.md`, `CLAUDE.md`, `.claude/skills/verify/SKILL.md` (используется в финальной приёмке); `docs/server-api.md:~261` — «hostPrimitives всегда пуст» и устаревший пример render-контракта.

**Rollback-политика** (ревью №11, дополнено ревью-3 №4): «additive optional» не даёт безопасного отката образа — строгий `storedPrototypeDocSchema` старого сервера не распарсит уже записанный документ с `region` при чтении; документы с `@eui/FlowRoot` тоже несовместимы со старым образом (нет host-рендерера и component pin для этого типа). В `docs/server-api.md` фиксируем оба класса; перед деплоем — обязательный логический бэкап (стандарт прод-процедуры), rollback-window упоминается в приёмке деплоя (та же политика, что для flows).

**Release-механика** (ревью-3 №4 — небезопасные межволновые состояния): вся фича разрабатывается на feature-ветке; деплой в прод = push в `main` → **один атомарный релиз после T7**. В `main` между волнами не мержим — промежуточные состояния (`region` принимается, но не рендерится; capability рекламирует незавершённый рендер; editor создаёт regions до T3) существуют только в ветке и в прод не попадают. Activation-флаги не нужны.
...
## Триаж ревью-раунда 4 (2026-07-17) — план одобрен

Вердикт ревью: **blocking-возражений нет**, `@eui/FlowRoot` реализуем по всему стеку (catalog/registry — точные ключи, имя нигде не парсится; snapshot/classify/hash — точное членство; Library и ABI не затронуты; release-схема закрывает межволновые состояния). Принято всё: major 1 → общий pure **preflight-анализатор по authored spec** до `toRuntimeSpec`/split (одна логика для validator и runtime fail-closed; все правила 1–7; fail-closed = отказ только от region-transform, Overlay extraction сохраняется); major 2 → единый композер **`buildScreenRenderPlan`** (терминальные ветки → merge remapped metadata с assert на коллизии; `setScreenSpec` на unsplit-spec; два комбинированных provider-теста); major 3 → `scripts/validate-templates.ts` (на `hostPrimitiveNames`) и `e2e/starter-ds.fixture.ts` (starterizer сохраняет `@eui/FlowRoot` + тест) включены в propagation; minor 4 → README/CLAUDE.md/verify-скилл/server-api.md (host discovery, render contract) в чеклисте; minor 5 → safe-area seam `var(--eui-safe-area-bottom, env(...))`; minor 6 → reservation-тест ожидает 422 (regex раньше 409) + 4 проверки классификации.

## Триаж ревью-раунда 3 (2026-07-17)

Принято всё (blocker + 5 major + 2 minor): blocker (коллизия имени `FlowRoot` с возможным legacy custom-компонентом: snapshot/classify/runtime-merge подменили бы смысл immutable-ревизий) → **тип `@eui/FlowRoot`**, недостижимый для custom-грамматики `/^[A-Z][A-Za-z0-9]*$/` — коллизия исключена по построению, вариант «аудит БД + version-aware resolver» отклонён как более сложный и не дающий гарантии для будущих инстансов; major 2 → definition с `slots:["default"]` по образцу Overlay, НЕ в `extractionPrimitiveDefinitions`, нормативная формулировка «не задаёт собственных inherited/layout-стилей» вместо «нулевого» контракта; major 3 → split возвращает render-plan с remapped metadata, обновляются все consumers вплоть до `EasyUiRuntimeProvider` (включая CJM/Gallery — точечно), render-тест через реальный provider, `canvasSpec.ts` делегируется/удаляется; major 4 → release-механика «один атомарный релиз после T7, в main между волнами не мержим» (activation-гейты отклонены как избыточные при feature-branch разработке), rollback-политика дополнена FlowRoot-документами; major 5 → fail-closed `applyRegionPolicy` (identity-return при невалидной структуре, включая duplicate kind); major 6 → Gallery остаётся host-only: заявление «единая фикстура на всех поверхностях» скорректировано — для Gallery отдельная host-only фикстура, загрузка pinned custom в `GalleryPreview` отклонена как расширение scope v1; minor 7 → CSS-контракт `wrapper:empty{display:none}` + тест с ненулевым inset; minor 8 → `docs/server-api.md` capabilities-пример в T6, host-count тесты 3→4 в W1, e2e-матрица «6 проектов» + preview-smoke в T7.
...
Файлы: `src/catalog/hostPrimitives/` (**новый примитив с типом `@eui/FlowRoot`**, константа `FLOW_ROOT_TYPE`: definition по образцу Overlay — `slots:["default"]`, без props/событий, block-контейнер без собственных inherited/layout-стилей; регистрация в `index.ts`/`definitions.ts` → попадает в builtin catalog hash; **НЕ добавлять в `extraction/definitions.ts`**), `src/prototype/schema.ts` (`REGION_KINDS`, `region: z.enum(...).optional()`, тип `RegionKind`), `src/prototype/runtimeSpec.ts` (`ElementMetadata.region`; strip зеркально `slot`; `applyRegionPolicy(tree, policy)` → `{content, regions}` по direct children root, режимы `inline | drop | extract`, **fail-closed identity-return** при невалидной структуре; общий **remap-helper `slotIndices`** по паре original/filtered children — фикс pre-existing бага в `splitHostPrimitives`/`splitCanvas`, пустые slot-группы сохраняются; split возвращает **render-plan с объединённой remapped metadata**), consumers metadata (`src/player/ScreenSurface.tsx`, `src/editor/EditorCanvas.tsx`, `src/editor/EditorScreenStrip.tsx`, `src/cjm/CjmScreenTile.tsx`, `src/gallery/GalleryPreview.tsx` — передают remapped metadata в `EasyUiRuntimeProvider`; `src/player/canvasSpec.ts` делегировать/удалить), `server/openapi.json` (реген — drift-check в verify), юниты, фикстура с regions на custom-only системе.
...
**Done (волна целиком)**: имя `@eui/FlowRoot` проверено end-to-end (defineCatalog → Renderer → snapshot/classify; POST custom с именем `@eui/FlowRoot` → **422** (regex отсекает раньше reservation-409); обычный custom `FlowRoot` публикуется и не конфликтует; `@eui/FlowRoot` в прототипе классифицируется как host — без custom pin и записи в manifest); FlowRoot-root остаётся `content` в `splitHostPrimitives`; `scripts/validate-templates.ts` на `hostPrimitiveNames`; strip/metadata/split покрыты (fast-path идентичность объекта; custom-root + Overlay-sibling + slotted children — slotIndices корректны после split; symmetric-тест для `splitCanvas`); два комбинированных render-теста через реальный `EasyUiRuntimeProvider` (named-slot parent + Overlay + Hotspot; FlowRoot region + Overlay); preflight/fail-closed кейсы (все правила 1–7, общая логика validator+runtime); тесты нейтральности FlowRoot (нет собственных стилей, наследование темы идентично); host-count ожидания 3→4 (`server/host-primitives.test.ts`, `Overlay.test.tsx`); `npm run validate:templates`, серверные validation/hash-тесты зелёные (обновить ожидания контрактной версии); `npm run verify` зелёный.
...
Файлы: `server/contracts.ts` (elementChangedSchema + capabilitiesResponseSchema), `src/prototype/revisionDiff.ts` (+тест), `server/routes/meta.ts`, `src/api/client.ts`, аудит `server/driver-mjs.d.ts`, `server/openapi.json` (повторный реген — contracts меняются снова), серверные тесты (diff add/remove/change; capabilities; discovery-схема содержит enum region; POST с неверным region → 422), `docs/server-api.md` (+rollback-политика с обоими классами — `region`-документы и `@eui/FlowRoot`-документы; обновить захардкоженный capabilities-пример).
**Done**: перечисленные тесты зелёные, `npm run verify` зелёный.

### T7 — cross-surface интеграция, e2e, доки, приёмка
Region-фикстура прогоняется по поверхностям: framed player, desktop present, editor canvas/strip, CJM, capture (в capture предустановить `eui.statusBarHidden=true` в localStorage и доказать inline-рендер) — основная custom-only фикстура; **Gallery — отдельная host-only фикстура** (GalleryPreview не грузит pinned custom — ревью-3 №6; проверяем только inline-рендер FlowRoot-структуры без custom-компонентов). Новый `e2e/dev/screen-regions.spec.ts` + **preview-smoke** (production build: FlowRoot/portal CSS живы в собранном бандле); e2e-сид: мобильный прототип со statusBar-имитацией + header + footer-таббар с navigate + второй экран без regions. Матрица (mobile-Chromium, `?mobile=1`): футер виден при длинном контенте; после скролла boundingBox баров стабилен, statusBar отсутствует; тап по таббару → navigate → новый экран сверху; экран без regions — контент на всю высоту; `?mobile=0` — inline, statusBar виден, тумблер скрывает; Overlay bottom + footer — Overlay поверх. Доки: `docs/prototype-format.md` (раздел Screen regions, allowlist, root-контракт `@eui/FlowRoot`, Overlay truth table: StageViewport = flex-колонка, Overlay-слой z-20), `.claude/skills/author/SKILL.md`.
**Done**: `npm run verify` + `npm run e2e` (все 6 project entries) зелёные; runtime-приёмка по скиллу `/verify` (скриншоты `?mobile=1` и тумблера); ручной пункт — реальный телефон (адресная строка/home-indicator/клавиатура). После T7 — единственный merge в `main` (один атомарный релиз, см. release-механику).
...
- Rollback образа после записи region- или FlowRoot-документов — несовместим (см. rollback-политику в T6-доках); стандартный логический бэкап перед деплоем обязателен.
- Промежуточные межволновые состояния небезопасны для прода — закрыто release-механикой (в `main` не мержим до T7).
