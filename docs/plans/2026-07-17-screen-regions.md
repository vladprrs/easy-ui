# План v4: screen regions — header / footer / OS status bar в прототипах
...
- **`applyRegionPolicy` fail-closed** (ревью-3 №5): при любой семантически невалидной структуре — root не `@eui/FlowRoot`, `repeat`/`visible`/`on` на root, два direct children с одним region-kind, битые child references — функция возвращает исходный `RuntimeTree` **по identity** (полный inline-fallback, ничего не extract/drop); `regions` — по одному поддереву на kind, не массив. Тесты на каждый invalid-кейс.
- **Remapped metadata доходит до провайдера** (ревью-3 №3): remap-helper внутри split недостаточен — consumers берут `.spec` и передают в `EasyUiRuntimeProvider` исходную metadata. Split-функции возвращают render-plan с объединённой remapped metadata; обновить всех consumers (`ScreenSurface`, `EditorCanvas`, `EditorScreenStrip`, `CjmScreenTile`, `GalleryPreview` — точечная правка передаваемой metadata; оговорка «CJM не трогаем» на эту точку не распространяется). Дубликат `src/player/canvasSpec.ts` делегировать общей реализации или удалить. Тест — render custom named-slot компонента через реальный provider после split, не только unit metadata.
...
**Done (волна целиком)**: имя `@eui/FlowRoot` проверено end-to-end (defineCatalog → Renderer → snapshot/classify; publish custom-компонента с именем `FlowRoot` по-прежнему легален и не конфликтует); FlowRoot-root остаётся `content` в `splitHostPrimitives`; strip/metadata/split покрыты (fast-path идентичность объекта; custom-root + Overlay-sibling + slotted children — slotIndices корректны после split; symmetric-тест для `splitCanvas`; render-тест named-slot custom-компонента через реальный `EasyUiRuntimeProvider` после split); fail-closed кейсы `applyRegionPolicy`; тесты нейтральности FlowRoot (нет собственных стилей, наследование темы идентично); host-count ожидания 3→4 (`server/host-primitives.test.ts`, `Overlay.test.tsx`); `npm run validate:templates`, серверные validation/hash-тесты зелёные (обновить ожидания контрактной версии); `npm run verify` зелёный.
