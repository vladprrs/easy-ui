...
- `expired` **из RFC убран** (триаж R3-B3): посаженный контракт — свипер физически удаляет просроченных кандидатов, отдельного терминального статуса нет. «Истёк» показывается вычислимо по `expires_at` до момента удаления; хранить его негде и незачем. Исключения из удаления: `status='promoted'` (V14) и кандидаты с decision-строками (раунд3-BL-2, §3.2а) — они переживают TTL.
...
Инварианты: строка иммутабельна кроме `status/status_reason/acceptance_run_id/promoted_version`; кандидат не участвует в latest-active resolution, catalog list/search, bundle-export; bundle кандидата — только через draft-роут с owner-check (см. §2). Гигиена: свипер `expires_at` (GC-паттерн `.candidates/`), **строки `status='promoted'` свипер не удаляет** (иначе `ON DELETE SET NULL` молча обнулит provenance-ссылку версии — триаж V14) **и с R3a не удаляет строки, у которых есть decision-строка в `candidate_decisions`** (надгробие reject переживает TTL — триаж раунд3-BL-2, §3.2а), per-user cap живых кандидатов в `capabilities.limits` (триаж D3).
...
- **Свипер пропускает кандидатов с decision-строками (триаж раунд3-BL-2 — блокер).** Редакция v7 давала авто-«разотклонение»: свипер удалял отклонённого кандидата по `expires_at` (72 ч), каскад сносил надгробие, и повторный `POST …/candidates` той же сборки создавал чистого кандидата — то самое «разотклонение», которого §4.1 обещает не вводить («отмены reject НЕТ»). Решение: `sweepExpiredCandidates` **дополнительно пропускает** кандидатов, у которых есть строка в `candidate_decisions` — ровно так же, как он уже пропускает `status='promoted'`. Надгробие переживает TTL и живёт столько же, сколько ревизия.
- **Анти-воскрешение (там же).** Повторный `POST …/candidates` той же сборки (детерминированный `candidate_id`, §5) **возвращает того же отклонённого кандидата** с `rejected: true` и `decision {...}` — а не создаёт новый и не «сбрасывает» решение. Это тот же путь «повтор при неизменном `{componentId, rev, build_fingerprint}` возвращает ту же строку» (§4.1), просто теперь строка гарантированно жива.
- **FK + `ON DELETE CASCADE` остаются** (триаж R3-раунд2-B3) — как гарантия целостности на **прочих** путях удаления кандидата (ручная уборка, будущие GC-пути, каскады тестовых фикстур): строки-сироты не должны переживать исчезновение кандидата. Но **штатно каскад по `rejected` не срабатывает** — свипер до таких строк не доходит (см. выше). Каскад — единственный разрешённый способ удаления decision-строк; append-only-свойство таблицы этим не нарушается.
...
- Append-only: строки не обновляются и не удаляются (кроме каскадной уборки вместе с кандидатом — силами FK, а не ручного DELETE; штатным свипером такая уборка не инициируется).
...
- **Обязательный тест R3b (переписан триажем раунд3-BL-2; редакция v7 требовала прямо противоположного):** «`sweepExpiredCandidates` **НЕ** удаляет отклонённого кандидата (и его decision-строку) даже после `expires_at`; повторный `POST …/candidates` той же сборки возвращает **того же** кандидата с `rejected: true`».
...
- **reject запрещён для `status='promoted'`** → **`409 candidate_promoted`** (триаж раунд2-M3, код исправлен раунд3-MJ-1): отклонять уже опубликованную сборку нечем — она в каталоге, и надгробие только заблокировало бы recovery-повтор саги. `details` конверта несёт `{currentVersion}`.

  **Два разных кода — не путать (раунд3-MJ-1).** Строка `candidate_already_promoted` в v7 была назначена reject-ветке, хотя **уже занята** посаженным кодом: `markPromoted` отдаёт её при попытке пометить кандидата, который уже promoted (CAS-конфликт саги promote, `details {promotedVersion}`). Разные субъекты, разные вызывающие, разное восстановление, поэтому коды разведены явно:

  | Код | Кто отдаёт | Смысл | `details` |
  |---|---|---|---|
  | `409 candidate_already_promoted` | `markPromoted` (фаза B саги promote) | CAS: кандидат уже помечен promoted другим прогоном саги | `{promotedVersion}` |
  | `409 candidate_promoted` | `POST …/reject` (R3b) | отклонение запрещено: сборка уже опубликована | `{currentVersion}` |

  Оба кода перечисляются в contracts раздельно; done R3b включает тест «reject promoted-кандидата → `409 candidate_promoted`, не `candidate_already_promoted`».
- **отмены reject НЕТ.** Решение человека терминально: выход из отклонения — новая ревизия компонента (у неё другой `candidate_id`, §5), а не «разотклонение». Ручки `unreject`/DELETE не вводятся ни в R3b, ни позже без отдельного решения. Подпорки этого свойства — на уровне хранения: свипер не удаляет кандидатов с decision-строками, а повторный `POST …/candidates` возвращает того же отклонённого кандидата (§3.2а, раунд3-BL-2); без них TTL сам по себе работал бы как отложенный `unreject`;
- аудит-событие `candidate.rejected {candidateId, componentId, reason, actor}`;
- отклонение не мутирует `component_candidates` и не трогает раны — это надгробие для UI и для promote-предиката (§4.3.1). Следствие, фиксируемое явно: **отклонённый кандидат с живым (`queued|running`) раном занимает in-flight-слот до терминализации рана** — ожидаемое поведение, не дефект; reject не является отменой рана (для этого есть `POST /acceptance-runs/:runId/cancel`). Свипером отклонённый кандидат не удаляется **вообще** (не «до `expires_at`» — уточнение раунд3-BL-2, §3.2а).

`POST` выполняет validate head'а (тот же `validateComponentHead`, тот же троттлинг) и материализует строку **и бандл по rev кандидата** (амендмент A10: вариант `ensureDraftCandidate` с явной ревизией — не head; после этого расхождение head'а с кандидатом — не условие корректности кадра, а advisory-метка `headDiverged` в evidence). Бандл **пинуется против `gcCandidates`**: GC получает провайдер пинов (список `sourceHash` нетерминальных ранов из БД — смена сигнатуры `server/components/candidates.ts`) и не вытесняет запиненные; `POST /api/acceptance-runs` → `409 candidate_evicted`, если бандл отсутствует. Повтор при неизменном `{componentId, rev, build_fingerprint}` возвращает ту же строку (`cached: true`) — **включая случай, когда эта строка отклонена: ответ несёт `rejected: true` и `decision {...}`, POST не «пересоздаёт» кандидата и не снимает решение** (раунд3-BL-2, §3.2а). Ошибки validate — коды P8. Списочный `GET` не вводится (триаж A7).
...
1. Предпроверки: `head_rev === baseRev` (иначе `409 revision_conflict {currentRev}` — единый канонический код, `candidate_stale` не вводим); `sha256(source(baseRev)) === sourceHash`; при переданном `acceptanceRunId` — статус `pass|pass_with_exceptions` и совпадение `policy_profile_hash` (R2); **rejected-предикат (R3b, переработан триажем раунд2-M1)** — см. ниже; `expectedCatalogRevision` — opt-in строгий CAS каталога.
   ...
   *Место реализации (уточнено раунд3-m-4):* предикат — **флаг-независимый хелпер над `db`, вызываемый внутри `promoteComponent` в фазе A.1, до `repo.stage`**, а **не** метод `server/acceptance/repo.ts`: acceptance-репозиторий инжектится только при `EASYUI_ACCEPTANCE_MATRIX=1` (`main.ts:261`), и предикат, живущий в нём, был бы выключен ровно в той конфигурации, где обходится R1-путём — то есть не выполнял бы своё назначение. Детали объёма — §11-R3b.

   *Примечание о кортеже:* `design_system` в `(component_id, design_system, rev)` **избыточен** — ревизия уже пинует дизайн-систему компонента, и пара `(component_id, rev)` определяет `design_system` однозначно. Оставлен в формулировке ради симметрии с составом `candidate_id` (§5) и читаемости предиката; на семантику не влияет.
...
*Статус ветки «promoted-кандидат → вернуть версию» (уточнение раунд3-m-5):* это **проектное поведение R2+, в посаженном коде R1 его нет**. Фактически повтор саги по уже promoted-кандидату падает в фазе B на `markPromoted` с `409 candidate_already_promoted` (§4.1) — что и обеспечивает искомую идемпотентность (публикация не дублируется), но иным ответом, чем описанная recovery-ветка. Все рассуждения ниже об идемпотентности опираются на **это** свойство; «вернуть версию» — целевая форма, реализуемая отдельным решением.

**Взаимодействие rejected-предиката с идемпотентностью (триаж раунд2-M3).** Предикат §4.3.1 проверяется **до фазы A** — то есть до любых мутаций. Ветка recovery «`promoted`-кандидат с renderable `promoted_version` → вернуть её» (**R2+, в коде R1 её нет** — см. выше) стоит **раньше** и через предикат не проходит повторно: уже опубликованная версия не может быть заблокирована задним числом надгробием (а reject `promoted`-кандидата и запрещён — §4.1, `409 candidate_promoted`). В текущем коде ту же роль играет отказ `markPromoted` в фазе B (`409 candidate_already_promoted`): дубль-публикации не возникает. Идемпотентность саги, таким образом, сохраняется в обе стороны: повтор успешного promote — тот же ответ; повтор после `fail()` — снова упирается в предикат, если ревизия отклонена.
...
| Write-путь: **POST create компонента** с переданным `figma` | `server/routes/components.ts` | **пишет seq-строку** (`rev = 1`, `seq = 1`) — см. ниже (раунд3-BL-1) |
| Write-путь: компонентный PUT с переданным `figma` | `server/routes/components.ts` | **тоже пишет seq-строку** — см. ниже (раунд2-B1) |
| Write-путь: `PUT …/provenance` | `server/routes/components.ts` | канонический источник seq-строк |
| Write-путь: `repo.restore` | `server/repos/components.ts` | seq-строка со значением `resolveProvenanceRaw(sourceRev)`, включая tombstone — см. ниже (раунд2-M2/раунд3-m-3) |
...
**Правило B1 (обобщено триажем раунд3-BL-1): seq-строку пишет ЛЮБОЙ write-путь с переданным `figma`.** Редакция v7 распространяла правило только на компонентный PUT — и оставляла дыру ровно того же класса, что закрывал B2 раунда 2, но для **новых** компонентов: `POST /api/components` с `figma` клал значение только в колонку `rev 1`, а первый же последующий `PUT …` без `figma` (или source-PUT) резолвился бы... на колонку — до тех пор, пока где-то не появится seq-строка; после появления любой seq-записи порядок `(rev, seq)` разрешался бы корректно, но промежуточные состояния и no-op-детекция расходились бы с provenance-путём. Правило поэтому формулируется по субъекту, а не по конкретной ручке:

> **Каждый write-путь, получивший значение `figma`, пишет seq-строку `component_provenance` для создаваемой/указанной ревизии тем же внутренним хелпером, что `PUT …/provenance`.** Пути: **POST create** (`rev = 1`, `seq = 1`), **компонентный PUT** (новая ревизия), **`repo.restore`** (новая ревизия), **`PUT …/provenance`** (указанная или head-ревизия).

- Тезис «`save` ничего не копирует» остаётся в силе **только про НЕпереданный `figma`** (inherit ≠ копирование).
- **Дедуп append-only-истории (раунд3-m-1):** seq-строка пишется, **только если резолвнутое сырое значение (`resolveProvenanceRaw` для целевой ревизии) отличается от нового**. Идентичное значение не порождает записи — иначе каждый source-PUT драйвера, таскающий один и тот же `figma`, раздувал бы историю строками-дублями, а `audit`-лента показывала бы фантомные «правки provenance». Правило единое для всех четырёх путей.
...
- Скрипт-гейт резолвера (§6, ниже) держит allowlist легальных писателей/читателей `figma_json`: сам резолвер, миграция backfill и **все четыре write-пути** — новый write-путь без seq-строки обязан упасть на гейте.
- Обязательные тесты: «`PUT …/provenance` на head → компонентный PUT с новым `figma` → резолвнутый provenance головы = **новое** значение»; «**POST create с `figma` → source-PUT без `figma` → резолвнутый provenance головы = значение из POST**» (раунд3-BL-1).

**`repo.restore` дописывает seq-строку (триаж раунд2-M2).** Restore создаёт новую ревизию из `sourceRev`. Простой перенос значения в колонку не работает: seq-записи на более поздних ревизиях **старше по `(rev, seq)`** и затенили бы восстановленное. Поэтому restore **добавляет seq-строку на новую ревизию** со значением `resolveProvenanceRaw(componentId, sourceRev)` — **в том числе tombstone `NULL`**, если у `sourceRev` provenance пуст (иначе восстановление «пустого» состояния было бы невыразимо).

*Уточнение раунд3-m-3:* `resolveProvenanceRaw(sourceRev)` пишется **в обе стороны** — и в колонку `component_revisions.figma_json` новой ревизии, и в seq-строку. Записать резолвнутое значение только в seq-строку, а в колонку скопировать сырую колонку `sourceRev` (как делает restore сегодня) — значит развести два источника: колонка показывала бы одно, фолбэк-резолв исторического `rev` — другое, и любой будущий читатель колонки (например, экспорт бандла R4+) получил бы значение, отличное от того, что видит API. Инвариант: **для ревизий, созданных после R3a, колонка и последняя seq-строка той же ревизии несут одно и то же значение.**

Обязательный тест: «`PUT …/provenance` на rev N → `restore` rev N−1 → голова резолвится в provenance rev N−1».
...
**AssetUsage.** В отчёт использования ассетов (`server/repos/assets.ts`) добавляется секция `provenance` — иначе ассеты, на которые ссылается только seq-запись, выглядят неиспользуемыми. *Обоснование сужено триажем раунд2-m3:* про «потерю RESTRICT-защиты» из v6 — снято как неверное: DELETE-роута у ассетов нет, и FK от provenance к ассетам не заводится, так что защищать RESTRICT'ом нечего. Секция `provenance` остаётся исключительно ради **видимости** использования в отчёте. Принятая оговорка: секция provenance видит только seq-строки; ассет, на который ссылается `figma_json` только исторической не-head ревизии (без seq-строки), в usage не виден — осознанно: после R3a write-пути всегда оставляют seq-строку, head покрыт backfill'ом.
...
- Компонентный PUT **и POST create** с `figma` продолжают работать (совместимость) — и после R3a пишут seq-строку тем же внутренним путём, что `PUT …/provenance` (правило B1, см. выше). Драйвер/скилл перестают слать `--figma` в верб `component` **только после посадки резолвера** (R3a), и переходят на `driver.mjs provenance` / `PUT …/provenance`.
...
- **R3 — provenance и UI. Разложена триажем R3-M5 на три волны с явным file ownership** (в v5 это был один пункт-абзац на три несвязанных подсистемы):
  ...
  - **R3a — provenance-слой.** Миграция (`component_provenance`, `candidate_decisions` с FK+unique-индексом, **backfill seq=1 по head-ревизиям**, срез provenance в `currentDataFingerprint`; номер — следующий свободный, §8), cross-revision резолвер в **двух формах** (`resolveProvenanceRaw` + обёртка, §6), `PUT /api/components/:id/provenance` с authz+аудитом, **seq-запись из ВСЕХ write-путей с переданным `figma` — POST create (`rev=1, seq=1`), компонентный PUT, `repo.restore`, provenance-PUT** (единый внутренний хелпер; колонка ревизии продолжает заполняться; **дедуп: строка пишется только если резолвнутое сырое значение отличается от нового** — правило B1/m-1 §6), **`repo.restore` пишет `resolveProvenanceRaw(sourceRev)` в обе стороны** — и в колонку новой ревизии, и в seq-строку (включая tombstone), перевод no-op-детекции на резолвнутое сырое сравнение с канонизацией входа, `validateStoredFigma` на `resolveProvenanceRaw`, секция `provenance` в AssetUsage (видимость, не RESTRICT), **set-based резолв в `figmaByComponent` (без N+1)**, contracts/openapi/sdk, верб `driver.mjs provenance <componentId> <figma.json> [--rev N]`.
    ...
    *Транзакционность (раунд3-m-2):* seq-строка пишется **внутри той же транзакции, что `repo.save`/`repo.create`/`repo.restore`**, а не отдельным вызовом после. Иначе крэш между двумя записями оставляет ревизию с provenance в колонке и без seq-строки (или наоборот) — расхождение, которое резолвер не различает от легитимного фолбэка. Все четыре write-пути — атомарны по паре «ревизия + seq-строка».
    *Done дополнительно:* скрипт-гейт резолвера в `npm run verify` (по прецеденту `check-openapi-drift.ts`/`check-renderer-pin.ts`, §6) с **закрытым allowlist'ом из резолвера, backfill-миграции и четырёх write-путей**; тесты наследования и no-op из §6; тест «component-PUT с новым `figma` после provenance-PUT → резолвнутый provenance головы = новое значение»; **тест «POST create с `figma` → source-PUT без `figma` → provenance головы = значение из POST»** (раунд3-BL-1); тест «provenance-PUT на rev N → restore rev N−1 → голова резолвится в provenance rev N−1»; тест дедупа «повторная запись идентичного `figma` не создаёт новой seq-строки»; **`npm run perf:library`** (горячий путь `figmaByComponent`, §6-M5); e2e-сценарий «provenance-PUT не создаёт ни новой ревизии, ни новой версии»; драйвер/скилл перестают слать `--figma` в верб `component` **только в этой волне** (не раньше).

  - **R3b — reject.** `POST /api/component-candidates/:candidateId/reject` (§4.1, включая **`409 candidate_promoted`** для promoted-кандидата — отдельный код, не занятый `markPromoted`'ом `candidate_already_promoted` — и `details` с существующим решением), **rejected-предикат promote по `(component_id, design_system, rev = baseRev)` на обоих путях, независимо от `candidateId` и от `EASYUI_ACCEPTANCE_MATRIX`** → `409 candidate_rejected` (§4.3.1), **правка свипера: пропускать кандидатов с decision-строками** (§3.2а), аудит `candidate.rejected`.
    *Место предиката (раунд3-m-4):* флаг-независимый хелпер над `db`, вызываемый **внутри `promoteComponent`, фаза A.1, до `repo.stage`**. **НЕ** в `server/acceptance/repo.ts`: тот инжектится только при `EASYUI_ACCEPTANCE_MATRIX=1` (`main.ts:261`) и был бы выключен именно в конфигурации, где предикат обходится R1-путём. *Примечание:* `design_system` в кортеже избыточен (ревизия пинует DS), оставлен для симметрии с составом `candidate_id`.
    *Файлы:* `server/routes/acceptance.ts` (candidates-роуты), `server/acceptance/repo.ts` (reject/decisions + свипер), `server/components/promote.ts` (предикат-хелпер), contracts.
    *Done дополнительно:* тест «reject → promote **без** `candidateId` (R1-путь) → `409 candidate_rejected`»; тест «reject promoted-кандидата → `409 candidate_promoted` (именно он, не `candidate_already_promoted`)»; **тест «свип НЕ удаляет отклонённого кандидата после `expires_at`; повторный `POST …/candidates` той же сборки возвращает того же кандидата с `rejected: true`»** (§3.2а, раунд3-BL-2); тест «предикат срабатывает при `EASYUI_ACCEPTANCE_MATRIX=0`»; тест идемпотентности «promoted-кандидат: повтор promote не создаёт второй версии — в коде R1 упирается в `409 candidate_already_promoted` фазы B, предикат `candidate_rejected` не срабатывает» (§4.3).
...
| B2 | No-op-детекция figma-PUT сравнивает с сырой колонкой ревизии — после посадки резолвера возвращала бы `unchanged` на изменившуюся provenance (и наоборот) | §6: сравнение переведено на **резолвнутый** provenance в канонической форме (*«`JSON.stringify(figmaSchema.parse(...))` с обеих сторон» — **сужено раундом 2 (m4)**: write-сторона канонична по построению, канонизируется только вход и — в фолбэк-ветке — колонка ревизии*); write-путь (`server/routes/components.ts`) включён в перечень путей §6; тест «PUT provenance → идентичный component-PUT → `{unchanged: true}`» |
...
| M6 | §12.6 «бэкфилл — к R3» смешивает три разные работы и не закрыт | §12.6: provenance-бэкфилл **не нужен** (резолвер падает на колонку ревизии) — ***отменено раундом 2 (B2)**: посылка о наследовании в `save` неверна, бэкфилл head-ревизий **нужен** и входит в миграцию R3a*; acceptance-бэкфилл legacy-версий — **R4+**; в R3 — только read-only отчёт: колонка «есть/нет acceptance evidence» в `driver.mjs audit --versions` |
...
| M3 | **Нет семантики отмены reject; reject `promoted`-кандидата не запрещён** — и то и другое ломало бы идемпотентность саги promote | §4.1: reject запрещён для `status='promoted'` → `409 candidate_already_promoted` *(**код исправлен раундом 3 (MJ-1)** на `409 candidate_promoted` — исходный занят `markPromoted`'ом)*; **отмены reject НЕТ** — единственный выход из отклонения — новая ревизия (записано явно). §4.3: идемпотентность саги сохранена — предикат M1 проверяется **до фазы A**, а recovery-ветка по `promoted`-строке стоит раньше и предикат не проходит заново |
...
Отклонённых находок раунда 2 нет. Дополнительно зафиксировано из текста ревью (не находка): **отклонённый кандидат с живым (`queued\|running`) раном занимает in-flight-слот до терминализации рана** — ожидаемое поведение, записано в §4.1. *(Хвост формулировки «и не удаляется свипером до `expires_at`» **отменён раундом 3 (BL-2)**: отклонённый кандидат не удаляется свипером вообще.)*

### Раунд 3 (дельта, v8)

Дельта-ревью редакции v7 против посаженного кода: **2 блокера, 1 мажор, 6 миноров**. Все находки приняты, решения финальные и внесены в текст выше. Раунд подтвердил закрытие блокеров раунда 2 (seq-запись из компонентного PUT, backfill, FK+unique на `candidate_decisions`, две формы резолвера) и вскрыл два системных дефекта: незакрытый write-путь `POST create` и конфликт TTL-свипера с терминальностью reject.

| # | Sev | Находка | Решение (внесено) |
|---|---|---|---|
| BL-1 | blocker | **`POST create` с `figma` открывает дыру B2 для новых компонентов.** Правило раунда 2 распространялось только на компонентный PUT; при создании компонента `figma` попадал лишь в колонку `rev 1`, seq-строки не возникало — тот же класс дефекта, что закрывал B2, но для новорождённых компонентов | §6: правило **обобщено на ВСЕ write-пути с переданным `figma`** — POST create (`rev=1, seq=1`), компонентный PUT, `restore`, provenance-PUT; единый внутренний хелпер. §11-R3a: объём и allowlist скрипт-гейта переписаны под четыре пути. Тест: «POST с `figma` → source-PUT без `figma` → provenance головы = значение из POST» |
| BL-2 | blocker | **Каскад FK + свипер = авто-unreject через 72 ч.** `ON DELETE CASCADE` (решение раунд2-B3) в паре с TTL-свипером означал: по истечении `expires_at` отклонённый кандидат удаляется, надгробие уходит каскадом, повторный `POST …/candidates` создаёт чистого кандидата — прямое противоречие с §4.1 «отмены reject НЕТ»; фактически отложенный `unreject` по таймеру | §3.2а/§3.2: `sweepExpiredCandidates` **дополнительно пропускает** кандидатов с decision-строками (как уже пропускает `promoted`) — надгробие переживает TTL. Анти-воскрешение усилено: **повторный `POST …/candidates` той же сборки возвращает того же отклонённого кандидата (`rejected: true` + `decision`)**, а не создаёт нового. FK `ON DELETE CASCADE` **остаётся** — целостность на прочих путях удаления, но штатно по `rejected` каскад не срабатывает. Тест §3.2а **переписан** (v7 требовал обратного): «свип НЕ удаляет отклонённого кандидата; повторный POST возвращает `rejected: true`». §4.1 приведён в соответствие |
| MJ-1 | major | **Код `candidate_already_promoted` уже занят** `markPromoted`'ом с другой семантикой (CAS фазы B саги promote, `details {promotedVersion}`); reject-ветка v7 переиспользовала его для «нельзя отклонить опубликованное» — два разных состояния под одним кодом, клиент не различает | §4.1: reject-ветка получает **отдельный код `409 candidate_promoted`** с `details {currentVersion}`; в §4.1 добавлена таблица разведения обоих кодов, в contracts-перечне они перечисляются раздельно. Done R3b: тест «reject promoted → `candidate_promoted`, не `candidate_already_promoted`» |
| m-1 | minor | Append-only-история раздувается дублями: каждый source-PUT драйвера с неизменным `figma` порождал бы новую seq-строку | §6 (правило B1): seq-строка пишется **только если резолвнутое сырое значение отличается от нового**; правило единое для всех четырёх путей. Тест дедупа — в done R3a |
| m-2 | minor | Не сказано, атомарна ли пара «ревизия + seq-строка»; крэш между записями оставляет расхождение, неотличимое от легитимного фолбэка | §11-R3a: seq-строка пишется **внутри той же транзакции**, что `repo.save`/`repo.create`/`repo.restore` |
| m-3 | minor | `restore` писал резолвнутое значение только в seq-строку, а в колонку копировал сырую колонку `sourceRev` — колонка и seq расходятся, будущий читатель колонки (экспорт R4+) увидит другое значение | §6: `restore` пишет `resolveProvenanceRaw(sourceRev)` **в обе стороны**. Инвариант: для ревизий после R3a колонка и последняя seq-строка той же ревизии несут одно значение |
| m-4 | minor | Предикат M1 в `server/acceptance/repo.ts` был бы выключен при `EASYUI_ACCEPTANCE_MATRIX=0` (репозиторий инжектится только под флагом, `main.ts:261`) — то есть ровно там, где предикат и обходится R1-путём | §4.3.1/§11-R3b: предикат — **флаг-независимый хелпер над `db` внутри `promoteComponent`, фаза A.1, до `repo.stage`**. Тест «предикат срабатывает при флаге OFF» — в done R3b. Отмечено: `design_system` в кортеже избыточен (rev пинует DS), оставлен для симметрии с `candidate_id` |
| m-5 | minor | §4.3 опирается на recovery-ветку «promoted-кандидат → вернуть версию», которой в коде R1 нет — обоснование идемпотентности повисает | §4.3: ветка помечена «**R2+, в коде R1 её нет**»; фактическая идемпотентность обеспечивается отказом `markPromoted` в фазе B (`409 candidate_already_promoted`) — записано явно в обоих местах §4.3 |
| m-6 | minor | §15 раунд 1 содержит формулировки, отменённые последующими раундами, без пометок — читатель принимает их за действующие решения | §15 раунд 1: ячейки **M6** («бэкфилл не нужен») и **B2** («каноническая форма с обеих сторон») аннотированы курсивом — «отменено раундом 2 (B2)» / «сужено раундом 2 (m4)» |

Отклонённых находок раунда 3 нет.
