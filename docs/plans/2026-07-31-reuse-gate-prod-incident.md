...
**I5 — Критерий включения enforce — ЭСКАЛИРОВАН И ЗАКРЫТ РЕШЕНИЕМ ВЛАДЕЛЬЦА 2026-07-31**
(владеет: оркестратор)
Автоматическое переключение было запрещено [записью готовности](../audit/2026-07-31-reuse-gate-enforce-readiness.md)
(**KEEP SHADOW**), и решение эскалировано человеку — ровно так, как предписывал §5.4. Владелец
проекта воспользовался предусмотренной веткой «принять риск и разрешить `enforce` на
доказательствах T0»: [запись о включении](../audit/2026-07-31-reuse-gate-enforce-decision.md).
Прод переведён в `REUSE_GATE=enforce` отдельным деплоем **после** выкатки волн 3–4, то есть
после устранения первопричины инцидента (`driver.mjs` без `intent`). Критерии отката в `shadow`
записаны в решении.
