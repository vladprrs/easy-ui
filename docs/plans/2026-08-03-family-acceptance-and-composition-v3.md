...
| Повторные captures неизменившихся cases (**сценарий §19.8: правка одного ассета/темы; кроме unknown impact**) | полный rerun | <10% | `run.progress.reused/total` на сценарии W6; **KPI измеряется после закрытия W5a — последнего bump `algoVersion`** (до этого фингерпринты мигрируют, см. D1) |
...
**A3. Resume — не мутация упавшего run'а, а дешёвый новый run.** Run иммутабелен; resume = новый run по тому же `{candidateId, caseSetId}`, переиспользующий per-case результаты по `case_fingerprint` (D1) и пересуёмывающий только недостающие. Плюс **автоматический внутрираночный retry инфраструктурных сбоев**: per-case бюджет `maxInfraRetries` (дефолт 2) на основе **новой таксономии исходов джобы** `jobOutcome: ok|worker_crash|timeout|queue_full|subprocess_error` (W1a; существующая классификация `noise.ts` — это качество консоли *завершившегося* капчура, D11, а не исход джобы — крэш chromium/таймаут до неё не доходят). Retry — только на не-`ok` исходах джобы; продуктовые ошибки (`productErrors`) не ретраятся. Форсирование — `refresh: "none"|"failed"|"all"|{caseIds:[…]}`, причина пишется в evidence.
...
**A10. Захват запинен к кандидату.** Сегодня `ensureDraftCandidate`/`enqueueComponentDraft` всегда читают **head** — за 8–15 минут run'а head может смениться, и кадры молча снимутся с другого билда. W1a вводит enqueue по явному `{rev, sourceHash}` кандидата: бандл материализуется при `POST /api/components/:id/candidates` **по rev кандидата, не по head** (вариант `ensureDraftCandidate` с явной ревизией). После этого равенство head и кандидата **не** является условием корректности кадра: расхождение с head — **advisory-метка в evidence** (`headDiverged: true`), run продолжается — иначе resume (A3) был бы невозможен при активной правке автором. Кандидат-бандл **пинуется против `gcCandidates`** (TTL 24ч / 32 МБ LRU): `gcCandidates` получает провайдер пинов (список `sourceHash` нетерминальных ранов из БД — смена сигнатуры в `server/components/candidates.ts`) и не вытесняет запиненные; `POST /api/acceptance-runs` → `409 candidate_evicted`, если бандл уже отсутствует (проверка и на постановке, и при вытеснении).
...
- **D1. `case_fingerprint`** — ядро reuse/дедупа/P1.4, **component-scoped**:
  ...
  ```
  `acceptance_case_results` дополнительно несёт `component_id` (денормализация), reuse проверяет владение. Границы W2/W3/W4/W5a поднимают `algoVersion` и обнуляют накопленный reuse — признанная плата за поэтапность; reuse-KPI замеряется после закрытия W5a (последний bump), на сценарии W6.
...
- **D10. Свёртка run-вердикта из per-case (контракт W1a, полная).** Пер-гейтовые статусы case: `pass|fail|indeterminate|skipped|error`. Свёртка run'а: `fail` — хотя бы один case `fail` **или `indeterminate`** по обязательному гейту (indeterminate блокирует приёмку, но несёт `class:"indeterminate"` и диагностику — «увеличить маргин», «непрозрачный фон» — а не визуальный дефект); `error` — есть case `error` после исчерпания `maxInfraRetries` и нет `fail`; `cancelled` — по cancel; watchdog/дренаж → `error`; `pass` — все обязательные гейты всех cases `pass`. Гейты `not-implemented` в свёртке **не участвуют** (они не обязательные по определению фазы); `pass_with_exceptions` возможен только при `allowExceptions` (в `default-v1` выключено — RFC §2.1). Алиасы наследуют вердикт целевого case; `reused` эквивалентен свежему; `skipped` допустим только для необязательных гейтов. Пустой набор cases (или всё схлопнулось в алиасы без целей) → `422 empty_case_set` на постановке. Инвариант-тест: `reused`/`skipped`/`alias` не могут замаскировать `fail`. Каждый failed case несёт `severity: {rank, class: "structural"|"geometry"|"aa"|"raw"|"indeterminate", score}`; `GET /cases` и run-репорт сортируют по нему.
...
**Замеры W1b (2026-08-03, дев-машина, 8 CPU — прод 1-CPU и медленнее).** Инструмент: `node scripts/measure-acceptance.mjs --cases N` (сам поднимает Bun preview `SERVE_DIST=dist EASYUI_ACCEPTANCE_MATRIX=1` на изолированном `DATA_DIR`, публикует фикстурный компонент-atom и гоняет холодный + тёплый ран). Профиль `default-v1` (render + geometry-probe + determinism на выборке), состояние кода — W1b **до** объединения сессий (W3), то есть режим «две джобы на case».

| N cases | Холодный run | На 1 case | Тёплый run (reuse) | Байты CAS | Пик RSS сервера |
|---:|---:|---:|---:|---:|---:|
| 20 | 40,8 с | 2,04 с | 0,51 с (20/20 reused) | 84,5 КБ | 274 МБ |
| 49 | 96,3 с | 1,96 с | 0,51 с (49/49 reused) | 201 КБ | 276 МБ |

Масштабирование линейное (2,04 → 1,96 с на case), оба рана — `pass`, 0 провалов. `rssPeakMb` — `VmHWM` только серверного процесса: chromium живёт в отдельном воркере (`spawnWorker`), его пик сюда не входит. Фикстура — тривиальный atom (один `div`), поэтому число измеряет накладные расходы конвейера, а не рендер продуктового компонента.

**Вывод по гейту O1: O1 не требуется по замеру.** 49 cases = **1,6 мин** холодного рана против порога 15 мин — запас ×9 даже до W3; чтобы упереться в порог на 1-CPU проде, каждый case должен подорожать почти на порядок. Оговорки: (1) дев-машина 8 CPU, прод медленнее — но узкое место здесь последовательное (джобы ставятся по одной), а не параллелизм; (2) тяжёлый продуктовый компонент дороже фикстуры; (3) `--cases 49` мерился на одинаковых по стоимости случаях. Пересмотреть решение, если замер W7 на реальной 49-кейсовой семье (`pay-payment-card`) даст >5 мин.
...
Добавить в `docker-compose.yml` проброс `EASYUI_ACCEPTANCE_DISABLED: ${EASYUI_ACCEPTANCE_DISABLED:-}`, `EASYUI_VALIDATE_DISABLED`, `EASYUI_THEME_RESOLVER_V2_DISABLED`, `EASYUI_ACCEPTANCE_MATRIX: ${EASYUI_ACCEPTANCE_MATRIX:-}`, `EASYUI_COMPOSITION_V3: ${EASYUI_COMPOSITION_V3:-}`. Пустые значения безопасны: все флаги читаются строгим сравнением с `"1"`. По `EASYUI_SURFACES` **правятся доки, compose не трогается**: релиз f5eaa65 осознанно включил surfaces по умолчанию (`:-1`), устарела формулировка в `docs/server-api.md` — исправить её; смена compose-дефолта молча выключила бы работающую фичу. Done: деплой; `capabilities` подтверждает управляемость **существующих** флагов (`componentValidate`, `acceptancePromote`); новые `EASYUI_ACCEPTANCE_MATRIX`/`EASYUI_COMPOSITION_V3` до W1a/W8a кодом не читаются — их проверка входит в done W1a/W8a. Перед W1a — **бэкап prod-volume** (канон `/deploy`).

### W1a — Durable-схема, run и гейты без capture-матрицы

**Объём.** Миграция **v25**: `component_candidates` (RFC §3.2), `acceptance_runs` (RFC §3.3 + `case_set_id`, `policy_profile_id`, `progress_json`, `impact_json`), `acceptance_cases` (+ поля D11, `severity_json`), `acceptance_case_results` (+ `component_id`), TEXT-колонки A9 на `component_publishes` (`DEFAULT NULL`, без FK, канон плоских ADD COLUMN), `design_systems.acceptance TEXT NOT NULL DEFAULT 'off'` (**DEFAULT обязателен** — иначе старый INSERT из `routes/designSystems.ts` падает при откате образа). Partial unique «≤1 нетерминальный run на кандидата» (первый partial index в проекте; маппинг ошибки → 409). Роуты `POST /api/components/:id/candidates`, `GET /api/component-candidates/:id`, `POST/GET /api/acceptance-runs*` (+`/cases`, `/evidence`, `/cancel`). Оркестратор вне помпы + watchdog (D2) + стартовая уборка. Гейты `contract`/`defaults`/`render`/`determinism` (на выборке, §4.2)/`audit`; `geometry` — **advisory-only** (v1-семантика в вердикт не входит — она и есть исходный дефект §19.2; боевой гейт приезжает в W3); `visual`/`readiness`/`regression`/`interactions` = `not-implemented`. Байтовый канал capture→CAS (A4: acceptance-джобы не ингестят в asset-store). Пин кандидата (A10: материализация бандла по rev, провайдер пинов в `gcCandidates`). Таксономия `jobOutcome` (A3). Per-run evidence-манифест + `SHA256SUMS` — **PNG-капчуры потребляются уже в W1a**: гейт `determinism` сравнивает повторные капчуры, evidence-манифест включает их SHA (это ответ, зачем image-джобы до появления `visual`). Свёртка D10. Источник cases — examples кандидата.

**Авторизация (контракт для всех acceptance/case-set роутов).** `requireUser` + owner компонента по денормализованному `component_id` (или admin — существующий short-circuit `requireResourceOwner` для админа сохраняется); `share`/`capture`-принципалы — 403 всегда (инвариант `catalogCandidates.ts`); артефакты CAS отдаются **только** через `runId`-scoped роут с проверкой владельца рана (ручек «по sha» нет).
...
**Done.** `npm run verify`; unit: fingerprint-детерминизм и component-scope (два компонента с одним sourceHash **не** делят результаты), стартовая уборка, watchdog, D10-свёртка (reused/skipped не маскируют fail), partial-index→409, авторизация (share/capture 403), CAS-байты не попадают в asset-store (счётчик строк `assets` до/после), пин кандидата (смена head посреди run'а **не** меняет снимаемый бандл, evidence несёт `headDiverged: true`; вытеснение бандла → `candidate_evicted`); e2e `e2e/preview/acceptance-run.spec.ts` (**capture-спеки живут в `e2e/preview/` — dev-проект не поднимает `SERVE_DIST`, `ScreenshotService.available()` вернул бы 501**); миграция на копии прод-БД; `EASYUI_ACCEPTANCE_MATRIX` не задан → 404 всех новых ручек.
...
**Объём.** `promoteComponent`: опциональные `candidateId`/`acceptanceRunId`, `409 acceptance_run_in_flight`, запись A9-ссылок в Phase B. Слоты `server/components/validate.ts`: **не** «третий системный слот» (на 1 CPU это +1 тяжёлый typecheck поверх capture), а приоритетная схема: оркестратор конкурирует за существующий `VALIDATE_GLOBAL_CONCURRENT=2` и **не занимает per-user слот пользователя** (`inFlightUsers` ключуется userId — иначе интерактивный validate владельца run'а получает 429 на 15 минут; оркестратор работает под системным principal с отдельным ключом). CLI `driver.mjs accept <id> [--case-set] [--json]` + зеркала. При `EASYUI_ACCEPTANCE_MATRIX` OFF promote с `candidateId`/`acceptanceRunId` → `422 acceptance_matrix_disabled` (A7). Правка слотов — отдельно откатываемый шаг: деплоится и наблюдается до включения matrix на проде.
...
**Объём.** Гейт `visual` (A5): нормализация размеров (crop по `cropLineage`, pad до общего холста, `indeterminate` при несводимости), метрики `rawDiffPct/aaDiffPct/maxChannelDelta/regions(≤12)/bestOffset`, per-case пороги из политики/манифеста; severity-ранжирование (D10) в действии; инвариант-тест D5. Bump `algoVersion` (последний — после закрытия W5a фингерпринты стабильны, reuse-KPI замеряется на сценарии W6).
...
| `docs/server-api.md` | W0–W7, W9 | append-only по секциям волны |
...
| `server/acceptance/**` (включая `runner.ts` и роуты `server/routes/{acceptance,caseSets}.ts`) | трек A серийно | владелец — текущая волна трека A; новые гейты — новыми файлами в `gates/` |
| `server/components/candidates.ts` | W1a (пин GC), W6 (`sourceShapeHash`) | серийно по порядку волн |
| `server/maintenance.ts` | W1a | эксклюзив |
| `server/components/validate.ts` | W1c | эксклюзив трека A |
| `server/screenshot/noise.ts`, `src/capture/{readiness.ts,CaptureSurface.tsx}`, `server/figma.ts`, `src/acceptance/caseSetSchema.ts`, `src/library/*` | W4 / W4 / W2 / W2 / W9 | по волне-владельцу |
...
Все blocker'ы раунда 1 подтверждены закрытыми, кроме нового противоречия, внесённого самим фиксом B4. Внесено в v3:
- **N1 (blocker, принято)**: правило «терминализация при смене head» убрано из A10 — оно противоречило A3 (resume при активной правке автором становился невозможен) и делало не-head кандидата неприёмным. Пин = материализация бандла по rev кандидата; расхождение с head — advisory-метка `headDiverged` в evidence.
- **N2 (major, принято)**: D10 дополнена — `indeterminate` блокирует `pass` (class `indeterminate` в severity), `not-implemented` вне свёртки, `pass_with_exceptions` только при `allowExceptions`, пустой набор → `422 empty_case_set`, cancel/watchdog/дренаж → `cancelled`/`error`.
- **N3 (major, принято)**: retry опирается на новую таксономию `jobOutcome` (исход джобы), а не на `noise.ts` (качество консоли завершившегося капчура) — A3/W1a.
- **N4/N5 (major, принято)**: done W0 сужен до существующих флагов; по `EASYUI_SURFACES` правятся **доки**, compose не трогается (f5eaa65 включил surfaces осознанно).
- **N6 (major, принято)**: назван механизм пина — провайдер пинов в `gcCandidates` (смена сигнатуры `candidates.ts`), материализация бандла в `POST /candidates`; файл добавлен в §6.
- **N7 (major, принято)**: обосновано потребление PNG до W3/W5a — гейт `determinism` и SHA-evidence с W1a; `determinism` добавлен в список гейтов W1a.
- **N8 (medium, принято)**: таблица §6 дополнена (`runner.ts`/роуты — серийное владение треком A, `candidates.ts`, `maintenance.ts`, `validate.ts`, `noise.ts`, `figma.ts`, `caseSetSchema.ts`, `src/library/*`, W0 в `docs/server-api.md`).
- **N9–N12 (minor, принято)**: единая точка reuse-KPI (после W5a, сценарий W6); дренаж при `EASYUI_ACCEPTANCE_DISABLED` — на следующем старте (env читается один раз); `422 acceptance_matrix_disabled` для promote со ссылками при OFF; убрана ложная посылка про 404 админу.
