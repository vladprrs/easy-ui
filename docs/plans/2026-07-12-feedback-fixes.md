# План v3: устранение проблем из developer-фидбэка (2026-07-12)

Источник: `docs/easy-ui-developer-feedback.md` (14 пунктов P0–P2). Раунд 1 ревью (Codex gpt-5.6-sol, max): 6 blocker / 18 major / 1 minor — все приняты. Раунд 2: 6 blocker / 5 major — все приняты. Триаж обоих раундов в конце.
...
2. **Доставка — собственный event-адаптер, только для custom-компонентов** (builtin остаются payloadless — деградация зафиксирована в acceptance matrix):
   ...
   - обёртка резолвит params в литералы и исполняет действия **своим executor'ом**: state-действия (`setState/pushState/removeState`) — напрямую через store-обёртку (H.1), не через `execute()` (который повторно резолвит `$`-ключи — `resolveAction`/`pushState` рекурсивно интерпретируют `$state/$id`); терминальные — через наши handlers (navigate/back/openUrl/restart). `$if` срезается до исполнения. Корреляция для inspector — synchronous, correlation id на emit.
...
5. **ABI/совместимость — shims v2**: ABI v1 неизменен. Компонент, импортирующий новый модуль `easy-ui/runtime` (тип `EasyUIComponentProps` = BaseComponentProps + `emit(name, payload?)` + `slots`, функции `token()`/`Icon` из F.4), публикуется с `hostAbiVersion: 2` и шимами `/api/shims/v2/*` (v2 = v1 + easy-ui-runtime; правило версионирования shim manifest из плана server-api-versioning соблюдено). Loader поддерживает оба ABI; render-status проверяет совместимость host ABI. `definition.capabilities?: {typedEvents?: true, namedSlots?: true}` — по ним валидация отличает custom-возможности; typed events/slots требуют ABI v2.
...
3. **Аутентификация капчера — scoped capture-session**: discriminated запись `{token(32B random), kind: "prototype"|"component", allowedUrls: string[] (точный immutable snapshot), expected: {revision|version, componentManifestHash, pins, dsMetaVersion, rendererBuild}, props? (для component-капчера — в сессии, не в URL), expiresAt}` в памяти. Токен минтится **при dequeue** job'а (не при постановке в очередь), TTL = job deadline + 30s, многоразовый в рамках job'а, revoke в `finally`. `allowedUrls`: конкретный capture-route+query, конкретный revision/version endpoint, только pinned bundle URLs, pinned `/api/assets/:id`, pinned `/api/design-systems/:id/versions/:v`, `/api/shims/*`, статика SPA (index/js/css). Worker шлёт `X-EasyUI-Capture: <token>` только на loopback-origin (`context.route`-инжект по exact origin; все прочие запросы — abort). Сервер принимает токен только при: `server.requestIP()` ∈ loopback (все формы), метод GET/HEAD, нормализованный decoded-путь ∈ allowedUrls. `createHandler` переводится на `fetch(request, server)`.
4. **Async job API**: `POST /api/prototypes/:id/screens/:screenId/screenshot` `{rev?|version?, viewport, deviceScaleFactor?, theme?, waitForFonts?}` → `202 {jobId}`; `GET /api/screenshot-jobs/:jobId` → `{status: queued|running|done|error, result?: {imageUrl, assetId, width, height, consoleErrors, pageErrors, bundleHash, componentPins, rendererBuild, browserVersion}, error?}`. Пул concurrency=1, глубина очереди ≤5 (иначе 429). Jobs — in-memory, TTL результата 10 минут (PNG остаётся в assets). Без браузера/дистa → 501 `screenshot_unavailable` сразу на POST (ApiError расширяется 429/501). Dev: работает при `SERVE_DIST` (иначе 501 с message).
   **Доказательство снапшота**: capture-shell выставляет `__EUI_CAPTURE_READY__ = {status:"ready"|"error", revision, componentManifestHash, pins, dsMetaVersion, error?}`; worker канонически сравнивает с `session.expected` и падает при mismatch; `status:"error"` — быстрый fail вместо таймаута. Хеши (`componentManifestHash`, `builtinCatalogHash`) добавляются в revision DTO additively (сейчас их там нет — `server/repos/prototypes.ts:129`).
