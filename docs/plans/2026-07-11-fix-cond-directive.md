...
1. **`src/prototype/runtimeSpec.ts` (новый)** — `toRuntimeSpec(spec)`: глубокий обход `elements[*].props` (объекты и массивы на любой глубине, без захода внутрь веток). Переписывается **только точная doc-форма**: внешний non-null, non-array JSON-объект с единственным own-ключом `$cond`, значение которого — такой же объект с ровно ключами `if`, `then`, `else` → `{$cond: if, $then: then, $else: else}`. (Валидатор уже требует ровно `if/then/else` — validate.ts:65 отклоняет лишние ключи; зафиксировать тестом.) Любой другой объект с ключом `$cond` (legacy-данные из старых ревизий, core-форма, lookalike) остаётся нетронутым — политика leave-untouched. Вход не мутировать. `visible`, `on`, `children`, `root` — как есть.
...
5. **Тесты**:
   - unit `src/prototype/runtimeSpec.test.ts`: точная форма на верхнем уровне / во вложенном объекте / в массиве; falsey-ветки (`then: false`, `else: 0`); отсутствие мутации входа; lookalike — нетронуты: `{$cond: {...}, other: 1}`, `{$cond: {if, then}}`, `{$cond: {if, then, else, extra: 1}}`, core-форма `{$cond,$then,$else}`, массив на месте `$cond`-значения; директивоподобный объект внутри литеральной ветки `then`/`else` сохраняется буквально (адаптер не заходит в ветки); `$state`/`$bindState`/`$template` не тронуты;
   - validate: negative-тесты на нечисловой операнд `gt`, на директиву в корне `props` и на `$cond` с лишним ключом (`extra`);
   - интеграционный (PlayerShell/ScreenView): экран с `Text.text = {$cond: ...}` рендерит `else`-ветку, после `setState` — `then`-ветку; варианты canvas и non-canvas; ассертить отсутствие ErrorBoundary-фолбэка и console.error, а не только итоговый текст;
...
- Аудит старых ревизий БД и повторная семантическая валидация на restore/publish — отклонено для этого фикса: ревизии проходили действовавшую на момент сохранения (менее строгую) версию валидатора, и от любых не-канонических форм рантайм защищён политикой leave-untouched адаптера. Отдельная задача, если появятся реальные legacy-данные.
...
## Триаж ревью (раунд 2, сессия 019f5027-6cc5-7c42-9d96-2edf8c4833eb)

**Вердикт: blocking-возражений нет, план одобрен к реализации.** Все 6 minor-замечаний приняты и внесены: детектор определён как non-null non-array объект; тесты на `extra`-ключ (адаптер + валидатор), на буквальное сохранение директивоподобных объектов в ветках; ассерты на ErrorBoundary/console.error в интеграционных тестах; формулировка про legacy-ревизии ослаблена.
