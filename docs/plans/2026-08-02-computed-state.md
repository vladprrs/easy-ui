...
- [x] Stage 3: исполнение выполнено 2026-08-02 по команде пользователя (T1 → волна T2/T3/T4 → T5 ∥ T6; 6 коммитов 1043996..8cffa39 + Fable-фикс `acceptancePromote` в контракте capabilities в составе T4). Финальный гейт: `npm run verify` зелёный, полный e2e 122 passed / 1 known skip, runtime-приёмка: snap стартеризованной cart-фикстуры exit 0, все 4 op'а сверены визуально (count 2 / sum 3 / sumProduct 8970 / add 8770 = 8970+300−500), API round-trip сохранения не теряет `computed`. Деплой не выполнялся (по указанию).
