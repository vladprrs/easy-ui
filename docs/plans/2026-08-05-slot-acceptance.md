...
**Resolver modes (v3):** `resolveSlotBindings` has two modes. **Gating** (run start + PUT/dry-run): full status policy per A2, hard 422s. **Reconstruction** (`runCases` fallback — pins were already authorized at createRun): status-blind, resolves `componentId/bundleHash/propsHash/slotsHash` from the publish row refusing only if the row is absent — a child archived mid-flight must not turn a resumable run into a terminal error (frame inputs are status-free, so fingerprints stay byte-identical). If reconstruction does refuse (row gone), the run terminalizes with a **named** status reason, not a bare `gates.error` string. **Regression tests (T2.1):** restart simulation (caseSets map entry deleted) → recomputed frame fingerprint equals persisted `acceptance_cases.frame_fingerprint`; same simulation with a child flipped to `archived` → run still reconstructs.
...
1. **Bytes-only + HTTP retrieval (v3, blocker fix):** overlay jobs force `deliver:"bytes"` — no asset, no visual-baseline poisoning path, "capture-only" literally true. But bytes were an in-process-only channel (consumed by the acceptance gate; `GET /api/screenshot-jobs/:jobId` returns `job.result` as JSON, and the contract union is `image|geometry` — a raw `Uint8Array` would serialize as numeric-keyed JSON and fail the contract). Therefore: new **`GET /api/screenshot-jobs/:jobId/bytes`** returning `image/png` (RESULT_TTL = 10 min, documented), and the job-status result for overlay jobs is a new declared variant `{kind:"image-bytes", width, height, byteLength, pngSha256}` — metadata only, never the array. `screenshotJobResultSchema` gains the variant; driver.mjs/skills get an overlay-aware fetch path (currently they hard-require `kind==="image"`/`assetId`). Done-criterion: an HTTP client fetches the overlay PNG end-to-end.
...
**Stage 2 round 3:** final targeted verification pass (1 reviewer over the v3 deltas only) — confirm no blocking objections remain, then Stage 3.
...
- **W2 (after W1):**
  ...
  - T2.5 (after T2.1 — needs `slotsHash` on cases): `server/acceptance/runner.ts` (per-case carry guard, `BaselineCaseSnapshot` Pick += props_hash/slots_hash) + `server/acceptance/impact.ts` if needed + their tests. Done: changed-slot case not carried while rest of family carries; slot-free positive-carry suite (`impact.test.ts:346+`) green.
