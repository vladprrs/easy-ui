...
## Overflow ownership (`elements[].overflowOwnership`)

`overflowOwnership: {axis: "x"|"y", mode: "scroll", viewportOwner?: string, expectedContentOverflow?: boolean}` declares that the element's subtree overflows **inside its own scrollport** along the named axis, rather than pushing the scene behind it. A `FlowRoot` screen 390 px wide holding two horizontal rails with 552 px of content is the canonical case: the overflow is the design, not a defect.

The declaration is read by geometry collection, not by the renderer: the subtree's contribution to the screen's `content` box is clipped to the scrollport boundary along the declared axis, and the overflow itself is recorded as a fact (`overflowOwners[]` with `scrollportBounds`, `scrollContentBounds`, `ownedOverflowPx`, `crossAxisOverflowPx`, `clipChain`). `viewportOwner` names the element whose box is the window, for the case where a wrapper declares and a child scrolls. `expectedContentOverflow` asserts that overflow along that axis is expected at all.

- Exactly one axis: two-axis ownership is deliberately inexpressible in v1 ("overflows in every direction" is the absence of a constraint, not ownership). `mode` is only `"scroll"`.
- The same token exists in composition v3 layout and compiles to the same runtime field.
- The measurement verdict gains two codes: `unowned-overflow` (overflow outside any declaration; a document with **no** declarations keeps the previous `content-clipped-by-frame` verbatim) and `owned-overflow-exceeds-axis` (declared on one axis, spilling on the other).
- **Writes are gated** by the geometry-ownership kill-switch (`422 flow_overflow_ownership_disabled`); reading a stored document is never gated. See `docs/server-api.md#владение-переливом-flowroot-eui-br-09-план-2026-08-08-9-featuresflowoverflowownershipv1`.
