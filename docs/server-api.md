...
| `POST` | `/acceptance-runs/:runId/resume` | `202` **новый** ран с `resumedFromRunId`/`attempt`/`resumedFrom` |
...
##### Продолжение остановленного рана (EUI-BR-06, план 2026-08-08 §6, `features.acceptanceResumeV1`)

**`POST /acceptance-runs/:runId/resume`** (тело `{}`) — **новый ран, а не воскрешение**: терминальный ран неизменяем (на него ссылаются receipts публикаций, `evidence_manifest_hash` и promote-инварианты), поэтому сервер ставит свежий ран поверх того же кандидата, набора и профиля и отвечает `202` с его `runId`, `resumedFromRunId`, `attempt` и `resumedFrom {runId, attempt, status, statusReason, phase, lastCompletedPhase, jobIds}` — прежняя ошибка едет вместе с родословной. Идемпотентность детерминирована: `idempotency_key = "resume:<sourceRunId>:<attempt>"`, поэтому повтор возвращает тот же ран, а второй параллельный resume того же кандидата отсекает существующий in-flight-индекс (`409 acceptance_run_in_flight`).

Продолжить можно **только ран, объявивший себя продолжаемым**. Такими его делают три пути: стартовая уборка после рестарта процесса (`status_reason: "interrupted"` — залипшие `running`/`pending` случаи незавершены по определению, их никто не закрывал), typed timeout инфраструктуры (`status_reason: "phase_timeout"` с названной фазой) и circuit breaker аллокации (`renderer_unavailable` | `capture_budget_exhausted` | `queue_starvation` после трёх подряд случаев с исходом класса аллокации). Всё остальное — вердикт, cancel, `refresh_scope_empty` — `409 run_not_resumable`: пересъёмка это `POST /acceptance-runs` с `refresh`, а не resume. Уже продолженный ран — `409 run_already_resumed` с id продолжения в `error.runId`.

**Что переиспользуется:** завершённые гейты фазы `validate` (`contract`/`defaults`/`audit`), чей **per-gate отпечаток** совпал, переезжают дословно и не исполняются заново; всё от `capture` и дальше снимается заново — кадра предка могло не существовать вовсе. Частично исполненный случай отдаёт ровно свои завершённые structural-гейты.

**Фазы рана:** `resolve` → `validate` → `allocate-renderer` → `capture` → `readiness` → `geometry` → `visual` → `determinism` → `verdict`. Фаза наблюдается **покейсово**, поэтому run-level `lastCompletedPhase` — это **минимум** по незавершённым случаям («дальше этой фазы ран целиком не продвинулся»); ран без незавершённых случаев отдаёт `verdict`. Аллокация рендерера получает **собственный дедлайн** (15 s до вехи `{"type":"allocated"}` воркера, после неё стартует дедлайн капчура), а её отказы — собственные исходы таксономии джобы: `renderer_unavailable` (терминальный: бинаря нет либо `launch` бросил; сюда же теперь мапится `501 screenshot_unavailable`, который раньше ретраился) и `allocate_timeout`.
...
##### Decoration-aware geometry (EUI-BR-05, план 2026-08-08 §5, `features.geometryDecorationOwnershipV1`)

Диагностика V0-D3 опровергла буквальный симптом («transform-хвост расширяет layout-union»): `visit()` исключает transform/out-of-flow узлы из union'а с самого Geometry Contract 2.0. Реальных маршрутов до блокера оказалось **четыре**, и волна закрывает каждый:

1. **probe-уровень показывал только paint-габарит.** `rects[]`/`content` — union `getClientRects()` всех потомков, то есть 391×112 с хвостом; автор переносил это число в `expectedGeometry` и получал безусловный `layout-overflow` против честных 391×88. Теперь замер отдаёт **оба**: `content` (paint-габарит, не переименован и не пересчитан) и новый `layout` (union in-flow боксов — ровно то, по чему считается вердикт). `driver.mjs geometry` / `preview --probe geometry` печатают их одной строкой `bounds: layout=… (declare THIS as expectedGeometry) paint=…`.
2. **Краска хвоста блокировала вердикт.** Узел вне потока, чья **pre-transform** коробка вложена в union остального поддерева, — декорация: её `postTransformPaintBounds` объясняет overflow целиком, и вердикт получает класс `paint-overflow-decoration`, который **не блокирует без единого допуска**. `allowPaintOverflow` больше не нужен (он отключал контроль краски целиком, включая необъявленную). Частично объяснённый overflow остаётся доволновым `paint-overflow-*` и блокирует.
3. **`expectedSurfaces` по макету давали `surface-mismatch` на `paint`.** При decoration-владении поверхность `paint` наблюдается **с поправкой**: сравнивается краска внутри оболочки «layout + не-decoration эффекты», а сырое измерение едет рядом (`paintOwnership {raw, observed, decorationSources}`). Хвост при этом никуда не девается — в кадр и в визуальный дифф уходят те же пиксели.
4. **Хвост-сиблинг убивал `rootBounds`.** `boxedGeneration` считал боксовых детей первого поколения без учёта потока, поэтому fragment-корень с декорацией объявлялся неизмеримым (`root: not-measured` ⇒ вечный `indeterminate` — ближайшее к «24 из 36 roots» фидбэка). Decoration-узел теперь прозрачен для этого спуска ровно как обёртка `display:contents`.

**Факты замера (аддитивные, вне отпечатка).** У каждого детального измерения появляется `outOfFlowNodes[]`: одна запись на **узел** (а не на причину, как в `effectSources`) с `causes[]`, `preTransformBounds` (offset-геометрия; `null` — offset-системы нет), `transform`, `postTransformPaintBounds`, `role`/`roleSource` и `participation` — причиной участия в каждой поверхности. `GEOMETRY_CONTRACT_VERSION` остаётся `2`: расширение замера кадры не инвалидирует (прецедент W1a, дифференциальный тест «замер расширился ⇒ `frameFingerprint` байт-в-байт прежний»).

**Per-case декларация.** `cases[].geometryOwnership: {"<elementKey>" | "<elementKey>//<суффикс elementPath>": {role: "decoration", participatesIn: ["paint"]}}` (`optional` без дефолта, до `limits.caseSetMaxGeometryOwnership` узлов) нужна там, где авто-правило молчит: pre-transform коробка **не** вложена в контур (неоднозначный DOM) либо поверхности объявлены по макету. Суффикс сравнивается с хвостом `elementPath` — внутренние узлы компонента своего маркера не имеют и наследуют ключ ближайшего, поэтому у тултипа и пузырь, и хвост оба `pay-tooltip`.

- **Слой — `frame` + `verdict`, и оба настоящих.** `frame`: декларация требует кадра, снятого волной (доволновой кадр не несёт `preTransformBounds`, и восстановить по нему decoration-семантику нечем), поэтому такой кейс получает `geometryContractVersion: 3` в `frameFingerprint` — **условно по манифестному факту**, известному до съёмки (отпечатки считаются при постановке рана, и условность «по результату измерения» дала бы кейсу два разных fingerprint). `verdict`: она же меняет прочтение фактов и пересчитывается без единого пикселя.
- **Авто-правило — не кадровый вход вовсе.** Оно не меняет пикселей, а меняет интерпретацию, поэтому инвалидируется вердиктным `geometryOwnershipPolicyVersion` в снимке политики (симметрично `comparisonPolicyVersion` слоя сравнения у BR-04) — то есть стоит **recompute'а**, а не пересъёмки и не bump'а `CASE_FINGERPRINT_ALGO_VERSION`.
- **Доволновой кадр + затребованная семантика = отказ.** Пересчёт кейса с объявленным `geometryOwnership` по метрикам без фактов владения невозможен: `reevaluateGates` отвечает `reevaluable: false`, и случай уходит на честную пересъёмку (прецедент `expectedSurfaces`).
- **Валидация против злоупотребления.** Декларация на **in-flow контейнере с layout-детьми** — `geometry_ownership_invalid` (`severity: error`, `ref` — путь узла) и `fail` гейта геометрии. Проверка ведётся по фактам замера (`ownershipViolations`), а не по форме манифеста: «этот селектор указывает на контейнер» — суждение о снятом кадре. Правило живёт в `gates/audit.ts`, а применяет его гейт геометрии: `audit` по `GATE_ORDER` исполняется до единственного измерения случая, и отклонять декларацию, не имея факта, значило бы отклонять её по догадке.

`EASYUI_GEOMETRY_OWNERSHIP_DISABLED=1` — общий kill-switch с BR-09: авто-правило не применяется (`rootBounds` fragment-корня снова `null`, краска декорации снова блокирует), манифест с `geometryOwnership` отвергается `422 geometry_ownership_disabled`, сбор и вердикт доволновые byte-for-byte.

##### Владение переливом FlowRoot (EUI-BR-09, план 2026-08-08 §9, `features.flowOverflowOwnershipV1`)

Warning `content-clipped-by-frame` считался union'ом **всех** маркеров против кадра — без осей и без владельцев. Экран `FlowRoot` шириной 390 px с двумя горизонтальными rail'ами по 552 px контента получал top-level предупреждение о переливе, хотя перелив тут и есть дизайн: rail прокручивается внутри своего окна, а сцену за собой не двигает.

`elements[].overflowOwnership: {axis: "x"|"y", mode: "scroll", viewportOwner?, expectedContentOverflow?}` (`optional` без дефолта; тот же токен объявляется в composition layout и компилируется в **то же** runtime-поле) говорит ровно это: «поддерево этого элемента переливается по объявленной оси внутрь своего scrollport'а».

- **Ось одна.** Двухосевое владение в контракте v1 невыразимо намеренно: «переливается во все стороны» — это не владение, а отсутствие ограничений. `mode` — только `"scroll"`: клип без прокрутки владением не является (он и так виден `clipChain`).
- **Сбор.** Вклад поддерева в габарит экрана (`content`) ограничивается границей scrollport'а **по объявленной оси**; `viewportOwner` переносит окно на названный элемент, если объявляет обёртка, а прокручивает ребёнок. Перелив из замера не исчезает — он получает владельца: `overflowOwners[]` несёт `scrollportBounds`, `scrollContentBounds` (полный габарит ленты — те самые 552), `ownedOverflowPx`, `crossAxisOverflowPx`, `clipChain` и, при объявленном `expectedContentOverflow`, наблюдённый факт. `rects[]` при этом **не** трогается: на нём стоят существующие потребители probe'а.
- **Вердикт замера.** `content-clipped-by-frame` не поднимается для перелива, покрытого владельцем. Остаётся два новых кода: `unowned-overflow` (warn) — перелив за пределами деклараций (у документа **без** деклараций код остаётся прежним `content-clipped-by-frame` байт-в-байт), и `owned-overflow-exceeds-axis` (warn) — владелец объявлен по одной оси, а поддерево переливается по другой. Vertical spill и overlap регионов судятся как прежде.
...
- **Promote (врезка в сагу, `server/components/promote.ts`).** Ран, провалившийся по **интеграционному** вердикту, всё ещё может стать версией — но только при выполнении всех четырёх условий §8: (1) набор объявил `comparison.ownership` **вместе** с `comparison.dependencyPolicy: "require-eligible-acceptance"`; (2) субъект чист по фактам рана — субъектный вердикт каждого случая плюс все невизуальные гейты **полного** дерева (`subjectPromotionEligible` над строками `acceptance_cases.gates_json`, а не над evidence-манифестом); (3) **каждая** runtime-зависимость дерева `slotBindings` опубликована и доказана собственной приёмкой — терминальным promotable-раном своей опубликованной версии (`component_publishes.acceptance_run_id`/`acceptance_run_ids` → `acceptance_runs.status`, честная проверка по БД); (4) провальный интеграционный вердикт сохраняется дословно в квитанции promote: `subjectPromotion: [{runId, caseSetId, integrationVerdict, subjectVerdict, dependencies:[{componentId, name, version, runId}]}]` в ответе и в `receipt_json` migration-саги. Каждое условие отказывает **своим** кодом (`422 subject_promotion_ownership_missing` — объявлена половина контракта; `subject_promotion_subject_failed` — грязный субъект, провал невизуального гейта или отсутствующий субъектный вердикт; `subject_promotion_dependency_ineligible` — с перечнем виновных зависимостей и причиной `not_published|pin_not_renderable|no_active_publication|no_acceptance_evidence|acceptance_not_promotable`; набор, объявивший владение и не связавший ни одной зависимости, отказывает тем же кодом с пустым списком: у остатка нет владельца). Набор **без** деклараций и выключенный kill-switch дают доволновой `422 acceptance_run_not_passed` байт-в-байт. Overlay-зависимости (неопубликованные кандидаты) сюда не входят — их держит более строгая проверка §W3 (тот же билд, а не просто приёмка).
...
##### Полный registry-resource barrier (EUI-BR-03, план 2026-08-08 §3, `features.resourceBarrierV4`)

Барьер ресурсов волны W2 доказывал только то, что видел: `<img>`, CSS-фон/маску и inline-SVG `<image>`. Мимо него проходили srcset-кандидаты, псевдоэлементы, шрифты, ассеты кандидата и — главное — **реестр иконок темы**: тема доезжает асинхронно, `__easyUiShared.icons` наполняется уже после снятия манифеста, и registry-`<img>` попадал в кадр недогруженным либо ловился дифом как `resource_late_after_barrier`.

- **Фаза `registry` до первого манифеста.** Барьер ждёт применения темы, прежде чем что-либо перечислять. «Тема есть или нет» страница не угадывает: число иконок реестра объявляет **сервер** в `bootstrap.resources.themeIcons` (тем же чтением темы, которым строит манифест шрифтов). `0` — фаза завершается мгновенно (дедлока у ДС без иконок нет by construction), `>0` — ожидание до `resourceBarrier.registryDeadlineMs` (500 мс) внутри общего бюджета барьера; исчерпание — типизированный `resource_barrier_timeout` с `ref: "registry:<observed>/<expected>"`, а не молчание.
- **Каналы манифеста** (контракт §6 фидбэка): `img`, `img-srcset`, `css-background`, `css-mask`, `css-content`, `icon-registry`, `font`. Три из них **report-only** и в `expected` не входят: srcset-кандидаты (decode-цель — только `currentSrc`, иначе фаза декода утраивается), шрифты (их уже ждёт фаза `fonts`) и объявленные исходником, но не отрендеренные ассеты. Псевдоэлементы `::before/::after` собираются под **документным предикатом** — один скан `document.styleSheets`: нет ни одного правила с `content|background-image|mask-image` ⇒ канал пропускается целиком; недоступная (cross-origin) таблица трактуется консервативно как «может быть». Выборка элементов ограничена тем же потолком 400, переполнение — `resource_manifest_overflow` с `ref: "pseudo:<N>"`.
- **Ожидаемый манифест.** `bootstrap.resources.expectedAssets` несёт id ассетов кандидата и его overlay-детей (те же, что питают allowlist) плюс hint `cases[].preloadAssets`. Наблюдённые доказываются наравне с остальными, необнаружённые остаются report-only: hint **не** освобождает сервер от обнаружения и не валит кадр за неотрендеренную ветку.
- **Пер-ресурсная запись** в `readiness.evidence.resourceBarrier.resources[]` и в receipt: `{assetId, ownerElementKey, ownerComponentId, channel, discoveredAt, url, requested, loaded, decoded, completedBeforeStableFrame, phase}`. `ownerComponentId` на странице всегда `null` (слот-привязки живут на сервере); `discoveredAt` — `bundle|resolved-tree|dom|computed-style|request`. Потолок receipt'а — 200 записей, обрезка названа флагом `resourcesTruncated`. Рядом едут `policyVersion: 4` и блок `registry {iconsExpected, iconsObserved, waitedMs, timedOut}`.
- **Сужение вердикта.** Гейт `readiness` отвечает `indeterminate` с типизированным `resource_barrier_incomplete` **только** когда все причины `met:false` барьерные (`resource_late_after_barrier`, `resource_decode_failed`, `expected≠decoded`). Шрифты, overflow, `resource_barrier_timeout` и любая смесь с не-барьерной причиной остаются `fail` как до волны. **Инвариант D5 не меняется**: `readinessMet` остаётся `false`, поэтому сравнивающие гейты случая пропускаются и кадр не становится визуальным доказательством; run-verdict тоже остаётся `fail` (свёртка не трогается) — различие видно в `gates[].metrics.codes` и в case-receipt.
- **Шимы `easy-ui/runtime`** (v2/v3/v4) читают `__easyUiShared` и `React` **в момент вызова**, а не захватывают на eval: шим, импортированный до `ensureEasyUiShared()` (или переживший замену объекта), навсегда возвращал из `Icon` `null` при живом реестре.

Слой отпечатков — **кадровый, двумя входами**: политика барьера стала v4 (другой `readinessPolicyHash`), и во `frameFingerprint` добавился `themeContentHash` (версия темы ДС + отсортированные пины её ассетов; `FIELD_LAYERS.themeContentHash = ["frame"]`) — иначе смена иконки темы не инвалидировала бы кадр. Поэтому включение волны стоит **пересъёмки** затронутых кейсов, а не re-diff'а.

**Kill-switch — иерархия из двух этажей**, оба restart-required (политика читается один раз на процесс и питает `policyProfileHash`, `readinessPolicyHash`, `rendererFingerprint`):

| Переменная | Поведение | `features.resourceBarrierV4` / `resourceBarrierPolicyVersion` |
| --- | --- | --- |
| `EASYUI_RESOURCE_BARRIER_DISABLED=1` | барьера нет вовсе, каждый профиль возвращается в **свою** доволновую политику (`default-v1` → v1, `pixel-strict-v1` → v2, `reference` → v2) | `false` / `1` |
| `EASYUI_RESOURCE_BARRIER_V4_DISABLED=1` | барьер работает, но по политике v3 **byte-for-byte** (нет фазы `registry`, новых каналов, записей и сужения вердикта) | `false` / `3` |
| оба сняты | волна активна | `true` / `4` |

Смоук-ключи включения — `acceptance.readinessPolicyVersion: 4` и `features.resourceBarrierPolicyVersion: 4`.
...
- **Шаги каскада не короткозамыкаются друг на друга** (волна 2026-08-06 §W4). Отказ пересчёта на шаге 2 **не** равен приговору кадру: причина откладывается, случай пробуется шагом 3 (re-diff того же кадра, где метрики измеряются заново), и только если невозможен и он — ран платит пересъёмкой. Ровно этого требует новая возможность отказа `recomputeVisual`: случай с `textAaBudget`, чьи сохранённые метрики сняты до волны и не несут `edgeResidual`, пересчитать нельзя, а **пересравнить** — можно.
...
```jsonc
{
  ...
  "cases": [{
    ...
    "comparison": { "matte": "#FFFFFF" },                              // W4: матирование сравнения ("none" — не матировать)
    "textAaBudget": "live-text-v1",                                    // W4: именованный пресет живого текста
...
Два поля **уровня случая** (не `policy.perCase`), оба строго опциональны и без zod-дефолтов:

| Поле | Что делает | Слой инвалидации |
|---|---|---|
| `comparison.matte` | `"none"` (дефолт потребителя) либо `"#RRGGBB"`: гейт `visual` компонует **обе** картинки над этим цветом после crop/placement/pad и **до** любой метрики | `comparison` ⇒ re-diff сохранённого кадра |
| `textAaBudget` | имя серверного пресета бюджета растрового текста; сейчас единственное значение — `"live-text-v1"` | `comparison` + `verdict` |

**Matte** (строка 7 фидбэка) закрывает пару «эталон экспортирован из Figma поверх непрозрачного
холста ↔ кандидат снят прозрачным». Капчур **не меняется** (`omitBackground:true` остаётся кадровым
контрактом): матирование живёт только на сравнении, поэтому его объявление даёт пересравнение того
же кадра, а не пересъёмку. Композитинг — straight-alpha `out = src·a + bg·(1−a)`, альфа результата
≡ 255; порядок фиксирован: `crop → place/pad → matte → метрики`. Факт матирования уезжает в метрики
гейта (`matteApplied: "#rrggbb"`) и в evidence: у матированного случая эталон-дериват пишется в CAS
(`normalized-reference.png`) даже без построения канвы. Побочное следствие — причина
`alpha-compositing` у такого случая **не называется вовсе**: после matte расхождений по альфе не
бывает by construction, и объяснять вердикт несуществующим событием нельзя.

**`textAaBudget: "live-text-v1"`** (строка 5 фидбэка, «Timer») — ответ на неустранимое: эталон
приёмки это Figma-PNG, у него нет renderer fingerprint, поэтому «один шрифтовой стек на паре
макет ↔ живой капчур» недостижим, и вместо свободной ручки продукт получает **документированный
профиль**. Числа принадлежат серверу — манифест объявляет только имя:

- `maxRawDiffPct` = **0.75 %** — выше этого расхождение уже не растровый шум;
- `minEdgeResidualPct` = **95 %** — та же константа, что у классификатора `text-raster-residual`
  (один источник правды: вердикт и его объяснение не могут разъехаться).

Вердикт: случай, провалившийся по своему бюджету, получает `pass`, **если** `rawDiffPct` ≤ порога
пресета **и** `edgeResidual.insidePct` ≥ порога пресета, то есть весь остаток лежит на собственных
контурах эталона. Факт применения — в метриках гейта (`textAaBudget {preset, maxRawDiffPct,
minEdgeResidualPct, applied}`), не в `causes`. Тюнинг порогов = **новый пресет** (`live-text-v2`):
одно имя не может означать в разное время разное. Смена/появление поля уводит случай в re-diff, а
не в пересчёт: пресет опирается на `edgeResidual`, которого в доволновых метриках нет вовсе, и
пересчёт по сохранённым числам честно отказывается (каскад пробует шаг 3 прежде пересъёмки).
...
- `layoutBounds` — union border-box'ов **in-flow** потомков маркера. Потомки с `position:absolute|fixed`
  ...
  - **Живой текст входит в контур** (контракт измерения 2, см. ниже): непустые (после `trim`)
    текстовые узлы in-flow элементов меряются через `Range.getClientRects()`. До этого строка,
    лежащая прямо в маркере или в обёртке `display:contents`, не имела border-box и не
    существовала для измерения вовсе: краску её гейт видел (`paintBounds` больше `layoutBounds`),
    а объяснить не мог — `paint-overflow-not-clipped` «no descendant effect explains it».
    Пробельные узлы отфильтрованы: перенос строки в JSX не двигает контур.
  - **Клип учитывается сверху вниз**: бокс каждого узла (и его текстовых узлов) пересекается со
    стеком клипающих предков **внутри поддерева маркера** (`overflow:hidden|clip`, `clip-path`).
    Клипнутая карусель — окно 350×40 с лентой 900×40 внутри — меряется по окну, а не по ленте.
    `clipChain[]` этого не заменяет: она восходящая, клипающий контейнер-потомок ей не виден.
- **Контракт измерения версионируется.** `GEOMETRY_CONTRACT_VERSION` (`src/capture/geometry.mjs`) —
  номер семантики `layoutBounds`; текущее значение — `2`. Значение `> 1` входит в
  `frameFingerprint` полем `geometryContractVersion`, поэтому смена семантики честно инвалидирует
  накопленные кадры: переиспользовать кадр, снятый по другой семантике, нельзя — вердикт геометрии
  сравнивал бы измерения из разных миров. `CASE_FINGERPRINT_ALGO_VERSION` для этого не годится: он
  в кадровый отпечаток не входит. Инвентаризация затронутых наборов —
  `bun scripts/audit-geometry-contract.mjs [--db <file>] [--server-url <origin>]`.
...
### Impact-driven gallery regression — `snap-plan` (волна W5, план 2026-08-07)

Пересъёмка галереи целиком стоила ретроспективе миграции YP v2 больше всего времени: правка одного
компонента заставляла снимать 43 экрана, потому что доказательства «этот экран не изменился» не
существовало. Волна вводит такое доказательство и одну read-only ручку поверх него.

`POST /api/prototypes/:id/snap-plan` (флаг `capabilities.features.impactedSnap`, потолок
`limits.snapPlanMaxScreens` = 256) отвечает по каждому экрану резолвнутой ревизии: `capture` с
названной причиной или `reuse` с подписанной сервером квитанцией. Ручка ничего не ставит в очередь
и ничего не пишет; матричной приёмкой (`EASYUI_ACCEPTANCE_MATRIX`) она **не** гейтится — галерейная
съёмка к приёмке не относится.

**Доказательство — `screenFrameFingerprint`**, sha256 кортежа, решающего пиксели **одного** экрана:

| Вход | Откуда |
|---|---|
| `prototypeInstanceId`, `screenId` | иммутабельная идентичность прототипа и экран кадра |
| `componentManifestHash` | `componentManifestHashOf` **подмножества пинов экрана** (пины ревизии ∩ resolved-дерево экрана) |
| `builtinCatalogHash` | ревизия |
| `designSystem`, `dsMetaVersion` | ДС **поверхности экрана** (`surfaceDesignSystem`) и её пин темы (`themePinsOf`) — строго per-screen, иначе мульти-ДС документ даёт ложный reuse |
| `screenSpecHash` | сам экран + документные входы рендера (`state`, `computed`, `device`, `designSystem`, `surfaces`) |
| `viewport`, `deviceScaleFactor`, `theme` | условия съёмки |
| `readinessPolicyHash`, `rendererFingerprint` | политика готовности и объявленный рендерер под ней |
| резолвнутая meta-версия темы + версия spacing-резолвера | `getLatestDesignSystemContent` для незапиннутой темы, пин — для запиннутой |

Отдельного `themeContentHash` нет и не нужно: версии ДС иммутабельны и append-only, поэтому номер
резолвнутой версии **и есть** содержимое. `rev` в хэш **не входит** и остаётся провенансом строки:
добавление экрана — это новая ревизия, и хэширование `rev` делало бы недоказуемыми ровно те экраны,
ради которых волна написана. Информации о пикселях `rev` не несёт — всё, что ревизия меняет для
данного экрана, уже представлено `screenSpecHash`, подмножеством пинов, `builtinCatalogHash` и пином
темы.

**Недоказанный reuse = capture.** Экран, чьё resolved-дерево не разворачивается полностью —
композиция без доступного на ревизии тела, тип без пина ревизии, вложенность глубже предела
раскрытия, — всегда `capture` с `reason: "unprovable"` и текстом причины в поле `unprovable`.
Остальные причины: `new` (кадров этого экрана нет вовсе), `renderer` (сдвинулся
`rendererFingerprint`; политика готовности входит в него), `theme` (версия темы, её пин,
spacing-резолвер или `builtinCatalogHash`), `impacted` (всё остальное — пины экрана, спека, условия
съёмки). Строка `reuse` несёт `reuseReceipt {screenId, screenFrameFingerprint, previousRev,
previousPngSha256, provenAt}`.

**Кадры пишет обычный галерейный путь** (`POST /api/prototypes/:id/screens/:screenId/screenshot`
с доставкой в asset-store) — таблица `prototype_screen_frames` (схема v34), retention: последние 5
ревизий на прототип (sweep на записи), `receipt_json` ≤ 64 КБ. Probe-джобы и джобы с
`candidateOverrides` кадров **не** пишут: у первых нет кадра, вторые рисуют неопубликованный код.
Строка без квитанции продолжает доказывать reuse — она лишь не умеет назвать причину пересъёмки
(деградация в `impacted`).

**`screenFrameFingerprint` — ключ reuse и только он.** Он не входит ни в один отпечаток приёмки
(`frameFingerprint`/`comparisonFingerprint`/`caseFingerprint`), не двигает
`CASE_FINGERPRINT_ALGO_VERSION` и не участвует ни в одном вердикте: галерейная съёмка и матричная
приёмка — разные контуры.

**Kill-switch `EASYUI_IMPACTED_SNAP_DISABLED=1`** гасит обе половины: ручка отвечает `404`,
...
```json
{
  ...
  "limits": { "elements": 500, "depth": 50, "bodyMiB": 1, "sourceKiB": 256, "assetMiB": 5, "repeatBudget": 2000, "repeatPerScreen": 20, "screenshotQueue": 5, "geometryRects": 2000, "flows": 24, "flowSteps": 50, "flowTotalSteps": 320, "flowDepth": 4, "compositionDepth": 5,
    ...
    "sourcePackageMaxExports": 256, "resourceBarrierMaxResources": 256, "resourceBarrierBudgetMs": 8000,
  ...
  "features": { "renderStatus": true, "screenshots": true, "visualRegression": true, "assets": true, "typedEvents": true, "repeat": true, "namedSlots": true, "themeVersions": true, "layoutContract": true, "flows": true, "computed": true, "screenRegions": true, "bundleExport": true, "bundleImport": true, "componentReuseGate": true, "compositionV2": true, "catalogMigration": true,
    ...
    "geometrySurfacesV3": false, "candidateDependencyOverlay": false, "suggestedPolicy": false,
    "resourceBarrier": true, "figmaSourcePackage": true, "runtimeSchemaDefaults": true, "captureNoiseSummary": true,
  ...
  "acceptance": { "policyProfiles": ["default-v1", "pixel-strict-v1"], "defaultPolicyProfile": "default-v1", "promotionPolicyProfiles": ["default-v1", "pixel-strict-v1"], "geometryContractVersion": 2,
    "comparisonSurfaces": ["root", "layoutUnion", "paint", "referenceExport"], "readinessPolicyVersion": 3 },
...
`acceptance` разводит два разных множества политик приёмки: `policyProfiles` — что примет `POST /acceptance-runs` в `policy` (иначе `422 unknown_policy_profile`), `promotionPolicyProfiles` — под каким профилем полученный вердикт [допускает публикацию](#promotion-policy-какой-ран-допускает-публикацию-волна-w3-план-2026-08-04) (иначе `422 acceptance_policy_mismatch`). Сегодня множества совпадают; различать их обязан клиент, а не догадка — пересечение задано конфигурацией сервера, а не инвариантом кода. `comparisonSurfaces` (волна W1a 2026-08-07) — закрытый список [поверхностей геометрии](#четыре-поверхности-геометрии-волна-2026-08-07-w1a), которые принимают `cases[].expectedSurfaces` и `cases[].comparisonSurface`; порядок совпадает с порядком `divergingSurfaces[]` вердикта. `readinessPolicyVersion` (волна W2 2026-08-07) — версия readiness-политики **дефолтного профиля**, то есть чем этот инстанс реально снимает кадры: `3` — строгая политика плюс [барьер ресурсов](#deterministic-resource-barrier--readiness-v3-волна-w2-план-2026-08-07). При `EASYUI_RESOURCE_BARRIER_DISABLED=1` здесь честно появляется доволновое значение профиля (`default-v1` → `1`); одно число на всех соврало бы, потому что `pixel-strict-v1` откатывается в `2`. Пара с `features.resourceBarrier`: флаг отвечает «барьер включён», версия — «чем снято». `geometryContractVersion` при этом **остаётся 2** и после волны 2026-08-07: замеры `rootBounds`/`referenceExportDims` аддитивны, ни один вход `frameFingerprint` не добавлен, корпус кадров не инвалидирован.
...
| `flowOverflowOwnershipV1` | [владение переливом FlowRoot](#владение-переливом-flowroot-eui-br-09-план-2026-08-08-9-featuresflowoverflowownershipv1): `elements[].overflowOwnership` (и composition layout-токен), вклад поддерева ограничен scrollport'ом по объявленной оси, факты `overflowOwners`, коды `unowned-overflow`/`owned-overflow-exceeds-axis` | матрицей **не** гейтится; общий с BR-05 `EASYUI_GEOMETRY_OWNERSHIP_DISABLED=1` → `false` и `422 flow_overflow_ownership_disabled` на **записи** документа с полем (чтение не гейтится) |
| `resourceBarrierV4` | [полный registry-resource barrier](#полный-registry-resource-barrier-eui-br-03-план-2026-08-08-3-featuresresourcebarrierv4): фаза `registry`, каналы srcset/псевдоэлементов/шрифтов/`icon-registry`, ожидаемый манифест ассетов кандидата, пер-ресурсные записи, `indeterminate resource_barrier_incomplete` на барьерных причинах | матрицей **не** гейтится; гаснет под **обоими** свитчами — `EASYUI_RESOURCE_BARRIER_DISABLED=1` и `EASYUI_RESOURCE_BARRIER_V4_DISABLED=1` (второй оставляет барьер по v3 byte-for-byte) |
| `resourceBarrierPolicyVersion` | **число**: фактическая версия политики барьера этого инстанса (`4` — волна активна, `3` — под v4-свитчём, `1` — барьера нет) | пара с `acceptance.readinessPolicyVersion`: флаг говорит «включено», версия — «чем снято» |
...
### Changelog платформенных capabilities (2026-08-07, ретроспектива миграции YP v2)

Волны плана `docs/plans/2026-08-07-migration-feedback-wave.md` (W1–W10). Правило волны то же, что у feedback-3: каждое новое поле инвалидирует **ровно один** слой каскада — `frame` (пересъёмка) → `comparison` (re-diff) → `verdict` (recompute). Единственный кадровый вход волны — `candidateOverlay` (и только у наборов, которые его объявили); всё остальное либо дешевле, либо не инвалидирует ничего. `acceptance.geometryContractVersion` **не поднимается**, полной пересъёмки корпуса эта волна не требует.

| Флаг | Что означает | Слой инвалидации | Как гаснет |
|---|---|---|---|
| `geometrySurfacesV3` | случай объявляет [четыре поверхности геометрии](#четыре-поверхности-геометрии-волна-2026-08-07-w1a) (`expectedSurfaces`, `comparisonSurface`, `clipExpectation`), вердикт **называет** разошедшуюся поверхность (`divergingSurfaces[]`, класс `surface-mismatch`) | по под-полям: `expectedSurfaces.referenceExport` + `comparisonSurface` → **comparison** (re-diff), `expectedSurfaces.root\|layoutUnion\|paint` + `clipExpectation` → **verdict** (recompute). Случай, не объявивший ничего нового, — байт-в-байт прежние хеши | `EASYUI_GEOMETRY_SURFACES_DISABLED=1` → вердикт целиком на легаси-ветке; без `EASYUI_ACCEPTANCE_MATRIX=1` — `false` |
| `resourceBarrier` | [детерминированный барьер ресурсов](#deterministic-resource-barrier--readiness-v3-волна-w2-план-2026-08-07) — readiness v3 у обоих профилей приёмки, режима `reference` и опт-ина галерейной джобы (`readiness: "barrier"`); потолки — `limits.resourceBarrierMaxResources`/`resourceBarrierBudgetMs` | **frame** для профилей, перешедших на v3: политика входит в `readinessPolicyHash`/`policyProfileHash`/`rendererFingerprint` | `EASYUI_RESOURCE_BARRIER_DISABLED=1` → каждый профиль в **свою** доволновую политику (default→v1, strict→v2, reference→v2), параметр `readiness:"barrier"` остаётся валидным no-op'ом. Читается один раз на процесс — смена значения требует рестарта |
| `candidateDependencyOverlay` | [`candidateOverlay`](#candidateoverlay-неопубликованные-зависимости-графа-план-2026-08-07-w3) в case-set-манифесте + overlay-форма slot-ребёнка: неопубликованный родитель и его неопубликованные зависимости принимаются **одним раном** | **frame** — и только у наборов с overlay; overlay-free набор байт-в-байт прежний | `EASYUI_CANDIDATE_OVERLAY_DISABLED=1` → `422 candidate_overlay_disabled`; без матрицы — `false` |
| `impactedSnap` | [`POST /prototypes/:id/snap-plan`](#impact-driven-gallery-regression--snap-plan-волна-w5-план-2026-08-07) — какие экраны галереи обязаны сниматься и почему, какие переиспользуются **с доказательством** | нет: `screenFrameFingerprint` — ключ reuse, ни в один приёмочный отпечаток и ни в один вердикт он не входит | `EASYUI_IMPACTED_SNAP_DISABLED=1` → ручка `404`, кадры экранов не записываются вовсе |
| `migrationCommit` | [сага миграционного коммита](#migration-commit-transaction--сага-миграционного-коммита-волна-w4-план-2026-08-07) `/api/migration-commits*` — resumable серверное состояние поверх существующих мутаций | нет (оркестрация) | `EASYUI_MIGRATION_COMMIT_DISABLED=1` → весь набор `404`; без матрицы — `false` |
| `suggestedPolicy` | [`suggestedPolicy` и advisory-протухание исключений](#suggested-policy-и-протухание-исключений-волна-w7-план-2026-08-07): предложение бюджета по типизированной причине + `policy_exception_stale` | нет — слой **report-only**: ни вердикт, ни promote от него не зависят | `EASYUI_SUGGESTED_POLICY_DISABLED=1` гасит обе производные; без матрицы — `false` |
| `figmaSourcePackage` | [пакет исходников Figma](#figma-source-package-волна-w8-план-2026-08-07) `/api/figma-source-packages*` и ссылка `figma.sourcePackageId` (потолок — `limits.sourcePackageMaxExports`) | **comparison** у зависимых случаев — и только через их собственный `referenceAssetId`; сам `sourcePackageId` **metadata-only** и не входит ни в один отпечаток | `EASYUI_SOURCE_PACKAGE_DISABLED=1` → ручки `404`, ссылка — `422 source_package_disabled` |
| `runtimeSchemaDefaults` | хост применяет Zod-дефолты схемы к props компонента, объявившего `definition.capabilities.runtimeSchemaDefaults`; флаг discovery отвечает «применяются ли дефолты **сейчас**», а не «умеет ли образ» | candidate id двигается сам — флаг объявлен в исходнике и уже учтён `sourceHash`; `BuildFingerprintInput` не расширялся | `EASYUI_RUNTIME_DEFAULTS_DISABLED=1` — **аварийный render-affecting** kill-switch вне отпечатков: приёмка флагнутых семей при нём недействительна (`runtime_defaults_disabled` в accept-status). Штатный откат компонента — републикация исходника без флага |
| `captureNoiseSummary` | сводка подавленного инфраструктурного шума капчура: `quality.suppressedCount` и `console.suppressed[{signature,count}]` в [receipt](#capture-receipt-волна-r5-план-2026-08-03-renderer-contract-2) | нет (receipt аддитивен) | kill-switch'а нет: блок аддитивен, а capture-маршруты SPA вынесены из-под `AuthProvider` — источник шума удалён, а не подавлен |
| `receiptEnvelopeVersion` | версия схемы [агентской квитанции драйвера](#скриншоты) (`envelope`) — **число**, а не булев флаг | нет | kill-switch'а нет; отсутствие ключа = образ старше W6b |

`acceptance.readinessPolicyVersion` — единственное новое поле блока `acceptance`; `limits` пополнились `caseSetMaxOverlayNodes`, `snapPlanMaxScreens`, `migrationCommitPhaseTimeoutMs`, `sourcePackageMaxExports`, `resourceBarrierMaxResources`, `resourceBarrierBudgetMs`. Восемь kill-switch'ей волны объявлены в `docker-compose.yml` с пустым дефолтом (= фича включена): без строки в compose переменная в контейнер не попадает вовсе, и «выключить в Dokploy» было бы невозможно. Прод-чеклист волны (бэкапы, rollback-window по миграциям v32–v36, go/no-go барьера, правило `EASYUI_PROMOTE_POLICY_STRICT`) — `.claude/skills/deploy/SKILL.md`.

**Три зафиксированных отказа/ограничения волны:**

1. **`GEOMETRY_CONTRACT_VERSION` не поднимается** (остаётся `2`). `rootBounds` и `referenceExportDims` измеряются **безусловно**, но это аддитивные факты: PNG не меняется, семантика `layoutBounds` не меняется, ни один вход `frameFingerprint` не добавлен — значит корпус кадров цел и golden не двигается. Цена честная и названа: доволновой кадр этих фактов не несёт, поэтому **первая** декларация новой поверхности на старом кадре стоит пересъёмки **одного** кейса (comparison-декларация — промах reuse-строки и `recapture:policy_delta`; verdict-декларация — явный отказ recompute «поверхность объявлена, факта в metrics нет»). Оговорка: per-surface вердикты существуют только при включённом geometry-гейте профиля (`gates.geometry: "not-implemented"` ⇒ поверхности не оцениваются).
2. **Прототипный overlay остаётся swap-only, composition-приёмки не существует.** [`candidateOverrides`](#overlay-кандидата-в-прототипном-кадре-план-2026-08-05-b) прототипной съёмки — подмена пина **уже опубликованного** компонента; документ прототипа принципиально не сохраняется с неопубликованным типом (инвариант `snapshotDefinitions`), и это остаётся в силе с feedback-3. Единственная durable-поверхность приёмки графа — **component case set** (`candidateOverlay` + overlay-форма slot-ребёнка); диагностические поверхности (превью компонента, `POST /compositions/:id/preview-tree`, render-status) принимают ту же карту и лишь **эхом** возвращают резолв узлов, ничего не сохраняя. Приёмки composition-целей не существует в принципе — `server/acceptance/` не знает composition.
3. **Межкадровый кэш барьера — non-goal этой волны.** Переиспользование результата барьера между кадрами с идентичным отпечатком ресурсов (worker-scoped прогретый кэш с инвалидацией по реестру) сознательно не строилось. Цена названа: **барьер исполняется на каждом кадре**, стоимость = `durationMs` × число кадров (локальный замер — порядка полусекунды на 256 ресурсов). Смягчение — реже снимать (`impactedSnap`) и переиспользовать browser-context пула; внутри кадра единственное переиспользование — in-page мемо по `resourceManifestHash`.

**KPI волны — как мерить (проводка по уже существующим данным, новой телеметрии не заводилось).** Baseline снимается **до** деплоя волны, иначе «после» ничего не доказывает.

- **Ревизий на выпущенный компонент** — `summary.revision` конверта `accept`/`accept-status` (ревизия кандидата, на которой семья принята); агрегация — проход по `links.json` клиентского кэша харнеса (`--cache-dir`/`EASYUI_CACHE_DIR`, [клиентский кэш](#клиентский-кэш-харнеса-волна-w7-план-2026-08-03-5)), где на компонент лежит связка candidate → run → cases.
- **`typedCausePct`** — доля провалившихся случаев, у которых верхняя причина типизирована: `summary.topCauses[]` того же конверта либо `causes[0].code` по случаям в `--json`.
- **`captured`/`reused`** — сводка `snap` (`node .claude/skills/author/driver.mjs snap <prototypeId> --impacted --summary-json`); план сам называет причину по каждому экрану, поэтому KPI «recaptured ≤ new + impacted» проверяется без ручного разбора.
- **Обращений за схемой = 0** — конверт самоописателен: `--summary-json` печатает ровно `envelope`, и агенту не нужен отдельный запрос к `/api/schemas/*`.
- **Хвост публикации** — «**1 resumable server workflow + 1 агентская запись**», а не «ноль ручных действий»: сага закрывает серверную часть (`phasesDone` квитанции), контрольные документы координатора (`WORKFLOW_STATE.md`/`BUILD_ORDER.md`) сервер не пишет и писать не будет.
- **Невыразимых поверхностей геометрии = 0** — `acceptance.comparisonSurfaces` перечисляет все четыре; корпус проверяется `bun scripts/audit-geometry-contract.mjs --db <копия>`.
- **«Преждевременных публикаций = 0»** серверного прокси не имеет: измеряется вручную по `BUILD_ORDER.md` координатора (доля lane'ов, где лист публиковался только ради родителя).
...
### Changelog платформенных capabilities (2026-08-08/09, снятие блокеров EUI-BR-01…10)

Волна плана `docs/plans/2026-08-08-blocker-removal-eui-br.md` (V0–V5) по фидбэку `docs/EASYUI_BLOCKER_REMOVAL_REQUIREMENTS_20260809.md`. Deliverable для координатора миграции — `docs/EASYUI_BLOCKER_REMOVAL_RELEASE_PACKAGE.md` (чеклист §16 по каждому требованию). Правило слоёв то же: каждое новое поле инвалидирует **ровно один** слой каскада — `frame` (пересъёмка) → `comparison` (re-diff) → `verdict` (recompute) → `report-only` (ничего). `acceptance.geometryContractVersion` волна **не двигает** (остаётся `2`): все новые факты замера аддитивны.

**`CASE_FINGERPRINT_ALGO_VERSION` тоже не двигается (остаётся `7`)** — и это сознательная замена механизма, а не пропуск. План §13 просил bump 7→8 против молчаливого reuse изменившейся семантики; реализация выразила ту же инвалидацию **двумя условными входами** — `comparisonPolicyVersion` (BR-04, слой сравнения) и `geometryOwnershipPolicyVersion` (BR-05, вердиктный слой). Bump ALGO обесценил бы reuse всего корпуса, включая случаи, которых волна не касается; условные версии двигают ровно те случаи, которые теперь судятся по новым правилам.

| Флаг / поле | Что означает | Слой инвалидации | Как гаснет |
|---|---|---|---|
| `features.prototypeSchemaResolverV2` + `features.prototypeSchemaResolverVersion` (2\|1) | BR-01: единый `ResolvedComponentGraph` на save/readiness/render-status/snap/preview-tree; пины композиции применяются только к её раскрытию (конфликт → `422 component_pin_conflict`), `track:head` резолвит голову в ДС закреплённой версии без fallback, неизвестный prop — типизированный `component_prop_unknown` с `resolvedVersion`/`sourceHash`/`propsSchemaHash`/`catalogRevision`/`acceptedKeys` | нет: путь save/readiness прототипа в приёмочные отпечатки не входит | `EASYUI_SCHEMA_RESOLVER_V2_DISABLED=1` → доволновая семантика byte-for-byte, версия честно `1`. **Один тумблер на 01a и 01b** — «включить только 01a» средствами env невозможно |
| `features.paintCapturePaddingV1` | BR-02: `cases[].paintPaddingPx {top,right,bottom,left}` — поле краски **по сторонам**; бюджет кадра `422 capture_budget_exceeded` (потолок — `limits.captureFrameBudgetMpx`), недостаточное поле — `paint_capture_clipped` с требуемым минимумом | **frame**, и только у случая, объявившего поле (условный спред). Канва сравнения от поля не зависит: кандидатский растр приводится к ней окном, метрики соседних случаев байт-в-байт прежние | `EASYUI_CAPTURE_V4_DISABLED=1` → `422 capture_padding_disabled` на `case-sets put`, кадр снимается скаляром |
| `features.exactContentHugCanvasV1` + `features.comparisonPolicyVersion` (2\|1) | BR-04: объявленная канва сводится **точно** (delta 0, без неявного zero-pad до `max(ref,cand)`), бюджет судится по поверхности сравнения (`rawDiffPctOfSurface`), эталон не того масштаба — `indeterminate reference_scale_mismatch` | **comparison** — через условный `comparisonPolicyVersion` в `comparisonFingerprint`: включение стоит re-diff'а, не пересъёмки | общий `EASYUI_CAPTURE_V4_DISABLED=1`; версия откатывается в `1` |
| `features.resourceBarrierV4` + `features.resourceBarrierPolicyVersion` (4\|3\|1) | BR-03: фаза `registry` до первого манифеста (реестр иконок темы, под-дедлайн 500 мс, мгновенный выход без темы), каналы `img-srcset`/псевдоэлементов/`font`/`icon-registry`/ожидаемых ассетов кандидата и overlay, пер-ресурсные записи контракта §6, сужение вердикта до `indeterminate resource_barrier_incomplete` **только** на барьерных причинах | **frame** у обоих профилей приёмки: политика входит в `readinessPolicyHash` → `policyProfileHash` → `rendererFingerprint`. Плюс `themeContentHash` — кадровый вход у случаев, у которых он есть | иерархия: `EASYUI_RESOURCE_BARRIER_DISABLED=1` гасит барьер целиком (каждый профиль в свою доволновую политику), `EASYUI_RESOURCE_BARRIER_V4_DISABLED=1` оставляет барьер по политике v3 **byte-for-byte**. Оба — **restart required** |
| `features.geometryDecorationOwnershipV1` + `features.geometryOwnershipPolicyVersion` (1\|null) | BR-05: аддитивные факты узлов вне потока (`preTransformBounds`, матрица, post-transform краска, причины участия в поверхностях), авто-правило decoration (прозрачность для `rootBounds`, неблокирующий `paint-overflow-decoration`), per-case `cases[].geometryOwnership` (потолок — `limits.caseSetMaxGeometryOwnership`), злоупотребление — `geometry_ownership_invalid` | факты замера — **report-only** (вне отпечатка, дифференциальный тест «замер расширился ⇒ кадр не сдвинулся»); авто-правило — **verdict** через `geometryOwnershipPolicyVersion`; манифестная декларация — **frame + verdict** у объявившего случая | `EASYUI_GEOMETRY_OWNERSHIP_DISABLED=1` → `422 geometry_ownership_disabled`, сбор и вердикт доволновые byte-for-byte |
| `features.flowOverflowOwnershipV1` | BR-09: `elements[].overflowOwnership {axis,mode,viewportOwner?,expectedContentOverflow?}` и одноимённый composition layout-токен; вклад поддерева владельца по объявленной оси ограничен границей scrollport'а; коды `unowned-overflow` / `owned-overflow-exceeds-axis` | **frame** у документов, объявивших владение (это персистируемая форма allowlist'а документа) | общий `EASYUI_GEOMETRY_OWNERSHIP_DISABLED=1` → `422 flow_overflow_ownership_disabled` **на записи**; чтение stored-документов не гейтится никогда |
| `features.acceptanceResumeV1` | BR-06: `POST /api/acceptance-runs/:id/resume` — продолжение **новым** раном с lineage (`resumedFromRunId`/`attempt`, идемпотентность `resume:<runId>:<attempt>`); шов `allocate-renderer` с собственным дедлайном, исходы `renderer_unavailable`/`allocate_timeout`, circuit breaker (`renderer_unavailable`/`capture_budget_exhausted`/`queue_starvation`), per-gate fingerprints в `gates_json`, причина падения случая в `error_json` | нет: reuse завершённых гейтов — только при совпавших per-gate отпечатках, иначе продолжение честно переисполняет | `EASYUI_ACCEPTANCE_RESUME_DISABLED=1` → `409 acceptance_resume_disabled`; без `EASYUI_ACCEPTANCE_MATRIX=1` — `false`. Миграция **v37** |
| `features.visualAttributionV2` | BR-07: per-node карта элементов кадра (артефакт `element-map.json`), owner-тоталы по **полной** diff-маске, контракт кластера §10 (`ownerElementKey`/`ownerComponentId`/`paintClass`/`structural`/`basis[]`/`confidence`), квитанция сравнения (matte/color profile/renderer/шрифты) | **report-only**: ни один вердикт от карты не зависит; сама карта вне отпечатков (дифференциальный тест) | `EASYUI_VISUAL_ATTRIBUTION_V2_DISABLED=1`; без матрицы — `false` |
| `features.rendererPolicyProfilesV2` + `acceptance.rendererPolicyProfiles[]` | BR-07: server-owned реестр профилей политики рендерера (`scope`, `maxResidualPct`, `expiry` по пяти отпечаткам), публикуемый **до** рана; применение — вторая инстанция визуального гейта, первый продюсер `exceptions[]` → `pass_with_exceptions`; профиль политики `default-v1-exceptions` в `acceptance.promotionPolicyProfiles` | **verdict** — и это единственная ось волны, которая **расширяет множество промоутабельных ранов** | `EASYUI_RENDERER_POLICY_PROFILES_DISABLED=1` → реестр пуст, `default-v1-exceptions` исчезает из `promotionPolicyProfiles`; без матрицы — `false` |
| `features.comparisonOwnershipV1` | BR-08: `comparison.ownership: "subject-and-integration"` + `subjectComponentId` + `dependencyPolicy`; два вердикта одного сравнения (subject-маска и полная канва), группировка исключённых пикселей по зависимостям; subject-promote непромоутабельного по integration рана при чистом субъекте и eligible-ранах всех зависимостей (`422 subject_promotion_{ownership_missing,subject_failed,dependency_ineligible}`) | **comparison** — и только у наборов, объявивших владение. Вердикт случая остаётся интеграционным | `EASYUI_COMPARISON_OWNERSHIP_DISABLED=1`; без матрицы — `false` |
| `features.blockerFingerprintV1` | BR-10: `blockerFingerprint` (`blk_<sha256>` от канонизированного basis + **сортированных** терминальных кодов; ни `runId`, ни время в пре-образ не входят) в виде рана, сводке и evidence + read-only `GET /api/acceptance-runs/:runId/retry-disposition` с глубиной `unchanged\|recompute\|rediff\|recapture\|rebuild` и советом `do-not-retry\|resume-run\|new-run\|update-source` | нет — ручка строго read-only (`no-store`), не создаёт ран и не пишет ни в одну таблицу | `EASYUI_BLOCKER_FINGERPRINT_DISABLED=1` гасит **обе** поверхности сразу (поле в ране и ручку); без матрицы — `false` |

`basis` ответа `retry-disposition` несёт четыре **версии политик волны** — `schemaResolverVersion`, `resourceBarrierPolicyVersion`, `comparisonPolicyVersion`, `geometryOwnershipPolicyVersion` (BR-10b). Все четыре **производные**, а не сохранённые: собственных колонок у них нет, они выводятся из текущего состояния сервера и профиля рана, поэтому в `changed[]` не попадают **никогда** (сравнивать значение с самим собой нельзя — тот же аргумент, что у `readinessPolicyHash`). Наблюдаемость у них другая: их **значение** говорит, под какой политикой ран пошёл бы сейчас, а вместе с ним меняется `blockerFingerprint` — кэшированный агентом блокер обязан перечитаться, а не пережить включение фичи под старым «do-not-retry». Глубину повтора называют слои: барьер → `frame` (recapture), сравнение → `comparison` (rediff), владение геометрией → `verdict` (recompute), резолвер схемы — **ни один** (кандидат приёмки резолвером прототипных пинов не параметризован; `rebuild` появляется только при сменившемся `sourceHash` головы компонента).

`limits` пополнились `captureMaxPaintPaddingPx`, `captureFrameBudgetMpx`, `caseSetMaxPreloadAssets`, `caseSetMaxGeometryOwnership`. Девять kill-switch'ей волны объявлены в `docker-compose.yml` — восемь с пустым дефолтом (= фича включена) и `EASYUI_RENDERER_POLICY_PROFILES_DISABLED` со значением `1` (профили меняют promote-eligibility и включаются отдельным решением). Без строки в compose переменная в контейнер не попадает вовсе, поэтому «выключить в Dokploy» было бы невозможно; следствие для выкладки — «deploy all-off» требует явной установки восьми переменных в env Dokploy, а не наступает сам. Порядок включения на проде, окна инвалидации, двухпушевой corpus-гейт и go/no-go — `.claude/skills/deploy/SKILL.md`, секция «Wave EUI-BR».

**Три зафиксированных ограничения волны** (названы, а не спрятаны):

1. **`ownerComponentId` пер-ресурсной записи барьера на странице всегда `null`.** Страница видит DOM, а состав случая (слот-привязки) живёт на сервере: он дозаполняет владельца при формировании receipt'а, если знает привязки. Соврать здесь ключом слота нельзя — это была бы догадка, выданная за факт, поэтому `null` остаётся честным «страница этого не знает».
2. **Атрибуция класса `registry-image` — маркерной гранулярности.** Кластер соотносится с ресурсом барьера через `ownerElementKey` ближайшего маркера (`data-eui-key`); внутренние узлы компонента ключей не имеют, поэтому «какая именно из двух иконок внутри одного маркера» атрибуция не различает. Карта элементов (S1) сужает это до узла с bbox там, где узел различим, но обещания «пер-иконочного» владения волна не даёт.
3. **`coveragePct` — отчётная величина, а не гейт.** Цель §10 фидбэка — ≥95 % атрибутированных пикселей; сервер считает owner-тоталы по **полной** diff-маске (не по усечённому `regions[]`) и честно кладёт неатрибутированные пиксели в `unknown`. Ни один вердикт от `coveragePct` не зависит, и низкое покрытие **не** смягчает вердикт: неатрибутированные пиксели делают кластер `structural`, а structural не смягчается ничем — ни профилем рендерера, ни общим бюджетом.
