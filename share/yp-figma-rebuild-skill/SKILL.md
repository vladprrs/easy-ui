...
Тот же цикл §4 с дополнениями:

- Сначала спроси: выражается ли это **composition** из готовых `pay-*`? Если у Figma-компонента нет собственного DOM/логики сверх раскладки детей — это composition, не TSX.
- Новый TSX оправдан, когда есть своя логика/состояния/семантика (карточка способа оплаты с anatomy, инпут с маской, свитч).
- Новый компонент уровня molecule/organism обязан нести `ownership: { reason: "<почему это не выражается composition из атомов>" }` в definition — без него publish падает с `422 atomic_policy_violation`. Это тот же аргумент несводимости, который ты уже сформулировал, выбирая TSX вместо composition.
...
- snap каждого экрана против экспорта соответствующего Figma-фрейма через `compare.mjs` — тот же порог; низ FlowRoot-футера capture может срезать за фолдом — это артефакт снимка, такие зоны сверяй скриншотом плеера (`?debug=1`);
