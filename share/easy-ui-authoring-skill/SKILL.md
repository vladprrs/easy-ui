---
name: easy-ui-authoring
description: Create custom components and clickable prototypes in easy-ui over its HTTP API — discover the component catalog, author a prototype JSON flow or a custom TSX component, publish them to the easy-ui server, and verify with render-status, geometry and server screenshots. Use when asked to create, add, update, or publish an easy-ui prototype or component.
---
...
- `definition.props` — Zod **strict** схема; `description: string` обязателен; опционально `slots?: string[]`, `example?`, `examples?` (до 8 именованных наборов props, имя — slug 1–32 символа, `default` зарезервирован), `atomicLevel?`, `capabilities?: {typedEvents?, namedSlots?, runtimeSchemaDefaults?}` (литеральные `true` — писать `{...} as const`; `runtimeSchemaDefaults` — хост применяет Zod-дефолты схемы к props, дублирующие `??` при этом удаляются, публикация даёт новый candidate id и требует новой приёмки семьи), семантика для валидатора (`interactive?`, `accessibleLabelProps?`, `urlProps?`). Сервер сохраняет провалидированный **input**, а не результат Zod transform/default.
...
node driver.mjs accept-status acc_… --case split-disabled     # drill-down: гейты, причины и квитанция одного случая
...
- **`--summary` — канон для агента; `--json` без него не меняется.** Полный отчёт failed-рана на 25 случаев — около 1800 строк (в каждом провале повторяются `metrics`/`regions`); `--summary` укладывает ту же приёмку меньше чем в 100 строк: `progress` со всеми счётчиками уровней reuse (`reused`/`frameReused`/`verdictRecomputed`/`rediffed`), `gates` строкой на гейт, `failedCases [{caseId, gate, raw, aa, cause}]`, группы ремедиаций и адрес evidence. Полный вид остаётся отладочным. Сводку считает сервер (`?view=summary` при `features.acceptanceSummaryView`), а старая сборка — не помеха: клиент сводит ту же форму локально (`summarySource: "client"`). Ссылки и квитанции кэша в обоих случаях строятся из **полного** рана.
- **Drill-down: `accept-status <runId> --case <caseId>`.** За подробностями одного случая идти сюда, а не за полным раном: гейты с `detail`, классифицированные причины, артефакты и **квитанция reuse** по уровням (`candidate/frame/readiness/geometry/visualMetrics/verdict` — `hit`/`miss`). Только она отвечает, пересчитывался ли вердикт: счётчик `reused` этого не различает. Exit 2, если случай упал.
...
- **Чем является ассет — говорит манифест.** `referenceSurface: "paint"` (дефолт) — ассет уже каноническая канва случая (прозрачное поле `margin` вокруг компонента); `referenceSurface: "content-hug"` — ассет обрезан по содержимому (штатный экспорт узла), и паддинг до канвы делает **сервер** (`referencePlacement` — смещение внутри канвы, device px; по умолчанию `margin × dsf`). `cropLineage.sourceSurface` отвечает, резать ли `rect`: `figma-node` (дефолт) — резать, `content-hug`/`paint` — уже вырезано, `rect` остаётся provenance'ом. **`content-hug` объявляется вместе с `expectedGeometry`**: без него корень канвы берётся из `layoutBounds` этого же рана, и случай с переиспользованным кадром получает `indeterminate reference_canvas_unresolved`.
